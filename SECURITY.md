...
- **控制台认证绕过 / 服务意外暴露**:未持有本地 token 即可调用 Web 监控台 API,或服务在默认
  配置下监听了回环以外的地址;拦截代理 `proxy.start` 同理——它签发并可向设备安装 CA,一旦
  绑到回环以外就是一个对全网可达的在途 HTTPS MITM,因此默认回环、空/空白 host 回落回环而非
  bind-all(`""` 会被 mitmproxy 解读为 `0.0.0.0`+`::`);显式绑定可路由网卡是使用方需自行隔离
  的部署选择,若空 host 仍被静默展开到所有网卡则按本类漏洞处理;
...
- **敏感信息泄露**:签名口令、token 等出现在错误信封、日志、产物或进程表中
  (现有实现把口令经环境变量而非命令行传给 apksigner——argv 对本机所有进程可见——
  并抹掉 stderr 里的口令,同类泄露按漏洞处理);
