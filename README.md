...
- 抓包（Web 与 Android 共用）：`proxy.start/stop/status/flows/websockets/flow.get/replay/export_har/ca.install_android`（mitmproxy 进程内）
...
321 个工具的读写归类（201 只读 / 120 写）在 `tools/catalog.py` 里逐个显式声明，策略在调用时
