...
- 集成 Gate 79 passed / 9 skipped（含 x86 与 x64 双架构、UI 自动化、r2/frida/windbg 可选后端、
  ...
  knowledge.query 的 kind 面板诚实经真实 MCP 端到端(agent 读 kinds 决定会话里有什么、任意平台)：
  knowledge 行按 kind 排序，故一页可能整页同一 kind——本 gate 用纯 stdlib 夹具钉住 kinds 描述的是
  整个会话而非本页:首页整页 aaa 仍报出会话里的每个 kind 且各计数之和等于 total、后页报同一面板、
  带 kind 过滤时只列该 kind 且 total 相符）
