...
- 集成 Gate 83 passed / 9 skipped（含 x86 与 x64 双架构、UI 自动化、r2/frida/windbg 可选后端、
  能力发现面经真实 MCP 服务器端到端（`capabilities.search/describe` 广告 r2/Ghidra/Android/Web 各线、
  过滤为诚实子集、未知 id 报 not_found、Windows 专属后端在非 Windows 如实报
  unsupported_on_platform、`meta.metrics` 对刚发生的调用如实记账,恒跑）、
