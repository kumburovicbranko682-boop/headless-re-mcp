...
- Web 静态：`js.deobfuscate/beautify/unpack_bundle`（webcrack）、`wasm.info/wat`（wabt）、`wasm.summary/strings/functions`（纯 Python 解析节表/导入导出/数据段字符串/函数签名，无需 wabt）；WASM 反编译复用 `ghidra.*` + ghidra-wasm-plugin
