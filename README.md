...
当前证据（在一台配好 x64dbg headless + Chrome/Playwright + mitmproxy + androguard + diec 的机器上实测；
...
- 集成 Gate 84 passed / 9 skipped（含 x86 与 x64 双架构、UI 自动化、r2/frida/windbg 可选后端、
  ...
  Detect It Easy 官方 CLI 二次意见端到端：`detect.scan(use_die=True)` 真的拉起 `diec -j`、把它的
  JSON 解析成 findings、并把有界原始输出落盘且登记进产物库——在真实 UPX 样本上 DIE 独立判出 UPX
  并与内置引擎一致，对其未打包原件则不误报 packer；缺 diec 时如实报 `unavailable` 而非当成扫过）
