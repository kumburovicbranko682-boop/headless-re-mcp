...
`HEADLESS_RE_EXTRAS=pe,web,android,browser,proxy ./scripts/install-linux.sh` 可扩展安装范围；含 `browser` extra 时脚本会自动执行 `playwright install chromium`（失败只警告不中断）。`HEADLESS_RE_INSTALL_BACKENDS=1` 会再经 apt 安装 Linux CI 同款 FOSS 后端（radare2 / UPX / wabt / apktool / apksigner / adb），并在有 curl+unzip 时拉取 CI 同款钉版 jadx（默认解包到 `~/jadx`，可用 `HEADLESS_RE_JADX_ROOT` 改，装好后请持久化脚本提示的 `HEADLESS_RE_JADX`，运行还需 JRE 21+），以及在 PATH 上有 npm 时 `npm install -g webcrack`（JS 线后端，需 Node 22/24）。`HEADLESS_RE_INSTALL_GHIDRA=1` 从上游 release 拉取 CI 同款钉版 Ghidra（约 400 MB，默认解包到 `~/ghidra`，可用 `HEADLESS_RE_GHIDRA_ROOT` 改）并装配套 PyGhidra，之后请把脚本提示的 `HEADLESS_RE_GHIDRA_HOME` 持久化；运行 analyzeHeadless 还需 PATH 上有 JDK 21+。
...
`session.create` 按扩展名与魔数自动判定目标类型（MZ→PE、`\x7fELF`→ELF、含 `AndroidManifest.xml` 的 zip→APK、`http(s)`/`.js`/`.wasm`→Web），也可显式传 `target`。PE 专属工具对非 PE 会话返回结构化 `target_mismatch`，不会深入后端才失败；ELF 会话直接走 `r2.*`/`ghidra.*` 等可移植后端，创建时同样识别 x86/x64 架构。
