...
- Web 静态：`js.deobfuscate/beautify/unpack_bundle`（webcrack）、`wasm.info/wat`（wabt）；`wasm.summary` 纯 stdlib 解析 WASM 结构（段布局、import/export 表、custom 段名），无需 wabt 即可用且返回结构化 JSON；WASM 反编译复用 `ghidra.*` + ghidra-wasm-plugin
