...
非 PE 那几条线的便携后端现在有一条自助 CI 通路：`.github/workflows/linux-integration.yml`（手动触发，对齐 `windows-integration.yml`）在 GitHub 自带的 Ubuntu 镜像上装齐 radare2 + C 编译器（portable-static）、wabt + webcrack（JS/WASM 静态）、Playwright Chromium（浏览器）、mitmproxy（抓包）、jadx + apktool（Android 反编译/重打包，都是自带的 Java 工具），再按非 PE 范围跑对应 Gate。portable-static（r2）此前只有 Windows PE 夹具、在 Linux/macOS 完全没有实测覆盖，现补了一条用系统编译器现编 ELF/Mach-O 小夹具的实测 Gate；更进一步，新增 `TargetKind.NATIVE`——按魔数把 ELF/Mach-O 认成原生会话（此前它们被当 PE、以 `not a PE file` 顶回，导致整条 `r2.*` 工具面在其老家平台通过正常建会话根本够不着），PE 专属调试器仍以 `target_mismatch` 拒绝原生会话，并补了「现编 ELF→建原生会话→`r2.open/functions/strings/imports` 逐一断言」的实测 Gate；原生会话还用纯 stdlib 的 `describe_native` 从头 64 字节解析出 `{format, bits, endian, type, arch}` 身份（与 PE 的 `architecture`、APK 的 `describe_apk` 同构），让 `get_session` 一眼看出是「64 位 x86-64 ELF 可执行」；抓包代理也补了「把经过它的流量真的录进环形缓冲并可取回/导 HAR、并能把录到的流重放回源站」的实测 Gate；浏览器动态线补了「按 requestId/scriptId 取回响应体与脚本源码、截图/HAR 落盘、导航换页、DOM 快照跟随换页、`web.wasm.list` 认出页面现场实例化的 wasm 模块」的实测 Gate（`test_web_dynamic_gate`）。
