...
- 集成 Gate 80 passed / 9 skipped（含 x86 与 x64 双架构、UI 自动化、r2/frida/windbg 可选后端、
  ...
  知识库会话记忆经真实 MCP 端到端：同 kind+key 幂等 upsert(replaced 由假转真、created_at 不变、
  value 就地更正)、query 按 kind 分组计数并可过滤且回读 value、按会话隔离、录入的事实进入
  report.generate(findings 计数与 Markdown)、无事实会话仍干净出报告)
