...
- 集成 Gate 80 passed / 9 skipped（含 x86 与 x64 双架构、UI 自动化、r2/frida/windbg 可选后端、
  ...
  状态跨真实服务重启持久化经两进程端到端：干净结束的会话不再活但审计/时间线/制品跨重启幸存且
  artifacts.read 冷读字节 sha256 精确吻合、崩溃遗留的开着会话重启后被 hydrate 回 session.list 与
  sessions.unclean 且 session.health 诚实报 healthy=None）
