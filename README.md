...
**Android 与 Web 两个目标域是新加的，成熟度仍低于 PE 那条链路，但差距在收窄**：契约（信封、读写分级、敌意输入）与降级路径有单元测试强制，且真机 Gate 现在也由 GitHub 托管的 `linux-integration` CI 作业在每次 push/PR 上跑——该作业装好 radare2、wabt、webcrack、Playwright Chromium、androguard/adbutils/frida、mitmproxy、以及 JRE + jadx + apktool + apksigner，实测的活路径包括：浏览器 CDP 驱动（打开/巡检/截图/脚本源码/导航/HAR 导出，外加**录下页面一条同源 `fetch` 请求再按 requestId 读回其状态码与响应体原文**）、抓包代理真机起停与端口释放并**真的录下一条经代理的 HTTP 请求再读回请求行与响应体**、把该 flow `replay` 一次并证明它**真的再次打到源站**（源站计数 +1、代理记成第二条 flow）、androguard 进程内解析一个**真实**（手工编码二进制 AndroidManifest 与 classes.dex 生成、无需 Android SDK）APK 的包名/权限/启动 Activity，并对内嵌的真实 DEX 做类/方法/字符串/xref 分析、还从一个手工构造 v1(JAR) 签名（`cryptography` 造 PKCS#7、无需 JRE）的 APK 读回真实签名者证书的序列号与 SHA-256 指纹（而这些此前都只跑失败路径）、jadx 在 JRE 上把这枚真实 DEX 反编译回 Java 源码（包/类/方法签名齐全，未知类走结构化 not_found）、apktool 把同一枚 DEX 反汇编成 smali 再重打包（decode/repack 往返，smali 里保留 jadx 折掉的 `const-string`）、apksigner 用 keytool 现造的密钥库给重打包产物签名并**由 apksigner 自己 verify 通过**（modify→repack→sign 闭环）、webcrack 拆真实 webpack bundle 与 `wasm-objdump` 段信息、radare2 活体分析 ELF（`tests/integration/test_r2_portable_elf_gate.py`）——而不再只在开发者本机手动跑过。整条 Android Java 工具链（jadx/apktool/apksigner）现都在 CI 内活体验证；仍未进 CI 的只剩真机设备（adb/frida 连真机），缺它时相关 Gate 会如实跳过，**skip 不等于 pass**。
