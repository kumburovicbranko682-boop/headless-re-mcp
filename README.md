...
- 集成 Gate 80 passed / 9 skipped（含 x86 与 x64 双架构、UI 自动化、r2/frida/windbg 可选后端、
  ...
  只读部署的写策略经真实 MCP 端到端：只读服务照常服务读、每个改动类工具被拒为 write_disabled
  (retryable=False、setting=local_full_access) 且仍在 tools/list 可见、写守卫先于操作(对不存在
  会话的 report.generate 报 write_disabled 而非 session_not_found)、无副作用,翻开标志后同一
  session.create 于同一 root 成功）
