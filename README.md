...
**Android 与 Web 两个目标域是新加的，成熟度仍低于 PE 那条链路**，但差距在收窄：契约（信封、读写分级、敌意输入）与降级路径有单元测试强制；可移植 Gate（Web CDP、Web 动态抓取（对真实本地页跑通 `network.list`/`network.get` 响应体/`har.export`/`screenshot`/`script.source`）、Web JS 反混淆与 webpack 拆包（webcrack）、WebAssembly 解码（wabt 处理真实模块的 `wasm.wat`/`wasm.info`）、抓包起停与端口释放及真实拦截/replay、多路并发抓包互不打挂、Android APK 静态分类、Android androguard 静态面（对 apktool 现搭的真实 APK 跑通 manifest/permissions/components/certificates/native_libs/classes/methods/strings/xrefs 并断言真实内容）、Android 反编译（jadx 经 `AnalysisService` 处理真实 DEX 并取回带方法体的 Java）、Android 反编译回编签名（apktool + apksigner 的补丁往返，改动穿过重打包与签名后仍在）、radare2 ELF 分析、整条 `r2.*` 服务面（在真实 PE 上端到端跑完 info/functions/strings/imports/exports/disasm/xrefs）、Ghidra headless 的 functions/decompile/symbols/xrefs（`ExportJson.py` 四种模式全覆盖）、本机 Frida 动态插桩（attach/模块与导出枚举/内存读取/探针 hook，对自起的子进程）、非 PE 会话的 knowledge/report 收尾工作流（`knowledge.record`/`query` 幂等更新与 `report.generate` 渲染，对 APK 会话且无需任何外部工具因此每次必跑）、Agent 工作台浏览器 E2E）现在由 `linux-integration` CI job 在托管 Ubuntu 上**每次提交真实执行**，不再只在恰好装了工具的机器上才跑。仍需真机的部分（真实设备的 adb，以及 USB/模拟器上的设备侧 Frida）在缺失时如实跳过，**skip 不等于 pass**。
