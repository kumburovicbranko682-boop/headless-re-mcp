...
- 集成 Gate 80 passed / 9 skipped（含 x86 与 x64 双架构、UI 自动化、r2/frida/windbg 可选后端、
  ...
  batch.analyze 批量三查韧性经真实 MCP 端到端：一坏样本不拖垮整批(缺失报 file_not_found、非 PE
  报 invalid_request、好样本照常开出可用会话)、succeeded/failed/count 准确、批量开出的会话是
  session.list 可见且可跑 report.generate 的一等会话、全坏批仍作为成功调用逐条报错）
