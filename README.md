...
- Web 静态：`js.deobfuscate/beautify/unpack_bundle`（webcrack）、`wasm.info/wat`（wabt）；`js.sourcemap` 纯 stdlib 读取 Source Map v3（列出原始文件树、标记哪些内嵌原始源码及其长度、汇总 mapping 形状），无需 Node/webcrack；WASM 反编译复用 `ghidra.*` + ghidra-wasm-plugin
...
- 集成 Gate 79 passed / 9 skipped（含 x86 与 x64 双架构、UI 自动化、r2/frida/windbg 可选后端、
  ...
  纯 stdlib 读取 Source Map v3 不依赖 Node）
