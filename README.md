...
**Android 与 Web 两个目标域是新加的，成熟度仍低于 PE 那条链路**，但差距在收窄：契约（信封、读写分级、敌意输入）与降级路径有单元测试强制；可移植 Gate（Web CDP、webcrack/wabt、抓包起停与端口释放及真实拦截/replay、Android APK 静态分类、radare2 ELF 分析、Ghidra headless 的 functions/decompile、Agent 工作台浏览器 E2E）现在由 `linux-integration` CI job 在托管 Ubuntu 上**每次提交真实执行**，不再只在恰好装了工具的机器上才跑。仍需真机或额外 CLI 的部分（真实设备的 adb/Frida、jadx/apktool 反编译回编）在缺失时如实跳过，**skip 不等于 pass**。
