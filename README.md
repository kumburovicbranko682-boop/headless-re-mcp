...
可移植原生后端（radare2、wabt 的 `wasm2wat`、webcrack，Android 写侧的 apktool/apksigner/jadx，以及 Web 动态所需的 Chromium）默认需自备。在 Debian/Ubuntu 上可用 `HEADLESS_RE_INSTALL_BACKENDS=1 ./scripts/install-linux.sh` 一并装好，或单独运行 `./scripts/install-linux-backends.sh`：脚本对已在 PATH 上的工具幂等跳过，用 apt 装 radare2/mingw-w64/upx/wabt/apktool/apksigner（mingw-w64 为 r2 与 UPX 的 PE 可移植 Gate 现编 Windows PE 夹具用，upx 即 `upx-ucl`）、用 `npm -g` 装 webcrack（需 Node 22/24）、从 GitHub release 拉 jadx（可用 `JADX_VERSION` 指定版本）。经由 `install-linux.sh` 触发时会自动带上 `browser` extra，再用 Playwright 装 Chromium 及其系统依赖（`playwright install-deps chromium` + `playwright install chromium`）——单独跑 `install-linux-backends.sh` 时若当前解释器里没有 Playwright 则如实跳过并提示装 `browser` extra。装上后 Web 的 `js.*`/`wasm.*`/`web.*`、radare2、以及 Android 的重打包/反编译相关 Gate 才会真正执行，而不是如实跳过（skip ≠ pass）。
...
但 Android 逆向不止于 DEX：几乎每个非平凡 App 都把原生代码以 `lib/<abi>/*.so`（ELF 共享库）形式带上，分析第一步就是枚举它导出的 `Java_...` JNI 入口、再读它们的反汇编/导入/字符串。可仓库里 `sample.apk` 的 `.so` 只是占位字节（androguard Gate 只读 zip 路径、从不加载），而既有 r2 Gate 分析的是可执行文件，于是 r2 在真实 `ET_DYN` 共享库上的**导出面**（`iEj`）从没真机跑过。新增的 `tests/integration/test_android_native_lib_gate.py` 现场用 `cc -shared` 编一个真实 `.so`（一个 `Java_...stringFromJNI` 导出 + 两个 native 辅助函数 + 一个 libc 导入 + 一段内嵌字符串，正是 Android 原生库的形状），对真实 r2 跑通 open/导出/函数/导入/字符串/反汇编：断言 `iEj` 把那个 JNI 入口列为导出（`is_imported=false`）并带可解析 VA、`aflj` 认出两个 native 辅助函数、`iij` 列出 `strlen`、`izj` 找到标记串、`pdj` 在该导出 VA 反出真实指令。为可构建性用 x86-64 编，但 r2 的读取面对 arm64 设备库完全一致。依赖 r2/rizin 与 C 编译器，缺则如实跳过（skip ≠ pass）。

radare2 侧此前的真机 Gate（`test_m11_r2_live_gate.py`）只开 Windows PE 夹具、只查一个函数的地址映射，在 Linux 上缺夹具就跳过，于是 r2 后端的解析面（`aflj`/`izj`/`iij`/`pdj`/`axj` 经 `enrich_r2_payload` 归一化）从没在这里真跑过。新增的 `tests/integration/test_r2_static_gate.py` 不依赖任何提交夹具:现场用 `cc` 编一个原生 ELF,对真实 r2 跑通整条读取面——`aflj` 列出编进去的 `add`/`compute`/`main`/`marker` 四个函数、`izj` 找到标记字符串、`iij` 列出 `printf` 等 libc 导入、`pdj` 反汇编 `compute` 出真实指令、`axj` 解析交叉引用(并断言请求地址被回显为归一化 Address)。既有的 PE Gate（`test_m11_r2_live_gate.py`）本应覆盖 rva/module 映射那一面，但它依赖一个提交进仓库的 Windows PE 夹具，而该夹具在 Linux CI 上并不存在——于是它在这里退化成编 ELF、走 va-only 分支，PE 特有的解析面（格式识别、ImageBase→rva 映射、PE 导入表）从没在 Linux 上真跑过。依赖 r2/rizin 与 C 编译器,缺则如实跳过(skip ≠ pass)。

于是补了 `tests/integration/test_r2_pe_portable_gate.py` 把这块补齐——不靠任何提交夹具、不碰 Windows：用 mingw-w64 现场交叉编一个真实的 PE32+（一个调 `GetCurrentProcessId` 的函数拉进 kernel32 导入、`printf` 拉进 CRT、一段内嵌标记串），再对 PATH 上的真实 r2 跑通 PE 特有的读取面。断言 `iI` 认出容器是 `PE32+`/`windows`(而非当作裸字节)、`aflj` 给出的函数地址同时带 `va` 与 `rva` 且 `module` 等于夹具名——rva 正是**关键判据**:ELF 没有首选装载基址、永远产不出 `rva`，它出现就证明后端确实按 PE 的 ImageBase 做了映射;`iij` 列出 `GetCurrentProcessId` 这个 kernel32 导入、`izj` 找到标记串、`pd` 在函数 VA 反出真实指令。这坐实了「便携后端」这条线:在 Linux 上不用任何 Windows 工具就能分析 Windows 二进制。mingw-w64 已并入 `install-linux-backends.sh`(实测 radare2 5.5.0 + mingw gcc 13),缺 r2/rizin 或 mingw 时如实跳过(skip ≠ pass)。

同一套「Linux 上现编 PE」的思路也把 **UPX 脱壳**这条便携线补上了。官方 UPX 路线此前只有 `test_m5_unpack_live_gate.py` 覆盖，而它要 Windows 的 `upx.exe`、提交进仓库的已打包 PE 夹具、动态那半还要 x64dbg——于是在 Linux 上整条跳过，`unpack.upx.test`/`unpack.upx.unpack` 两个工具从没在这里执行过，尽管底层适配器只是调 `upx -t`/`upx -d`、与格式无关。可服务层仍是 PE 绑定的（`session.require_pe` + 前后 `scan_pe` 对比），于是新增的 `tests/integration/test_upx_portable_gate.py` 用 mingw-w64 现场编一个真实 PE、用系统 UPX（`upx-ucl`）打包，再对这个会话跑通两个工具：`unpack.upx.test` 断言 `upx -t` 通过、回报版本号、原文件字节未变；`unpack.upx.unpack` 断言产出落盘、架构一致、原文件仍未变，而**关键判据**是导入表——UPX 把 IAT 塌缩成一个加载器桩（打包后只剩个位数导入），真正的 `upx -d` 必须把它重建回几十个导入（实测 5→48），外加区段数从 3（UPX0/UPX1/rsrc）恢复到 19；再用「明文标记串只在脱壳后的镜像里出现、在打包镜像里不存在」double-check 确实解压而非拷贝。upx 已并入 `install-linux-backends.sh`（实测 upx 4.2.2），缺 upx 或 mingw 时如实跳过（skip ≠ pass）。
...
Ghidra headless 此前只有降级路径（未配置时 `capability_unavailable`）的覆盖，真机路径无人跑过，于是藏了两个只在非 Windows 才暴露的 bug：启动器选择器在 Linux 上优先挑了 Windows 的 `.bat`（Popen 报 Exec format error），且 `ExportJson.py` 读了 Ghidra 根本不定义的全局 `ARGS`（正确 API 是 `getScriptArgs()`）导致每次导出都为空——两者都已修复。新增的 `test_ghidra_headless_gate.py` 现场编一个小 ELF、跑通 `ghidra.functions/symbols/xrefs/decompile`：断言编进去的 `add/compute/main` 都在、`add` 有来自 `compute` 的交叉引用、反编译出的 C 里含对 `add` 的调用。依赖 `HEADLESS_RE_GHIDRA_HOME`、JRE 与 C 编译器，缺则如实跳过。（本轮实测:在这台机器上装了 Ghidra 11.1.2 + JRE 21 后,该 Gate 4 项全过——便携后端 Ghidra 的 headless 路径确实能端到端跑通,而非只如实跳过。）

Ghidra 的价值在**反编译**,而 r2 那条原生库 Gate 只做到反汇编/导出这一层,于是「r2 分诊 → Ghidra 反编译」这条 Android 原生逆向的常规链路,其反编译那半此前从没在真实 `.so` 上跑过(既有 Ghidra Gate 分析的是可执行文件)。新增的 `tests/integration/test_ghidra_native_lib_gate.py` 补齐它:编与 r2 原生库 Gate 同形的 JNI `.so`(一个 `Java_...stringFromJNI` 导出 + `native_add`/`native_compute` 一对 + 一个 `strlen` 导入 + 一段内嵌字符串),对真实 Ghidra 跑通 `functions`/`symbols`/`xrefs`/`decompile`。**关键在反编译断言**:`native_compute` 反出的 C 必须同时出现它调用的 `native_add` 与 `strlen`(实测 `iVar1 = native_add(param_1,0x2a); sVar2 = strlen(SECRET);`),JNI 导出必须反出返回字符串符号的函数体(实测 `return SECRET;`)——证明反编译器在共享库里真的解出了调用图与数据引用,而不只是「分析跑完了」。为可构建性用 x86-64 编,但反编译路径对 arm64 设备库完全一致。缺 `HEADLESS_RE_GHIDRA_HOME`/JRE/C 编译器时如实跳过(skip ≠ pass)。
...
Web 动态（Playwright/CDP）此前的真机 Gate 只开一个 `data:` URL——它既不发网络请求也不加载外部脚本，于是**流量抓取那一面**（`web.network.list`/`web.network.get`/`web.script.source`/`web.har.export`/`web.screenshot`）根本没被跑过,而这恰恰是动态分析里最常用的部分。新增的 `tests/integration/test_web_dynamic_gate.py` 起一个本地 HTTP 源(一个 HTML 文档拉一个外部脚本、脚本再 `fetch` 一个 JSON),经 `AnalysisService` 打开真实页面后走完整条链:断言 `web.network.list` 抓到文档/脚本/fetch 三条 200(含 MIME)、`web.network.get` 取回 fetch 的 JSON 响应体、`web.script.source` 经 CDP 取回外部脚本源码并带标记、`web.console` 记到内联与 fetch 后两条日志、`web.dom_snapshot` 拿到渲染后的标题与正文、`web.screenshot` 落一张已登记的 PNG、`web.har.export` 导出的 HAR 里带着这几条。纯 HTTP、免 CA、免外网,跑的是 service 层(连带验证 CDP 接线、制品落盘与 capture 登记),缺 Playwright 或 Chromium 起不来时如实跳过(skip ≠ pass);Chromium 现已并入 `install-linux-backends.sh`。

同一条 CDP 链路上，页面的**主动驱动**与**运行时 WASM**此前也没真机覆盖：`web.navigate` 只有 mock，`web.wasm.list`（把 `Debugger.scriptParsed` 里 `scriptLanguage=WebAssembly` 的条目筛出来）从没被真实模块喂过。新增的 `tests/integration/test_web_drive_gate.py` 补上这两块：`web.navigate` 把一个已开会话从起始页驱到第二页，断言返回的 url/title 与随后 `web.dom_snapshot` 的标题/正文都换成了新页（而不是“调用返回 ok”就算数）；`web.wasm.list` 则让页面用 `WebAssembly.instantiateStreaming` 真的编译一个 41 字节模块（导出 `add`，wasm 字节内嵌、不依赖 wabt），轮询 CDP 脚本表直到那条 `WebAssembly` 记录出现，断言它指向 `/mod.wasm`、该视图不漏进任何 JavaScript、并通过 `add(2,3)==5` 的 console 日志证明模块是真跑了而非只被解析。纯 HTTP、免 CA、免外网，实测 Chromium headless；缺 Playwright/Chromium 时如实跳过（skip ≠ pass）。

抓包（mitmproxy）除了起停/端口释放的生命周期 Gate，现在还有一个端到端的抓包 Gate（`tests/integration/test_proxy_capture_gate.py`）：起一个本地 HTTP 源、把请求经代理转发过去，断言 `proxy.flows` 记到了这条流、`proxy.flow.get` 能取到请求/响应体、`proxy.export_har` 导出的 HAR 里带着它，并进一步用 `proxy.replay` 重放该流、确认请求被再次发出并二次捕获——纯 HTTP、免 CA、免外网。而抓包这条线真正的价值在**解密 HTTPS**，这块此前从没真机跑过：为此给 `proxy.start` 加了 `ssl_insecure`（等价于 mitmproxy 的 `--ssl-insecure`）——被分析目标常用自签或固定证书，默认校验上游证书时 mitmproxy 会回 502「certificate verify failed」、一条都记不到，这个开关（默认关、随 start 回执一并回报）让操作者能拦下这类源。配套的 `tests/integration/test_proxy_https_gate.py` 做端到端验证：用 `cryptography` 现场签一张 127.0.0.1 的自签叶证书、由本地 TLS 源提供，经 `proxy.start(ssl_insecure=True)` 走真实 `CONNECT` 隧道、客户端只信任代理自己生成的 CA；断言客户端拿到 200（证明代理成功假冒了源、中间人链路端到端成立）、解密后的 URL 与响应体落进 `proxy.flows`/`proxy.flow.get`/`proxy.export_har`，并以一个**载荷性的反向对照**收口：`ssl_insecure` 关闭时同一请求被 502 拒掉、什么都解不出——证明这个开关是拦截自签源的必要条件，而非可有可无的默认。纯自签、免外网，缺 mitmproxy（连带缺 cryptography）时如实跳过（skip ≠ pass）。Web 静态侧此前修过一个真实缺陷：`js.unpack_bundle` 在 webcrack 2.x 下必然报 “output directory already exists”（客户端预建了输出目录），加上 `-f` 后恢复可用。这一修复现已在真机上端到端验证过（实测 webcrack 2.16 + wabt 1.0.34，此前 CI 里没装这两个工具、Gate 一直如实跳过）：`tests/integration/test_web_re_gate.py` 的 `js.deobfuscate`/`js.unpack_bundle`/`wasm.wat`/`wasm.info` 全跑通，且把 JS 断言从“输出非空”收紧成“确实恢复出隐藏秘密”——夹具 `fixtures/web/obfuscated_sample.js` 把秘密藏成转义字面量 `\x48\x33...`，只有 webcrack 真正反混淆后其可读形式 `H3adl3ss` 才会出现（原始字节里并不含这个明文）；unpack Gate 进一步读回落盘的入口文件确认这个秘密在，顺带把“目录已存在导致空树”（缺 `-f`）那条失败路径也一并挡死。WASM 侧同样从“能跑”做深成“能看懂”：两个 wasm 测试此前只喂一个空模块（magic+version）或只带一个空 type 段的合成字节、断言输出里有 `"module"`/`"Type"` 就算过，根本没验证工具是否真的解出了函数、导入、导出与指令。现在改成用 `wat2wasm` 现场编一个真实模块（一个来自 `env` 的导入函数、一个导出的 memory、一个 global、两个带真实指令的导出函数），再断言 `wasm2wat` 反出 `i32.add` 函数体、`env.log` 导入、三个命名导出与 `i32.const 42` global，`wasm-objdump -h -x` 列全 Type/Import/Function/Memory/Global/Export/Code 各段并标注 `env.log` 与那三个导出名——实测 wabt 1.0.34。依赖 webcrack（Node 22/24）与 wabt（`wat2wasm`/`wasm2wat`/`wasm-objdump`），缺则如实跳过（skip ≠ pass）。
