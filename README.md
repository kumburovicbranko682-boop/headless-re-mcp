...
- Web 静态：`js.deobfuscate/beautify/unpack_bundle`（webcrack）、`wasm.info/wat`（wabt）、`wasm.sections/imports/exports/names/strings`（纯 Python 读二进制，免 wabt）；WASM 反编译复用 `ghidra.*` + ghidra-wasm-plugin
