...
- Android 动态：`frida.devices/device.connect/server.ensure/applications/spawn/device.attach/java.classes/java.methods`；`spawn` 授权自己拉起的进程，`device.attach` 探针挂接一个已在运行的 pid（取自 `applications`）并授权它，二者是 java 枚举/hook 的唯一入口；hook 复用 `frida.hook.template`（含 `android_ssl_unpin` / `android_crypto_monitor` / `android_root_bypass`）
...
266 个工具的读写归类（148 只读 / 118 写）在 `tools/catalog.py` 里逐个显式声明，策略在调用时
