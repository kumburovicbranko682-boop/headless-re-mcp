...
- Web 静态：`js.deobfuscate/beautify/unpack_bundle`（webcrack）、`wasm.info/wat`（wabt）；`js.webext` 纯 stdlib 离线检查浏览器扩展包（Chrome `.crx` v2/v3 与 Firefox/zip `.xpi`），读取 manifest 权限面（permissions/host_permissions/背景 worker/内容脚本/CSP）与归档文件清单，无需解压或浏览器；WASM 反编译复用 `ghidra.*` + ghidra-wasm-plugin
...
- 集成 Gate 79 passed / 9 skipped（含 x86 与 x64 双架构、UI 自动化、r2/frida/windbg 可选后端、
  ...
  纯 stdlib 离线检查浏览器扩展包 .crx/.xpi 不依赖解压/浏览器）
