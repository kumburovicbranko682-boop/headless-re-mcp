...
非 PE 那几条线的便携后端现在有一条自助 CI 通路：`.github/workflows/linux-integration.yml`（手动触发，对齐 `windows-integration.yml`）在 GitHub 自带的 Ubuntu 镜像上装齐 radare2 + C 编译器（portable-static）、wabt + webcrack（JS/WASM 静态）、Playwright Chromium（浏览器）、mitmproxy（抓包）、jadx + apktool（Android 反编译/重打包，都是自带的 Java 工具）、Temurin JDK 21 + 固定版本 Ghidra（`analyzeHeadless`，便携反编译），再按非 PE 范围跑对应 Gate。portable-static（r2）此前只有 Windows PE 夹具、在 Linux/macOS 完全没有实测覆盖，现补了一条用系统编译器现编 ELF/Mach-O 小夹具的实测 Gate；更进一步，新增 `TargetKind.NATIVE`——按魔数把 ELF/Mach-O 认成原生会话（此前它们被当 PE、以 `not a PE file` 顶回，导致整条 `r2.*` 工具面在其老家平台通过正常建会话根本够不着），PE 专属调试器仍以 `target_mismatch` 拒绝原生会话，并补了「现编 ELF→建原生会话→`r2.open/functions/strings/imports` 逐一断言」的实测 Gate；原生会话还用纯 stdlib 的 `describe_native` 从头 64 字节解析出 `{format, bits, endian, type, arch}` 身份（与 PE 的 `architecture`、APK 的 `describe_apk` 同构），让 `get_session` 一眼看出是「64 位 x86-64 ELF 可执行」，而这份架构还会一路串到 r2 工具输出——`enrich_r2_payload` 过去只从 PE 头推架构，ELF 没 PE 头就什么都不标，现在把 `session.architecture` 从服务层串到 `R2Client` 再到 enrich（PE 侧同值、零回归），ELF 的 `r2.functions/disasm/xrefs` 输出这才和 PE 一样带上 `architecture`；Ghidra 这条线此前零真机覆盖、且实为不可用（导出脚本还是 `@runtime Jython`，而 Ghidra 11.4 起移除了内置 Jython，`analyzeHeadless` 报成功却不写 JSON，整条 `ghidra.*` 静默失败；Linux 上 `_find_analyze_headless` 还先挑 `.bat` 启动器导致「Permission denied」够不着后端）——现把导出脚本移植成 Java GhidraScript（Ghidra 自编译、无 PyGhidra/Jep 依赖、产出 JSON 契约不变）、按 OS 择优挑启动器，并补了「现编不 strip 的 ELF→`ghidra.functions` 列出 `gate_root` 等自有函数→`ghidra.decompile` 在还原的 C 里认出对 `gate_leaf` 的具名调用→`ghidra.symbols` 带 name/address/type 交回符号表→`ghidra.xrefs` 对被调两次的 `gate_leaf` 解析出真实引用」逐一压全四种导出模式的实测 Gate（`test_ghidra_native_gate`，已在 Ghidra 12.1.3 实测通过）；抓包代理也补了「把经过它的流量真的录进环形缓冲并可取回/导 HAR、并能把录到的流重放回源站」的实测 Gate；浏览器动态线补了「按 requestId/scriptId 取回响应体与脚本源码、截图/HAR 落盘、导航换页、DOM 快照跟随换页、`web.wasm.list` 认出页面现场实例化的 wasm 模块」的实测 Gate（`test_web_dynamic_gate`）。
...
Frida 那条线此前唯一的真机 Gate 只认 Windows PE 夹具，Linux 上零覆盖；现补 `test_frida_local_gate`：对现开的子进程（Python 解释器自己）驱动真的 `FridaClient`，验 pid 授权闸、attach、模块与 libc 导出枚举、在 libc 基址读 4 字节断言 ELF 魔数（证明真读到目标地址空间）、canned 脚本加载，无设备/无外网。沙箱在 OS 层拒绝 ptrace 时按 frida 自己的措辞诚实 skip。
