...
- 集成 Gate 80 passed / 9 skipped（含 x86 与 x64 双架构、UI 自动化、r2/frida/windbg 可选后端、
  崩溃恢复经真实进程边界端到端（run 在 provider 流中被 SIGKILL -9 后,接管同一状态目录的新进程把死 run
  如实记为 interrupted:service_restarted 而非永远假装 streaming、事件轨迹留有 run.failed 记录、
  被杀的 mission 自动回队列并由新进程在预算内跑完,全程无人介入,POSIX 恒跑）、
