...
**Android 与 Web 两个目标域是新加的，成熟度仍低于 PE 那条链路，但差距在收窄**：契约（信封、读写分级、敌意输入）与降级路径有单元测试强制，且真机 Gate 现在也由 GitHub 托管的 `linux-integration` CI 作业在每次 push/PR 上跑——该作业装好 radare2、wabt、webcrack、Playwright Chromium、androguard/adbutils/frida、mitmproxy、以及 JRE + jadx + apktool + apksigner，实测的活路径包括：浏览器 CDP 驱动（打开/巡检/截图/脚本源码/导航/HAR 导出，外加**录下页面同源 `fetch` 请求再按 requestId 读回其状态码与响应体原文**（文本体读回原字符串、二进制体则 base64 解码后把真实字节溢写到产物文件并报 `body_bytes`）、以及页面 `WebAssembly.instantiate` 一个模块后经 `web.wasm.list` 列回该 WebAssembly 脚本）、抓包代理真机起停与端口释放并**真的录下一条经代理的 HTTP 请求再读回请求行与响应体**、把该 flow `replay` 一次并证明它**真的再次打到源站**（源站计数 +1、代理记成第二条 flow）、启代理后断言它铸出的拦截 CA 是一枚合法自签 X.509 根证书（`CA=True`、issuer==subject，即 `proxy.ca.install_android` 会推给设备的那张证书）、androguard 进程内解析一个**真实**（手工编码二进制 AndroidManifest 与 classes.dex 生成、无需 Android SDK）APK 的包名/权限/启动 Activity/打包的原生 `.so` 与其 ABI，并对内嵌的真实 DEX 做类/方法/字符串/xref 分析、还从一个手工构造 v1(JAR) 签名（`cryptography` 造 PKCS#7、无需 JRE）的 APK 读回真实签名者证书的序列号与 SHA-256 指纹（而这些此前都只跑失败路径）、jadx 在 JRE 上把这枚真实 DEX 反编译回 Java 源码（包/类/方法签名齐全，未知类走结构化 not_found）、apktool 把同一枚 DEX 反汇编成 smali 再重打包（decode/repack 往返，smali 里保留 jadx 折掉的 `const-string`）、apksigner 用 keytool 现造的密钥库给重打包产物签名并**由 apksigner 自己 verify 通过**（modify→repack→sign 闭环）、webcrack 拆真实 webpack bundle 与 `wasm-objdump` 段信息、radare2 活体分析 ELF（`tests/integration/test_r2_portable_elf_gate.py`）、frida **活体注入一个本地进程**（起一个子进程、`attach`、枚举模块与 libc 导出、按模块基址 `memory_read` 读回 ELF 头 `\x7fELF`——正是这条路径钉住了「用 `NativePointer.readByteArray` 而非已被移除的 `Memory` 全局」的现代 frida API 修复、并把 `noop` hook 模板在目标进程里真编译加载，未知模板得结构化 `invalid_params`）——而不再只在开发者本机手动跑过。整条 Android Java 工具链（jadx/apktool/apksigner）现都在 CI 内活体验证，frida 的进程注入/枚举/读内存也在 CI 内对真实本地进程验证；连 `mcp doctor` 自身对已装后端的探测也有活体 Gate（`test_doctor_gate.py`：装了的后端不得被报 `missing`、Windows-only 项在 Linux 上报 `unsupported_on_platform` 而非 `missing`），保证这份成熟度清单本身不会因探测漂移而虚报。仍未进 CI 的只剩真机设备（adb/frida 连真机 Android），缺它时相关 Gate 会如实跳过，**skip 不等于 pass**。
