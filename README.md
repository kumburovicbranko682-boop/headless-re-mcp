...
- 集成 Gate 81 passed / 9 skipped（含 x86 与 x64 双架构、UI 自动化、r2/frida/windbg 可选后端、
  ...
  报告身份面经真实 MCP 端到端(APK 身份事实进得了那份被留存的报告、任意平台)：`report.generate`
  渲染的是评审者会留存据以行动的唯一文档，而 Android 目标的关键事实(装了哪些原生 ABI、几个 dex、
  是否带 v1 签名)由 `describe_apk` 在 `session.create` 时免解包读出并存进会话——本 gate 用 stdlib
  夹具把这些事实做成可证伪而非装饰:一个已签名多 ABI 的 APK 报告把 Target 写作 apk、带 apk identity
  表、列出两个 ABI、数出两个 dex、signed_v1 读作 yes;一个未签名单 ABI 的 APK 报告 signed_v1 读作 no
  且 dex/ABI 各计一(数字随包而动、非常量);而 PE 报告把 Target 写作 pe 且不含任何 identity 段——这份
  身份呈现由目标驱动、不是硬钉在每份报告上的噪声）
