...
- 集成 Gate 80 passed / 9 skipped（含 x86 与 x64 双架构、UI 自动化、r2/frida/windbg 可选后端、
  ...
  浏览器跨线程驱动、关闭会话同时回收浏览器与抓包端口、长跑页面不按次泄漏句柄、.NET 托管元数据/IL
  读取面端到端：dotnet.inspect/enumerate/il/xrefs 在一枚真实程序集上还原出可核对的类型/方法/字段/
  IL/MemberRef，纯 Python 读取无需 .NET 运行时故恒跑）
