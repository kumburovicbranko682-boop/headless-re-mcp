...
- 集成 Gate 81 passed / 9 skipped（含 x86 与 x64 双架构、UI 自动化、r2/frida/windbg 可选后端、
  存储故障在真实 MCP stdio 服务下的诚实降级经端到端（库变只读时 session.create 照常工作但信封如实报
  persisted:false 并点名原因、库损坏时存储型工具回结构化 storage_unavailable 且标不可重试、整棵产物树
  被删后下一次调用原地重建目录与 schema,三种情形同进程无重启自愈,POSIX 恒跑）、
