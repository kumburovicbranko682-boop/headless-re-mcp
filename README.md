...
- 集成 Gate 83 passed / 9 skipped（含 x86 与 x64 双架构、UI 自动化、r2/frida/windbg 可选后端、
  Web 目标身份与降级经真实 MCP 端到端（.js/.wasm 开成带 binary+sha256 的 web 会话、URL 开成无 binary
  浏览器无关会话、无 playwright 驱动时 web.open 降级 capability_unavailable、未开浏览器时 web.navigate/
  screenshot 报 invalid_state 而非崩溃、apk.open 于 web 会话报 target_mismatch,恒跑）、
