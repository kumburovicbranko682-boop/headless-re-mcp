...
- 集成 Gate 81 passed / 9 skipped（含 x86 与 x64 双架构、UI 自动化、r2/frida/windbg 可选后端、
  ...
  浏览器跨线程驱动、关闭会话同时回收浏览器与抓包端口、长跑页面不按次泄漏句柄；新增 PE r2 服务面
  实测——对真 64 位 PE 走 `r2.open/info/functions/strings/imports/exports/disasm/xrefs` 全链，
  验证 x64 身份与镜像基址、地址映射、反汇编在函数入口对齐、xref 端点带映射地址，越界地址被拒）
