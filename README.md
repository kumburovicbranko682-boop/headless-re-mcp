...
- 集成 Gate 84 passed / 9 skipped（含 x86 与 x64 双架构、UI 自动化、r2/frida/windbg 可选后端、
  能力目录完整性经真实 MCP 端到端（每个 capability 广告的 tools 都在真实 tools/list 里、search 与 describe
  两视图一致、id 唯一、status 恒为合法探针状态、unknown id 报 not_found;每个 status_probe 都是 run_doctor
  的真实探针,防目录相对探针/工具面悄悄漂移,恒跑）、
