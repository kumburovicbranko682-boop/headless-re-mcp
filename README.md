...
- 集成 Gate 79 passed / 9 skipped（含 x86 与 x64 双架构、UI 自动化、r2/frida/windbg 可选后端、
  隐藏桌面隔离、连接掉线自愈、crackme 端到端、浏览器 CDP、Web 动态抓取
  （CDP 网络流/响应体/HAR/DOM/截图端到端）、抓包起停与端口释放、浏览器生命周期、
  浏览器跨线程驱动、关闭会话同时回收浏览器与抓包端口、长跑页面不按次泄漏句柄）
- 9 个 skip 均有明确原因：缺 .NET 样本（2）、未安装 Exeinfo PE（3）、未安装 webcrack（1）与
  wabt（1）、以及 2 个有文档说明的故意跳过
- Web 那条线本轮补了动态抓取的真跑证据：装上 Playwright 托管的 Chromium headless shell 后，
  起一个临时 localhost HTTP 服务（HTML 文档 + 外链脚本 + 被 fetch 的 JSON 端点），整条 `web.*`
  穿过 `AnalysisService` 对 headless Chromium 断言——CDP 抓到文档/脚本/fetch 三条网络流、
  回取某条响应体拿到带标记的真实 JSON、未知请求 id 返回 `not_found`、控制台原样抓到打印行、
  DOM 快照拿到标题与正文、截图落成真实 PNG 制品、HAR 导出回读为 JSON 且 entry 数与回执一致、
  非正导航超时被拦为 `invalid_params`——该用例本机通过
