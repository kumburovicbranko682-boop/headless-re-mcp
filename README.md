...
- 集成 Gate 80 passed / 9 skipped（含 x86 与 x64 双架构、UI 自动化、r2/frida/windbg 可选后端、
  ...
  .NET 托管检查纯 Python 经真实 MCP 端到端：合成最小托管 PE(真实 COR20+BSJB) 被 dotnet.inspect
  认成 verified_clr 的 pure_managed、enumerate/xrefs 走 dotnet_metadata 后端(非 IDA)、缺失 token
  的 il 报 not_found、无需 de4dot 或样本;合法原生 PE 被诚实判为 is_dotnet=False/not_dotnet 且各
  元数据工具拒为 not_dotnet）
