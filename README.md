...
- Android 动态：`frida.devices/device.connect/server.ensure/applications/spawn/attach.app/java.classes/java.methods`；hook 复用 `frida.hook.template`（含 `android_ssl_unpin` / `android_crypto_monitor` / `android_root_bypass`）
