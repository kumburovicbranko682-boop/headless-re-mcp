...
- Web 静态：`js.deobfuscate/beautify/unpack_bundle`（webcrack）、`wasm.info/wat`（wabt）；WASM 反编译复用 `ghidra.*` + ghidra-wasm-plugin；`web.har.inspect` 纯 stdlib 离线读取 .har 会话（无需浏览器/代理），按 host/method/status 过滤与分页汇总网络日志
...
- 集成 Gate 79 passed / 9 skipped（含 x86 与 x64 双架构、UI 自动化、r2/frida/windbg 可选后端、
  ...
  离线读取 .har 会话并按 host/method/status 过滤与分页汇总网络日志）
