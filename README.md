...
- 集成 Gate 82 passed / 9 skipped（含 x86 与 x64 双架构、UI 自动化、r2/frida/windbg 可选后端、
  APK 纯 stdlib 身份识别经真实 MCP 端到端（无 androguard 也能开:合成包按内容而非扩展名判为 apk、
  会话元数据带准 native_abis/dex_count/signed_v1、无壳解码器时 apk.open 诚实降级为 capability_unavailable
  而非 target_mismatch、无 manifest 的纯 zip 按非 PE 拒收,恒跑）、
