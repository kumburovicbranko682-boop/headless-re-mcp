...
### 新增（`apk.class_fields` 列出类声明的字段）

- 新增 `apk.class_fields`：`apk.methods` 的字段侧对应物。`apk.methods` 枚举一个类的方法，此工具
  枚举它的字段，于是类里的静态密钥、配置开关、缓存状态无需反编译即可看到。字段的 `descriptor` 是
  其类型（`Ljava/lang/String;`、`I`、`[B`），`access` 是标志串（如 "public static final"），
  二者合起来把常量和可变状态区分开。接受点分或 `Lsmali/` 形式的类名；与 `apk.field_xrefs` 搭配可
  查某字段在何处被读写。返回 `class_name`、`fields`（name/descriptor/access）、`count`、`total`、
  `offset`、`has_more`；`total` 为收集数，上限 2000，超出置 `scan_capped`。只读，工具面
