...
### 新增（`apk.field_xrefs` 字段读写引用）

- 新增 `apk.field_xrefs`：`apk.xrefs` 的字段级对应物。对静态密钥、配置开关、token 缓存来说，
  "谁碰了它"是关键，而且读写要分：只读的是常量，被写的是在某处被设置、值得一查。走 androguard 的
  字段 xref（`get_xref_read` / `get_xref_write`，返回 `(ClassAnalysis, MethodAnalysis)`），
  对每个同名字段（字段名跨类不唯一，可能多个字段贡献，和方法 xref 一样）收集访问点。每行给出
  `access`（"read" 或 "write"）与访问方法的 `class`、`method`，按三元组去重排序；`reads`/`writes`
  分别计数；`matched` 表示是否存在同名字段（区分"没有这个字段"与"有个字段但没人碰"）。返回
  `accesses`、`field_name`、`count`、`reads`、`writes`、`matched`、`has_more`，`limit` 上限
  1000 与 `apk.xrefs` 一致。只读，工具面 265→**266（149 只读 / 117 写）**。
