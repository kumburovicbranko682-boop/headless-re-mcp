...
能力发现面新增经真实 MCP 服务器的端到端 Gate（`tests/integration/test_capabilities_discovery_gate.py`）。Agent 规划前得先知道这套安装能做什么——`capabilities.search`/`capabilities.describe` 就是它发现有哪些后端(IDA、x64dbg、radare2、Ghidra、Frida、androguard/jadx/apktool、Playwright、webcrack、wabt、mitmproxy、ADB)、每个暴露哪些工具、在本机是 `ready` 还是 `missing` 的入口。目录本身已在单测里钉到真实工具名与 doctor 探针,但"这两个查询确实经传输层可达、过滤诚实、未知 id 报错而非回空对象"此前没有端到端守护。五条用例拉起真实 `python -m headless_re_mcp serve` 经 stdio 驱动:钉住发现面广告了项目正在成熟的非 PE 各线(radare2/Ghidra/Android/Web 均可见)、校验每条目形状且 status 属于 doctor 自己的状态词表、证明 backend 过滤精确收窄到单一后端且 status 过滤自洽(都是全集的严格子集)、`describe` 按 id 取回单个能力而未知 id 以 `not_found` 关闭失败;另钉平台诚实——Windows 专属后端(x64dbg / Win32 UI / WinDbg)在非 Windows 主机必须报 `unsupported_on_platform`,不许伪装成 `missing` 更不许 `ready`——以及 `meta.metrics` 对本会话刚发生的发现调用如实记账(`capabilities.search` 计数、零失败)。断言的是契约而非本机恰好装了哪些后端,故与工具链无关、恒绿。只依赖检出与已装的包,恒跑不 skip。
