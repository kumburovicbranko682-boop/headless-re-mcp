...
### 新增（`device.forwards` 列出设备上的 adb 隧道）

- 新增只读工具 `device.forwards`：`device.forward` 的取值一侧。forward/reverse 隧道存活在 adb
  服务器而非本进程，因此本工具列出绑定到该设备的全部隧道——本 agent 建的、以及人或别的工具建的
  （frida-server 端口、调试 socket），这是 `device.forward` 与 `release_forwards` 单独看不到的。
  回 `forwards`（每项含 `local` 与 `remote`），当前 adb 构建能读 reverse 时以同样结构回 `reverses`
  （读不了则整键省略而非给空列表，避免把「读不到」当成「没有」），外加 `count` 与 `has_more`，
  每方向截到 32 条时不会被读成全部。`device.*` 只读线补上隧道清单。
