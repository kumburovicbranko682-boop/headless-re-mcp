...
- `device.meminfo`（只读）：解析 `/proc/meminfo`，报告设备的内存画面——MemTotal、MemAvailable、
  SwapTotal 及其余内核大小计数，供逆向在依赖设备前评估机型档位与是否装得下重型插桩（frida agent、大
  dump）。`/proc/meminfo` 全局可读，无需 root。`meminfo` 把每个字段映射到以千字节（kB）为单位的值，
  与内核所标的 kB 一致；少数无单位的计数字段（如 HugePages_Total）被有意剔除，使每个值单位一致、调用
  方无需猜测。映射封顶（256）并带 `has_more`，读取结果为空（活设备不可能没有）会当作错误而非空映射。
