...
- 同一处再补齐最常见的一批控制流：表里只有 `br`/`brfalse`/`brtrue`，比较分支
  （`beq`/`bge`/`bgt`/`ble`/`blt` 及其无符号 `.un` 变体）全缺——短式占 0x2E–0x37、
  长式占 0x3B–0x44，还有退出 try 区的 `leave`/`leave.s`（0xDD/0xDE）。每个循环、每个
  `if` 都编成一条比较分支，每个异常处理块都以 `leave` 收尾，却统统落到未知操作码分支，
  只前进 1 字节、把分支位移（短式 1 字节 / 长式 4 字节）当成后续操作码，此后整段
  按短式 1 字节或长式 4 字节持续错位而 `partial` 仍为 false。所有分支位移都是相对下一条
  指令的有符号偏移，故新名与 `leave`/`leave.s` 一并并入 `_SIGNED_OPERANDS`：向后跳转
