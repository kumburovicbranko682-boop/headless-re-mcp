...
- `proxy.queries`（只读）：把整条 flow ring 里各请求 URL 的 query 参数做跨请求聚合——token、
  API key、跟踪 ID 常年明文躺在 query string 里，而 `proxy.flows` 一次只给一条 URL，聚合视图才
  能让「被上百个请求复用的同一个密钥」浮出来。回带 `params`（每条含 `name`、`count`、`hosts`、
  `sample_values`、`truncated`）、`param_count`、`flows_scanned`、`flows_with_query` 与
  `has_more`。`params` 按出现次数从多到少排序、封顶 256 个参数名，`count` 是跨 flow 的总出现次数
  （不是去重后的 URL 数）；每个参数只留少量去重样本值与少量主机，`truncated` 标记被裁剪的名字/值
  或超上限的主机/样本列表，`has_more` 表示去重后的参数名超过 256。装满 256 且 `has_more` 为
  false 才是全部参数。
