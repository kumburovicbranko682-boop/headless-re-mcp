...
- `device.features`（只读）：通过 `pm list features` 列出设备的硬件/软件特性——这与
  `device.packages`（已装应用）是不同的事实，装的是应用可以据以分流的平台能力：
  `android.hardware.telephony`、`android.software.webview`，以及带值的
  `reqGlEsVersion=196610` 之类。每行 `feature:<名字>` 收成一条（原样保留 `feature:` 之后的 token，
  于是带 name=value 的版本项也不丢），排序、封顶 1000 并回带 `has_more`；装满上限的一页不会被当成
  全部特性。一行 adb 报错（设备离线/掉线）按失败处理，而不是空特性集。
