...
### 新增（`apk.resources` 读取 resources.arsc 的字符串资源）

- 之前 Android 静态面能读 DEX 字符串池（`apk.strings`）、清单、组件、权限、证书、原生库，
  却读不到 `resources.arsc` 里的字符串资源——而硬编码的接口地址、后端域名、密钥别名、应用标签
  常常正落在 `res/values*/strings.xml`（编译进资源表）里，不在 DEX 里。要看它们此前只能
  `apk.decode` 解包整棵资源树，代价与副作用都大得多。新增只读工具 `apk.resources`：经
  androguard `APK.get_android_resources().get_resolved_strings()` 取出
  `{包名: {locale: {name: value}}}`，摊平成按 `package → locale → name` 稳定排序的
  `(package, locale, name, value)` 列表，与 `apk.strings` 同构地分页
  （`offset` / `limit` ≤ 2000，`count` / `total` / `has_more`，`total` 采集上限 5000 时
  置 `scan_capped`）；单值按 2000 字符截断，公开但在该 locale 无取值的名字（值为 None）不入表。
  另回传本资源表中的 `packages` 及其 `packages_has_more`。无 `resources.arsc` 的拆分包/无资源包
  返回空 `resources` 列表加一句 `note`，而不是后端错误。归类为只读（`agent_auto_execute`），
  工具面因此 265 → **266**（只读 148 → 149，写不变），README 三处工具算术与
  `tools/catalog.py` 计数同步更新。新增 `test_apk_resources_fields.py` 覆盖字段/分页/None
  跳过/无资源包/采集封顶，并把 `apk.resources` 纳入既有的负偏移 schema 与页上限一致性守卫。
