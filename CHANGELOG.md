...
可选原生后端诚实性新增经真实 MCP stdio 服务的端到端 Gate（`tests/integration/test_optional_native_backend_honesty_gate.py`）。radare2 与 Ghidra 是可选的;在这些 gate 运行的盒子上它们没装,而可选后端的全部意义就在于其工具对「为何跑不了」保持诚实:一个在没有 radare2 的主机上向 radare2 要函数的 Agent 必须得到「后端没装」,而一个 radare2 装了却崩的 Agent 必须得到「后端跑了并失败」——这是调用方会据以采取不同行动的两个答案(去装它,还是去看它为何坏)。把二者混为一谈、或以抛异常替代任一者,都会把 Agent 晾在原地。两个答案都不需要真工具,故本 gate 跨真实 MCP stdio 传输确定性地强制出二者、且与主机上恰好装了什么无关:缺失即 `capability_unavailable`——把后端指向一个不是文件的路径,便无论 PATH 上有什么都令其不可用,于是每个 r2.* 与 ghidra.* 都报干净的 `capability_unavailable`(在 PE 会话上如此,因可用性检查在先,在 APK 会话上亦然);在场但坏即 `backend_error`——把 radare2 指向一个真实但非零退出的可执行,便令其可用却失败,于是 r2.* 各报 `backend_error`(泾渭分明,绝非 `capability_unavailable`、也绝非抛异常);后端相互独立——radare2 已配置(尽管是坏的)而 Ghidra 仍缺失时,Ghidra 继续报 `capability_unavailable`,配置一个可选后端不会让另一个谎称自己在场。纯 stdlib 夹具、stdio 回环、无真后端;坏可执行那一半用 POSIX shell 脚本,缺失那一半任意平台。
