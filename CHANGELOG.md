...
- **`web.network.get` 把二进制响应体当文本处理**。`Network.getResponseBody` 对二进制体
  （图片、字体、wasm、gzip 等）回的是 base64 字符串，旧路径直接按文本走：容量上限按 base64 长度
  （比原始负载约大 33%）来量，一份实际能落盘的响应会被误判成 `too_large` 拒掉；溢出时又把 base64
  文本写进名为 `.bin` 的文件，`body_path` 拿到的不是真正的字节，抓到的 wasm/图片没法直接喂给
  `wasm.wat` 或查看器。现在二进制体先解码：上限按解码后大小量，溢出落盘的是原始字节，内联预览仍是
  base64（合法 JSON、可解码）但只取整三元组的头部、不超内联上限。文本响应行为不变。
