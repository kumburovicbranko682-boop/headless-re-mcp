...
- `device.cpuinfo`（只读）：按块解析 `/proc/cpuinfo`，报告设备的处理器布局——多少核，以及每个核的
  原始 key/value 行（架构、arm 的 `Features` 或 x86 的 `flags` 特性串、implementer，以及末尾可能的
  Hardware 块），供逆向判断 ISA 与某个原生库可能用到的 crypto/SIMD 扩展。`/proc/cpuinfo` 全局可读，
  无需 root。输出与架构无关：`blocks` 是逐块的 key→value 映射，原样保留内核所写；`processors` 统计带
  `processor` 键的块（真实核数，忽略末尾汇总块）。`blocks` 封顶（128）并带 `has_more`，超长的值会被
  截断并标 `value_truncated`，读取结果为空（活设备不可能没有）会当作错误而非空列表。
