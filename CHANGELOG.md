...
对象形态的 `reasoning` 增量不再被重复吐出。`openai_compatible._hidden_texts` 已在键循环里读过 `reasoning`（`_plain_text` 本就能从对象里取文本），后面却又有一段对象分支重读同一字段——当某些 provider 把 `delta.reasoning` 发成 `{"text": …}` 对象时，一个块产生两个 `reasoning_delta` 事件，orchestrator 又把它们拼接进本轮，思考文本被存成两份。删掉那段冗余分支后，字符串与对象两种形态都只发一次（`reasoning_content` 等其它键不受影响）。
