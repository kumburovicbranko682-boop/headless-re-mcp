...
### 新增（`web.frames` 列出页面 frame 树与各 frame 的 origin）

- 新增 `web.frames`：枚举页面的 frame 树，逐个给出 URL、origin 与父 frame。`web.dom.snapshot`
  只序列化顶层文档，跨域 iframe（支付表单、内嵌 OAuth、广告/追踪）在那份 HTML 里是黑盒，
  而整套 `web.*` 面既列不出这些 frame、也给不出它们的 origin。本工具直接读 Playwright 的
  frame 图（**不注入任何页面脚本**），因而对页面自身脚本都进不去的跨域 frame 同样可见。每个
  frame 是一行扁平记录，带 `index` 与父 frame 的 `parent`（主 frame 为 null），据此即可重建
  树而分页仍简单；`index`/`parent` 是整份收集列表里的绝对位置，跨页稳定。`origin` 是存储与
  Cookie 归属的安全元组（`about:blank`/`data:`/`srcdoc` 等不透明 origin 回 `""`），`name` 是
  frame 元素的 name/id。frame 数封顶 1000，`offset`/`limit`/`total`/`has_more` 与其余分页读取器
  一致。只读，工具面 265→**266（149 只读 / 117 写）**。
