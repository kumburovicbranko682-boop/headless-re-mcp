...
Agent 事件流的密钥脱敏新增经真实 HTTP+SSE 的端到端 Gate（`tests/integration/test_agent_redaction_gate.py`）。Agent 的事件流就是审计轨迹:运维实时盯着、事后回看它,而逆向的工具参数与结果里常带凭据(让模型试的一把 key、粘进提示里的 Authorization 头),orchestrator 在事件离开进程前按 key 名脱敏(并遮蔽字符串内的 bearer 值)。这条安全承重的性质此前无端到端证明。两条用例驱动真实 `serve-web`,假 LLM 提议一个参数塞满金丝雀密钥(api_key / password / session_token / 字符串内 `Bearer …`)的 `session.create`:full_access 下从实时 SSE 与持久 history 两处都断言 `tool.proposed` 事件把这些字段遮蔽为 `***REDACTED***`、非密钥参数(binary)原样保留、四个明文金丝雀在整份事件与线程往来里全程不出现;request 模式下 `approval.required` 审批卡同样脱敏,而用该卡片广告的 `args_sha256`(基于**原始**参数,脱敏不触碰摘要)审批仍然放行并真实分发,审批响应回吐的工具调用也脱敏。无真实 LLM、仅回环网络、纯 Python。
