...
### 新增（`apk.files`：枚举 APK 包内文件——assets/ 里的第二个 DEX 此前根本看不见）

- APK 工具面此前**没有任何办法枚举包内文件**：`apk.native_libs` 只看 `lib/`，其余内容（assets/、res/raw/、
  根目录）除非上重型的 `apk.decode` 全量解包，否则不可见。而 assets/ 里藏第二个 DEX、嵌套 APK/ELF/ZIP 正是
  Android 恶意样本动态加载的经典手法——分诊第一步"包里有什么"竟无从回答。
- 新增只读工具 `apk.files`（工具面 265 → 266，读 148 → 149）：文件名取 androguard 的解析结果（与全工具面在
  敌意 zip 上信任的同一解析器），排序、分页（offset/limit，上限 1000，越界在源头钳制），回 files/count/
  total/offset/has_more；`prefix` 参数可收窄到一个子树（如 `assets/`），total 计收窄后的名单。每条带
  `size`（取自 zip 中央目录，不解压任何条目）；中央目录读不动（敌意/损坏 zip）时**该条目只是缺 size 而不是
  从名单里消失**——缺席表示"没读到"，不是 0 字节。重名条目**不去重**：同名出现两次是真实信号（zip 条目遮蔽）。
- 三层接线齐全（client → `service_apk.apk_files` → MCP 工具 + catalog 只读声明 + MCP 面期望清单）。新增回归：
  真 zip → 排序名单带准确 size、描述点名字段；prefix+offset/limit → 收窄分页与 total/has_more 如实；
  损坏 zip → 名单照常（含重名）、全部缺 size。
