...
制品 GC 保留新增经真实 MCP stdio 服务的端到端 Gate（`tests/integration/test_artifact_gc_retention_gate.py`）。长跑部署会不断产出制品——报告、反编译、抓包——其磁盘不能无界增长。`artifacts.gc` 是守住这条线的工具:按最旧优先删除已登记制品,直到整棵树装进一个字节预算。有两条性质让它可无人值守运行,而两者都容易被悄然改坏:它按最旧优先逐出并在预算处停手,故它回收的空间与保留的制品恰是预算所蕴含的那些;它绝不删除最新的制品,哪怕预算比那个制品还小——回收空间不能把调用方刚产出、正要读的结果吞掉。两者在 MCP 边界上只用 `report.generate` 铸造制品即可观测,故本 gate 驱动真实 stdio 传输:生成若干已知大小的报告制品、从 `artifacts.list` 读回其大小与 newest-first 顺序,再钉住 `artifacts.gc` 在三种预算下的删/留/上报——一个放过全部的预算(零删、如实报全量大小)、一个差一个制品的预算(精确删最旧、保留最新三个、`bytes_remaining_estimate` 等于保留和)、一个小于单个制品的预算(仍保留最新那一个,再跑一次也不跌破该底)。纯 stdlib、stdio 回环、无分析后端、任意平台。
