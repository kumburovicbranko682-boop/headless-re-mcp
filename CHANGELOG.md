...
### 新增（`web.targets` 列出 context 内所有页面/标签）

- 新增 `web.targets`：列出浏览器 context 里的每个页面/标签，而不只是会话驱动的那一个。
  会话只驱动单个 page，但 `window.open` 弹窗或 `target=_blank` 链接会在同一 context 里再开一个
  page——OAuth/SSO 同意步骤、支付窗口正是这样出现的——而整套 `web.*` 面此前既看不到这些
  page 存在、也拿不到它们的 URL。本工具读 Playwright 的 `context.pages`。每个 page 是一行，带
  `url`、`title`、`is_active`（本会话驱动、`web.screenshot`/`web.dom.snapshot` 读的那个）与
  `opener`（开出此 page 的父 page 索引；顶层标签或 opener 未知时为 null）；`index`/`opener` 是整份
  列表里的绝对位置，跨页稳定。读取中正在关闭而抛错的 page 回空 url/title，不拖垮整次枚举。
  page 数封顶 200 并以 `targets_capped` 如实上报，`offset`/`limit`/`total`/`has_more` 与其余分页
  读取器一致。只读，工具面 265→**266（149 只读 / 117 写）**。
