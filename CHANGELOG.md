...
Web 线诚实降级新增经真实 MCP stdio 服务的端到端 Gate（`tests/integration/test_web_identity_degradation_gate.py`）。既有 Web gate 只在 Chrome、webcrack、wabt 都在场时才是真的;在裸机上它整个 skip,于是 Agent 在没有这些后端的盒子上真正会遇到的行为——初次接触 web 目标的常见情形——从未被证。而那行为正是全部契约:什么都没装也能开一个 web 会话,且每个重型工具都以 Agent 可推理的结构化信封作答,而不是崩溃、协议错、或工具凭空消失。本 gate 对一个没有浏览器、没有 JS/WASM 后端的服务驱动真实 MCP stdio 传输,钉住:身份无需后端——一个 `.js` 文件和一个 `http` URL 各自开成 `web` 会话,不碰网络也不需要 Chrome;静态文件工具降级为结构化信封——`js.*` 与 `wasm.*` 在 webcrack/wabt 缺失时报 `capability_unavailable` 而绝不抛异常,非 WebAssembly 输入也被干净处理;浏览器工具会叫 Agent 先开浏览器——`web.scripts` 及其同类在什么都没开时返回 `invalid_state`(非崩溃),且这与 Chrome 是否安装无关,`web.open` 降级为结构化信封,`web.close` 对空闲会话是干净的 no-op;异目标工具双向拒绝 web 会话——PE-only 的 `static.open` 与 APK-only 的 `apk.open` 各自答 `target_mismatch`。纯 stdlib 夹具、stdio 回环、无需后端、任意平台。
