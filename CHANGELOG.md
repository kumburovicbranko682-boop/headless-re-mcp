...
- `proxy.cookies`（只读）:扫保留的抓包 flow,归集观察到的 cookie,免去逐个 `proxy.flow.get` 翻头。
  用 mitmproxy 自己的 cookie 解析(`response.cookies` / `request.cookies`),回 `cookies`,每条带
  `name`、`value`（token 本体,封顶 2048 字符,超长(如大 JWT)时置 `value_truncated` 与 `value_length`,
  避免把截断值当完整值拷去重放会话)、`domain`、`path`、`source`（服务端 Set-Cookie 记 "response"、
  客户端回带的 Cookie 记 "request")与 `secure`/`http_only`/`same_site`（仅 response cookie 带这些标志,
  request cookie 只有名值、空 path、无标志)。按 (name, domain, source) 去重,一条一个 cookie 而非一条
  一个 flow;response cookie 取最新一次。只扫仍保留的 flow——`flows_scanned` 与 `flows_omitted`（正文被
  环淘汰、连头都读不到的 flow)说明读了多少、丢了多少,故短列表配上 flows_omitted 不会被误读成「cookie 少」。
  Set-Cookie 属性大小写无关解析。列表字段是 cookies 而非 items。
