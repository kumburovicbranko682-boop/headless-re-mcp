...
Agent 工作台。工具面从 199 增至 **341（223 只读 / 118 写）**；读写分级在
...
### 新增（Android：Dalvik 字节码反汇编）

- 新增 `apk.disassemble`:把 Android 静态线补上"看方法体"这一层——`apk.method_info` 只给签名与访问标志、
  `apk.decompile` 要外部 jadx,唯独没有一条不依赖反编译器就能读某个方法真实字节码的路。这个直接从
  androguard 拿 EncodedMethod 的 code item,逐条走 Dalvik(smali)指令。按类(点分 `com.x.Foo` 或 `Lcom/x/Foo;` 形式)
  与方法名解析;名字有重载时用 descriptor(如 `(I)V`)挑一个,否则取第一个带方法体的重载并置 ambiguous。native/
  abstract 方法没有字节码,回 has_code=false。回 class_name、method_name、descriptor(选中的重载)、params、return_type、
  access、ambiguous、overloads(同名个数)、has_code,再加 instructions(分页)、count/total/offset/has_more 与
  scan_capped(指令上限)。每条指令带 offset(方法 code item 内的字节偏移)、mnemonic(Dalvik 助记符,如 invoke-virtual)、
  operands(渲染后的操作数,分支目标按 offset 解析)、size(字节),另在可得时带 opcode(数值)与 hex(原始编码)。
  类或方法找不到回 not_found;会话不是 APK 回 target_mismatch。
...
- 新增 `ghidra.exports`:`ghidra.imports` 的对偶——这个二进制对外提供的符号:共享库导出的函数、可执行
...
- 新增 `wasm.exports`:纯 Python 列出模块导出并解析函数导出的签名。summary 只按 name/kind/index
  列;这里是带类型的导出 API 面。分页回 `exports`、`count`/`total`/`offset`/`has_more`,加
  `imported_func_count` 与 `types_resolved`(类型节解析不了时为 false)。每行带 `name`/`kind`
...
- 新增 `wasm.names`:纯 Python 从 `name` 自定义节还原符号表——保留名字的模块上最有价值的一读。
  `wasm.functions` 只借用函数名,这里把整个 name 节倒出来:模块名、函数名映射,以及(别处都不给的)
  每函数的局部/参数名——正是它们让反编译可读。回 `has_name_section`(stripped 模块为 false)、`module`
  (模块名或 null)、`functions`(按索引排序的 index->name,分页)、`function_count`/`function_total`/
  `offset`/`has_more`,再加 `locals`、`local_function_count` 与 `locals_truncated`。每个 locals 行带
...
- 新增 `wasm.globals`:纯 Python 列出模块定义的全局变量(节 6)。summary 只给计数,这里逐个命名:
  ...
  i32.const/i64.const 的 `value` 或 global.get/ref.func 的 `index`;多指令初始化时 op 为 "complex"。
  分页回 `globals`、`count`/`total`/`offset`/`has_more`,加 `imported_count` 与 `resolved`
  (节解析不了时为 false),全局数触顶置 `scan_capped`。
...
- 新增 `apk.intent_filters`:把每个组件的 `<intent-filter>` 摊开——应用声明的入口点。`apk.components`
  只给名字,这里给外界怎么触达:组件应答的 action/category,以及让它成为深链/自定义 scheme 处理器的
  data 过滤(scheme/host/path/mimeType)。只回真正声明了过滤器的组件。回 `components`、`count`、
  `total`、`has_more`。每个组件带 `type`(activity/service/receiver)、`name`、`exported`(true/false,
...
- 新增 `apk.urls`:从 DEX 字符串常量里蒸馏网络指标(URL/主机/IP)。`apk.strings` 列出所有常量,这个
  只留网络相关的——分诊最先想看的 C2 端点、API base、追踪信标与硬编码 IP——去重并分类成一份清单。
  URL 按 http/https/ws/wss/ftp 匹配,剥掉尾部标点,拆成 scheme 与 host。回 `urls`(分页排序)、`count`/
  `total`/`offset`/`has_more`;`hosts`(按有多少条不同 URL 指向它排名的每主机计数)加 `host_count` 与
...
- 新增 `apk.security`:一眼给出 `<application>` 元素的安全姿态——审阅 Android 包时第一件
  要看的。回 `package`、`debuggable`、`allow_backup`、`uses_cleartext_traffic`、
  `network_security_config`、`application_class`(自定义 `Application` 子类,常是加固/加载器
...
- 新增 `web.frames`:列出页面的 frame 树——主文档与每个 iframe。嵌入内容视图:广告框、第三方支付
  iframe、隐藏的点击劫持覆盖层、沙箱小组件,每一个都加载并运行顶层页面没写的代码。回 `url`(主 frame)、
  `frames`、`count`、`total`、`truncated` 与 `cross_origin_count`(host 与主文档不同的子 frame——值得看的
