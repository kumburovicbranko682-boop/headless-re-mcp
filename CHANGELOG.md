...
### 新增（`web.page` 读取页面当前 URL / 标题 / 加载状态）

- 新增只读工具 `web.page`：读取页面**此刻**的 `url`、`title` 与 `ready_state`
  （`document.readyState`：loading / interactive / complete）。`web.navigate` 回的 url/title 是
  goto 那一刻的快照，导航之后的 JS 跳转或 SPA 路由切换会让它变旧；`web.page` 每次都取活值，
  于是「落地页到底是哪个」在重定向后仍能问清。URL 被缓冲区截断时带 `url_truncated`；页面无法
  求值（求值被拦、文档已不在）时**省略** `ready_state` 而非猜一个会被读成已就绪的值。不含 html
  字段——要文档用 `web.dom.snapshot`。取值走 CDP 会话所在的 runner 线程，与 `web.dom.snapshot`
  同一范式。Web 动态线 `web.*` 工具随之 12 → 13。
