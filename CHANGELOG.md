...
畸形 .NET 输入新增经真实 MCP stdio 服务的零事故 Gate（`tests/integration/test_malformed_dotnet_no_incident_gate.py`）。`dotnet.inspect`/`enumerate`/`il` 是纯 Python 的 ECMA-335 读取器，无外部依赖、任何盒子都跑——而它啃的是攻击者控制的字节。这类解析器的契约是:每个畸形输入都变成一个点名问题所在的结构化裁决，绝不是 `internal_error` 事故（那是本进程自身缺陷的收容所），也绝不挂起;元数据表尽可以声称二十亿行，读取器不许信。本 gate 以 stdlib 在一个已知有效的最小 CLR PE 上只损坏 CLR 层来端到端钉住（PE 本身仍可解析、真正走进托管读取器而非在上游被挡）:COM 描述目录为空诚实是 `not_dotnet` 且 enumerate 如实说;声明了 CLR 目录但元数据缺失、非 `BSJB` 或被截断者 inspect 为目录*线索*（`clr_directory_hint`）、enumerate 为 `clr_unverified`——读取器不假装表在那里;一个在 48 字节表体后声称 0x7fffffff 行的 `#~` 表流被限到流真正能放下的行数（total=3）而非 materialise 二十亿行（即文档记载的那条守卫:一个 60KB 文件曾以此吃掉 1.2GB 堆与二十五秒）;`dotnet.il` 对非 MethodDef 令牌与 rid 0 报 `invalid_argument`、对越界 rid 报 `not_found`。另附一个有效空 CLR 壳作锚，证明 gate 不是靠上游全拒来空洞通过。纯 stdlib 夹具、stdio 回环、无真后端、任意平台。
