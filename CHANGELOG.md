...
- **抓包缓冲无界**。摘要环是有界的，但保存完整 flow 对象（含报文体）的那份是普通 dict，
  ...
  已被环形缓冲淘汰，而不是假装不存在。
- **`proxy.export_har` 不限大小，和它的 Web 兄弟不一致**。摘要按字段有界，但一整环接近上限
  的 URL 仍会序列化成数十兆的 HAR，先落到产物根、再等留存回收；而 `web.har_export` 会丢掉
  最新的条目直到 HAR 落在采集上限内并如实上报。现在 `proxy.export_har` 采用同一逻辑：超过
  `UNREGISTERED_CAPTURE_MAX_BYTES` 就逐批丢弃最新条目并置 `truncated`，返回里补上 `size`；
  连空壳都放不下时报 `too_large` 而不是写出超限文件。
