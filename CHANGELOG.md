...
- `device.unix_sockets`（只读）：解析 `/proc/net/unix`，列出设备的本地 IPC 面——进程为进程间通信绑定的
  具名端点，既包括 `/dev/socket` 下的文件系统套接字，也包括抽象命名空间名（以前导 `@` 打印，如
  `@jdwp-control`）。这是逆向查看某应用或系统对外暴露了哪些服务的入口，与 `device.connections` 里的
  TCP 套接字互补。`/proc/net/unix` 全局可读，无需 root。只返回具名套接字：匿名套接字没有可标识的 path，
  且占表的大多数，故有意剔除以让结果就是真实的 IPC 面（是有意省略而非静默丢弃）。每条含 `path`、`type`
  （STREAM/DGRAM/SEQPACKET 或原始十六进制）、原始 `state` 十六进制与 `inode`。列表封顶 1024 并带
  `has_more`，始终读不到内核表头（拒绝、缺失）才算错误。
