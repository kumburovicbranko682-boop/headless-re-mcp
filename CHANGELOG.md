...
纯 Python 的发现存储面（`knowledge.record` / `knowledge.query`）新增端到端 Gate（`tests/integration/test_knowledge_store_gate.py`）。该存储是 Agent 与 `report.generate` 回读的持久记忆,其“同 `(kind, key)` 原地替换、值大小上限、按 kind 过滤、分页”这些语义此前没有独立守护。五条用例对着提交的 PE 夹具跑:记录/查询往返;同键再记录时原地替换(总数仍为 1、`created_at` 保留、`replaced` 由 false 翻 true)而非追加;查询按 kind 过滤、给出 `{kind: count}` 汇总、并以诚实的 `has_more` 在不重叠的窗口间分页;值超上限与 kind/key 空或超长各以 `invalid_request` 关闭失败;且会话关闭后读仍可解析、写则失败关闭。测试期无需任何工具链,故此 Gate 恒跑不 skip。
