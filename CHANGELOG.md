...
- **`apk.subclasses`**：给定类或接口名，列出扩展或实现它的内部类，是 `apk.class_info` 继承视图
  的反向查询——不是「这个类继承谁」，而是「谁继承/实现了这个类型」，用来枚举全部 Activity
  （`Landroid/app/Activity;`）、某回调接口的全部实现、或某自定义基类的全部子类。目标类型无需在
  APK 内定义，因此框架类是合法查询、永不报 `not_found`。每条匹配是 `class_name` 与 `relation`
  （`extends` 表示目标是父类，`implements` 表示目标是所实现的接口；一个类只继承一个父类、实现多个
  接口，故两者按行互斥）。接受点名或 `Lsmali/` 形式，跳过 external 类。结果按 `class_name` 排序，
  `total` 为收集数、封顶 10000（超出置 `scan_capped`），`offset`/`has_more` 分页。只读工具。
