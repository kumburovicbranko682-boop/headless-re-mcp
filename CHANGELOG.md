...
- **`artifacts.read` 只回 `size`，没有 `has_more`，翻页者停在开头**。面上其它分页读全都回
  `has_more`，让「填满 limit 的一页」不被当成整体；这里却只给 `size`，调用方得把 hex 解回、
  数出字节、再和 `size` 比才知道还有没有。一个用默认 4 KiB limit 去读 200 MB 转储的翻页器只看到
  `data` 而无 `has_more`，便会停在文件头。现在补 `bytes_returned`（本页实际字节数——`limit`
  只是请求量、末页更短）与 `has_more`（`offset+bytes_returned` 之后仍有字节即为真），调用方无需
  解码即可推进 `offset`。新增回归覆盖满页/中段/末页短读、EOF 及越界读、单页读完整文件，及描述点名
  两个新字段。
