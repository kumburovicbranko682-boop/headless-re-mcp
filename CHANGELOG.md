...
遥测计数准确性新增经真实 MCP stdio 服务的端到端 Gate（`tests/integration/test_metrics_telemetry_gate.py`）。`meta.metrics` 是无人值守部署用来了解「做了多少活、失败了多少」的抓取口——per-tool 调用数、失败计数、延迟分位,外加可安全用于告警速率的 lifetime `*_total` 计数器。这些数一旦与实际发生的偏离,建立其上的每块看板与错误预算告警都在悄然说谎,要到事故时才被发现。本 gate 在裸机上钉住若干锋利边角:每次调用都记到正确的工具与正确的结果——返回 `ok=False` 信封即失败、`ok=True` 不是,且 per-tool 累计与聚合逐项对账;新启动的进程报诚实零值(空 `tools`、零计数、真实 `capacity`),不是 `null` 也不是错误;lifetime `*_total` 累计且快照之间绝不回落,而 `meta.metrics` 会计入自身既往调用(它也是被 instrument 的工具),故据其算出的速率单调;`limit` 只截断 `recent` 窗口(newest-first)、绝不改动聚合计数。一个 gate 依赖的微妙点:`meta.metrics` 的记录发生在其 handler 读完 ring *之后*,故任一快照从不计入自身、只计已在其之前完成的调用。全部跑在真实 MCP stdio 传输上、对一个 ring 起始为空的新服务;纯 stdlib、任意平台。
