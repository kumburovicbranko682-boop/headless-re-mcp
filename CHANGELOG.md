...
修复浏览器 Agent 工作台冒烟 Gate（`tests/integration/test_agent_browser_smoke.py`）在非 Windows 主机上一直硬失败的两处陈旧:一是用写死的 `C:\Program Files\...\chrome.exe` 启动 Chromium(缺该路径即抛异常而非 skip),二是仍在断言早期英文 SPA 的文案与 `RunProgress` 事件字面量,而工作台早已整体本地化为中文、`RunProgress` 也不再渲染这些事件串,故此测试实际上在任何机器都跑不过。现改为:用与 Web 后端一致的方式启动 Playwright 自带 Chromium(依次尝试 `HEADLESS_RE_TEST_CHROME` 覆盖、自带 Chromium、系统 Chrome `chrome` 通道,全不可用则带原因 skip 而非失败);全部选择器改指当前中文 UI(落地标题、消息输入框、发送/设置按钮、供应商字段、批准/拒绝按钮),并把“本轮完成”标记限定到可见的 `.transcript`,不再误锁检查器里隐藏的 `message.delta` `<pre>`;审批路径改用 `patches.apply`(FILE_WRITE + STATE_CHANGE,在默认策略排除名单内,故即便加壳分析预设也会停下等待,而裸 `state_change` 会被自动批准);重载持久化改为断言恢复流从持久化游标续读(`after=<seq>`)且批准/拒绝会清掉审批卡,不再依赖重载后的消息回填。口令与令牌脱敏这组核心断言保持不变,现能对真实 headless Chromium 在 Linux 上端到端跑通。
