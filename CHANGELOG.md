...
- main 侧新增的 ECMA-335 索引宽度边界测试直接钉 `metadata_enum` 的私有
  `_coded_index_size` / `_simple_index_size`，crafted 敌意根测试则按旧的单参数
  五元组 API 驱动 `_parse_metadata_root`；本分支同期把宽度帮助函数迁到公共
  `dotnet.tables` 模块、给 `_parse_metadata_root` 加了 `entry_token` 参数并把返回
  扩成十二槽。两侧在 git 文本层自动合并干净，但合并树收集即失败（ImportError）
  外加六条 TypeError——合并 main 时一并调和：宽度测试改从 `dotnet.tables` 导入，
  敌意根测试用星号解包只钉「version/streams 前四槽 + 末槽 MetadataStats」的
  敌意输入语义，不与元组中段的持续演化耦合。
