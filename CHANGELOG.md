...
- **`proxy.export_har` 不设字节上限，能把多兆字节文件写进产物根**。摘要虽已逐字段限长，但一整
  个环（至多 2000 条、URL 各上限 16 KiB）序列化出来仍是数兆字节，且在保留策略跑之前就落到产物
  根；`web.har_export` 早已做法是把最新的条目逐批丢到 HAR 不超容量上限、并回报 `truncated`，proxy
  这一侧却原样全写还少报。现在与 web 对齐：超限就从尾部成批丢弃直到装得下，回 `truncated` 与
  `size`；连空壳都超上限时报 `too_large` 且不落盘。
