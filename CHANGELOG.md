...
- **代理保留环的字节计量必须“失败朝上”钉死**：`_FlowRecorder` 把整条 flow(头 + 体)留在环里供
  `proxy.flow.get` 事后取用,这正是无人值守抓包过夜把主机内存吃光的那一环——所以每条 flow 由
  `_flow_stored_bytes` 称重,单条重于 `_MAX_STORED_BODY` 时丢掉其体(`body_omitted`)而非留存。底下的
  称重助手 `_content_len`/`_encoded_len`/`_headers_len` 才是这道内存界真正生效的地方,而既有用例只称过
  规规矩矩的 flow。内存守卫的要害规则是:*量不出*的输入必须记作*超*预算、绝不记少——一个 `str()` 会抛
  的值要记成大于上限(好让它那条 flow 被丢体)、一个头集合只累加到上限为止而不被无界遍历(敌意服务器能塞
  上千个头)、一个取不到的体长读作 0 而不是把记录器整崩。任一处记少,都会让一条本该被丢的 flow 继续驻留
  ——恰是这个环要防的泄漏。新增 6 个用例把每个助手的失败方向与它驱动的端到端丢体钉死(上限被缩小以保持
  夹具廉价):`_encoded_len` 对无法字符串化的值返回 `_MAX_STORED_BODY + 1`(而非 0),对普通串返回精确的
  UTF-8 字节数;`_content_len` 对真实体返回字节长、对缺失/空/无长度(畸形 flow 对象)的 `raw_content`
  安全归零而不抛;`_headers_len` 累加到上限即停(结果略高于上限、绝不等于完整总和),对缺失或迭代抛异常
  的头集合归零而非把异常抛穿抓包;最后端到端:一条头体积撑破单条上限的 flow 被记录时丢体——摘要仍落地
  (抓包不丢)、但 `raw` 存的是丢体哨兵、`retained_bytes` 保持 0、摘要标 `body_omitted`,而上限之内的
  小 flow 整条留存(对照证明这是上限在起作用、而非记录器把一切都丢了)。
