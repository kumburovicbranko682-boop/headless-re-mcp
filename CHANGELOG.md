...
### 修复（logcat 单行超过字符上限时整行丢弃的分支无测试守住）

- `device.logcat` 的 dump 超过 `_MAX_LOGCAT_CHARS`(200 000)时保留最新字节,但该尾片以「行中间」起始,
  于是先剪掉开头那截残行再切分:`text = text[newline + 1 :] if newline != -1 else ""`。既有的 logcat
  截断测试都用短行(200 / 2000 字符)拼出 dump,保留的 20 万字符尾片必定横跨若干换行,`newline != -1`
  这一侧总会命中并剪掉一截残行——没有一个夹具能造出「尾片里一个换行都没有」的情形,`else ""` 分支从不执行。
  当**单行本身**就比整个字符上限还长时(应用把整条堆栈、序列化 blob 或 base64 一行打出),尾片整块落在
  这一行内,`find("\n")` 返回 -1,`else ""` 把这整条残片丢弃,`lines` 回空且 `truncated=true`——诚实地表示
  「尾片是一条未终结的行,一行都不给」,而非把一条 20 万字符的残行伪装成 `lines[0]`。删掉 `else ""` 让 -1
  落到 `text[0:]`,那条巨型残片就被当成一条完整日志记录交回,正是这段截断修剪要防的误解析。新增回归:
  以真正未终结的超长尾片钉住空结果(单条巨行、以及短行后跟巨行两种),并保留「巨型首行被剪后其后完整行
  悉数存活」的对照,确认空结果专属于「尾片无换行」而非泛指截断;删掉 `else ""` 后由且仅由前两条测试捕获。
