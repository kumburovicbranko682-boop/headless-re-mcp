...
能力目录完整性新增经真实 MCP stdio 服务的端到端 Gate（`tests/integration/test_capability_catalog_integrity_gate.py`）。`capabilities.search`/`describe` 是 agent 了解本 build 能做什么、该去够哪个工具的地图,每条目把一个 capability 绑到决定其就绪的 doctor 探针(`status_probe`)与它解锁的 tools。两种悄然漂移能在无任何测试察觉下毁掉这张地图:`status_probe` 指向不存在的探针——`_probe_status` 对未知探针名返回字面量 `"missing"`,与真正缺失工具报的同一个串,于是一个拼错或改名的探针把该 capability 永久钉死在"不可用",哪怕宿主其实装了该工具,且状态串无法区分二者,故须对 doctor 的真实探针集校验;`tools` 条目指向未绑定的工具名——`capabilities.describe` 便把 agent 指向一个永远调不到的幽灵工具。两条不变量今天都通过,本 gate 的存在是为在目录相对探针或工具面漂移的那一刻立即失败:幽灵工具那半对真实 MCP `tools/list`(客户端所见的确切工具面)校验,探针那半对 `run_doctor` 的真实探针集校验,并附带 search/describe 两视图一致、id 唯一、status 恒为合法探针状态、unknown id 报 not_found、条目字段齐备等完整性断言。纯 Python、stdio 回环、任意平台。
