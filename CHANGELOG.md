...
- `device.partitions`（只读）：解析 `/proc/partitions`，列出设备完整的块存储图——内核看到的每个块设备，
  包括未挂载的分区（另一个 A/B 槽、boot、vendor、recovery 以及原始磁盘本身），这是 `device.mounts`
  （只显示已挂载文件系统）看不到的。逆向据此在拉取或推理镜像前理解存储布局与槽位方案。`/proc/partitions`
  全局可读，无需 root。每条含 `name`、`major`、`minor`、`blocks`（内核的 1024 字节块计数）与
  `size_bytes`（blocks × 1024，使大小无歧义）。通过要求 `major`/`minor`/`blocks` 为整数来跳过表头与
  任何非数字行，使错误文本不会被当成分区。列表封顶 512 并带 `has_more`，读取结果为空（活设备不可能）会
  当作错误而非空列表。
