...
### 新增（`apk.files` 列出 APK 包内全部条目）

- 新增只读工具 `apk.files`：列出 APK zip 里打包的全部条目，而不仅是 `apk.native_libs` 关注的
  `lib/*.so`——额外的 `classesN.dex`、`assets/`、`res/`、`resources.arsc`、内嵌证书/密钥，以及
  `lib/` 之外的 native 库都能看到。回 `files`，每项含 `name`，并在 zip 中央目录暴露时给 `size`
  （未压缩）与 `compressed`；取不到时按项省略，不把「未知」读成「零字节文件」。分页字段 `count`、
  `total`、`offset`、`has_more` 沿用 `apk.classes` 等约定，`total` 为收集数（上限 20000），
  `scan_capped` 表示包内条目更多。大小从中央目录读取、绝不解压，且只查当前页的名字，恶意 APK
  的海量条目或超大资源都不会把整包读进内存。跨 androguard 版本兼容：4.x 的 `infolist()` 返回中央
  目录条目字典、旧版 stdlib `ZipFile` 返回 `ZipInfo` 列表，两种形状都能取值，取不到则整体降级为不带大小。
  `apk.*` 只读线新增一个包内条目列举工具。
