...
- 新增 `js.sourcemap`：纯 stdlib 读取 JavaScript Source Map v3，**无需 Node/webcrack**。
  source map 是继 bundle 之后最有价值的 Web 逆向物证——它给出打包前的原始文件树，且常常内嵌
  原始源码；它本就是纯 JSON，但此前工具面里没有任何东西能打开它：分析者手里拿到一份 `app.js.map`
  （或顺着 `sourceMappingURL` 存下来的）却没有离线追问「它揭示了哪些原始文件、是否内嵌了源码」的
  办法，除非开浏览器 devtools。`js.sourcemap` 用 stdlib 精确回答：列出 `sources`（有界）与
  `sources_total`、`sources_content_embedded` 及 `sources_detail`（每条 source/has_content/
  content_length），给出 `names_total`、mapping 形状（`mappings_size`/`generated_lines`/
  `segment_count`）与 `ignore_list`（`x_google_ignoreList`）；flat map 与 v3 index map
  （`sections`）都支持，后者按段聚合。它是**汇总而非提取**：既不返回源码正文，也不解码 VLQ
  `mappings`（只按 ';' 分行、',' 分段做精确计数），因此读取器保持精确且廉价。非 JSON 或非
  source map 报 `invalid_params`，超 16 MiB 上限报 `too_large`，缺文件报 `not_found`。新增
  真实 MCP stdio Gate（`test_js_sourcemap_gate.py`，无需任何后端，始终执行）与
  `test_sourcemap_summary.py` 单测覆盖（含 flat/index map、错型成员不崩、路径有界、非 map 拒绝）。
