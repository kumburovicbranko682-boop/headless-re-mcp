...
- 同文件同则第二处：`_summarize_value` 对 dict 值静默只取前 4 个键，超出的键不留任何
  痕迹——按本模块自己在 `_note_if_partial` 立下的规矩（"报告没提的发现与从未记录的
  发现无法区分"），6 键值读起来像完整的 4 键值即误导。改法：截断时追加 `, …`；
  配套测试钉住 4 键无标记、6 键以 `, …` 结尾。
