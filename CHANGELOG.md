...
工作台 profile 与 persona 控制面到 LLM 线上的接线新增经真实 HTTP 的端到端 Gate（`tests/integration/test_agent_profile_persona_gate.py`）。两个控制台开关决定 agent 究竟是什么,但只有真正抵达 provider 请求本身才有意义——不是 UI、不是目录,是发给模型的字节。工作台 profile(full/pe/android/web)裁剪 agent 被**提供**哪些工具:pe 聚焦的 run 不该拿到可能误用的 apk.*/web.*,而每种 profile 都须保留 session.* 核心;裁剪按 run 实时读取,故在控制台切焦点会重塑下一个 run 而无需重启。选中的 persona **就是**系统提示——但桌面与隐身两条安全规则会追加在 persona 说的任何内容之后,故自定义 persona 无法悄悄丢掉"snapshot 窗口数为 0 时别说 GUI 已开"或加壳样本处理与哪些写入仍需审批那两句。二者此前只对辅助函数有单元证明,没有在控制台 HTTP 开关遇上线上字节处证明过。Gate 用记录每个请求的假 provider 驱动真实 `serve-web`,经公开 API 翻转 profile 与 persona,再读回模型实际收到什么:断言 web profile 的 tools 面无 apk.* 但有 web.*/js.*、pe profile 连 proxy.* 一并消失而 session.* 恒在、pe⊂web⊂full 严格子集;自定义 persona 的金丝雀在系统提示里、而它故意省略的两条安全规则片段仍被追加。无真实 LLM、仅回环、纯 Python。
