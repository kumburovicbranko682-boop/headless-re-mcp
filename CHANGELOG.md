...
Android 身份识别新增经真实 MCP stdio 服务的端到端 Gate（`tests/integration/test_android_identity_gate.py`）。Android 面不能因为没装 androguard/jadx 就整体塌成「不可用」:会话创建只用标准库从包里读廉价身份事实——哪些 ABI 带原生代码、几个 dex、是否有 v1 签名——好让 Agent 打开一个 APK、知道它是什么,再从重型工具那里得到诚实的「已打开,但无法反编译」,而不是崩溃或工具凭空消失。既有 Android gate 对进程内服务证明这点;本 gate 跨真实 MCP stdio 传输(Agent 实际驱动的那层,adapter、工具守卫、JSON 信封都夹在调用方与解析器之间)钉住同一契约:APK 被判为 `apk` 且其 stdlib 元数据精确且是算出来的而非常量——native ABI 去重并排序、dex 计数、检出 v1 签名——而一个「瘠」APK 报出每个字段的空/零/假一侧,证明字段确会移动;PE-only 工具以 `target_mismatch` 拒绝 APK 会话,而 androguard 支撑的工具在后端缺失时降级为结构化的 `capability_unavailable` 信封(绝非协议错或抛异常);目标种类按内容而非信任判定——WebAssembly 模块即便名字不对也按 magic 认出、一个名为 `.apk` 却不是可读归档的包以清晰消息失败、一个普通 zip 回退到原本的「not a PE file」错误而非更含糊的说法。纯 stdlib 夹具、stdio 回环、无后端、任意平台。
