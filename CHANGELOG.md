...
- `device.routes`（只读）：解析 `/proc/net/route`，列出设备的网络拓扑——每个网卡匹配哪个目的/掩码、
  从哪个网关出去。默认路由（目的 0.0.0.0）指出设备实际使用的网关。这是网络侦察里「路由」这一半，与
  `device.connections`（套接字）、`device.arp`（邻居）配套。`/proc/net/route` 全局可读，无需 root。
  每条含 `iface`、`destination`、`gateway`、`mask`（内核以小端存储，这里翻转还原成点分 IPv4）以及原始
  `flags` 十六进制。读取成功以内核的 Destination 表头为准，空表是诚实结果（没有配置路由）而非错误，只有
  始终读不到表头（拒绝、缺失）才算错误。列表封顶 512 并带 `has_more`。
