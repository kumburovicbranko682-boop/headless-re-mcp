...
### 新增（`device.sockstat6` 从 `/proc/net/sockstat6` 报 IPv6 套接字分配汇总）

- 新增只读工具 `device.sockstat6`，是 IPv4 套接字汇总（`device.sockstat`）的 IPv6 搭档:`TCP6 inuse`、
  `UDP6 inuse`、`RAW6`、`FRAG6`。与 IPv4 文件一样每行都是 `Label: name value ...`，统一解析为逐标签
  的「计数名→整数」映射——同样能捕获逐套接字快照会漏掉的 IPv6 套接字泄漏的聚合视图。诚实边界与
  `device.ipv6_addrs` 一致:三种结局严格区分——设备离线（adb 主机错误回包）报 `backend_error`;内核关闭
  IPv6 或文件被限制（回 "No such file" / "Permission denied" 且无标签）如实报 `available: false`，既非
  失败也非空的成功;可读文件则 `available: true` 并给出标签。无法识别的非错误输出报 `backend_error` 而不
  臆测为空。标签集合有界并在超过上限时置 `has_more`。新增 `tests/unit/test_device_sockstat6_fields.py`
  覆盖逐标签键值对解析、离线报错、IPv6 关闭的 available=false 与无法识别输出报错等分支。工具面
  265 → 266（只读 148 → 149）。
