...
- **`apk.method_info`**：按名给某个类的方法做画像——`apk.class_info` 的方法级搭档。`apk.methods` 列出
  一个类的方法带 descriptor/access 但不含连接度；`apk.xrefs`、`apk.callees` 给裸名的调用者/被调用者
  列表；本工具则对 `class_name.method_name` 的每个重载给出其 `descriptor`、`access` 以及衡量其角色的
  计数：`callers`（入向调用，被依赖程度）与 `callees`（出向调用，做事多少）。同名可有多个重载，故按
  descriptor 逐行返回。接受点名或 `Lsmali/` 形式。`matched` 在无命中（类或名不存在）时为 false 而非
  报错，探测很便宜。返回 `class_name`、`method_name`、`methods`（descriptor、access、callers、callees）、
  `count`、`total`、`offset`、`has_more`、`matched`；external 方法跳过。只读工具。
