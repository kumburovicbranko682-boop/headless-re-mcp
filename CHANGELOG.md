...
### 新增（`apk.secrets` 扫描 DEX 串池里的硬编码凭据）

- 新增 `apk.secrets`：`apk.strings` 给整池、`apk.urls` 给端点，此工具只留下匹配已知密钥形态的
  token——Google API key、AWS access key id、GitHub / Slack / Stripe token、JWT、PEM 私钥头
  （与 gitleaks/truffleHog 同一族形态）。每行给出 `category`（命中的形态）与 `value`（命中的
  token，按上限截断），按 (category, value) 去重排序。这是对字面量的模式匹配：看不到运行时拼接
  出来的 key，也可能误报只是长得像的字符串——`category` 命名的是形态，不等于确证的活密钥；且只扫
  DEX 串池，不含 resources.arsc 或原生库。返回 `secrets`、`count`、`total`、`offset`、
  `has_more`，`total` 为去重命中数，上限 5000，超出置 `scan_capped`。只读，工具面
  265→**266（149 只读 / 117 写）**。
