...
`batch.analyze` 不再把混合批次里的非 PE 样本记为失败。`open_static`（默认开启）驱动的是 PE/IDA 静态后端，对 APK 或 Web 资产本就不适用；过去这个不适用被折进 `ok=false`，于是对一个混装文件夹做无人值守分诊时，每个 APK/Web 样本都被报成"失败"——尽管其会话已创建、身份已提取。现在非 PE 样本保持 `ok=true`，条目带 `static_open=false` 与 `static_open_applicable=false`（也不再为一个注定 `target_mismatch` 的目标空起一个静态 worker）；真正的失败（文件不存在、创建失败、PE 静态打开失败）语义不变，`succeeded`/`failed` 只数真实结局。新增经真实 MCP stdio 服务的混合目标分诊 Gate（`tests/integration/test_batch_mixed_targets_gate.py`）：一个 APK、一个 Web 资产、一个不存在的路径以 `open_static=true` 同批提交，APK/Web 条目 ok 且其 session_id 被同一连接上的 `session.get` 证实是活会话（target 与身份元数据俱全），缺失路径仍是带结构化错误的真失败，计数恰为 2/1。
