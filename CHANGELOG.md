...
- 新增只读工具 `web.storage`：现代单页应用把 JWT、刷新令牌与特性开关塞在 `localStorage`/
  `sessionStorage` 里，此前只能看 `web.cookies`（HTTP cookie 罐，且尚未并入主线）而读不到这两处。
  用一段固定读取脚本（不是任意 JS 求值，仅遍历两个 Storage 对象，与 `web.dom.snapshot` 的固定
  `evaluate` 同理，仍不违背「不开放任意 JS 求值」边界）经 `page.evaluate` 取值。回 `url` 加
  `local`、`session` 两块，每块含 `available`、`items`（`key`、`value`、`value_size`，值截到 8192
  字符时给 `value_truncated`）与 `count`。诚实分级：`available=false` 表示该源拒绝读取（opaque
  origin、禁用存储），与 `available=true` 且 `items` 为空（可读但为空）区分开——否则「被拦的读」会
  被读成「这里没有令牌」；`count` 是该存储真实键数，满 500 键上限时给 `items_truncated`，截断的一页
  不会被读成整仓。键计数、单值大小双重有界，单个恶意源无法让工具吐出整仓缓存。`web.*` 只读线新增一个存储读取工具。
