...
### 新增（`apk.read_file` 读取 APK 内单个打包文件的原始字节）

- 新增只读工具 `apk.read_file`：`apk.files` 清单的取内容一侧——无需整包解包即可取出某个打包条目的
  原始字节（内嵌 config JSON、证书/密钥、`resources.arsc`、native `.so` 的文件头等）。回 `name`、
  `size`（条目的完整未压缩长度）、所读窗口（`offset` 加至多 `max_bytes`，上限 262144）的 base64
  `data`、`returned`（窗口字节数）、`truncated`（窗口之后仍有字节时为真）、`encoding`，以及对整条目求得的
  `sha256`（无需分页读完即可指纹化一份内嵌证书/密钥）。取字节前先按中央目录检查未压缩大小，超过 16 MiB
  的条目按 `too_large` 拒绝而不是拉进内存（`get_file` 会解压整条目）；缺失条目按 `not_found` 而非空读。
  `apk.*` 只读线补上「取单个打包文件内容」。
