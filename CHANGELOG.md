...
工具调用遥测面新增经真实 MCP 服务器的端到端 Gate（`tests/integration/test_tool_telemetry_gate.py`）。无人值守部署靠 `meta.metrics` 与 Prometheus `/metrics` 读自身健康——每工具的调用数、失败数、延迟分位;这些数只有当工具经传输层插桩(`telemetry.instrument`)分发时才产生,而该插桩只在 MCP 服务器的 `register_tool` 应用,直连 `service.*` 与 agent `catalog.invoke` 路径都刻意不记录,所以只在环上做断言的单测证明不了分发接线。用例拉起真实 `python -m headless_re_mcp serve`(制品根按测试隔离,遥测环从空开始,只计本测发起的调用),经 stdio 驱动确定性工具集:一次建会话、一次 knowledge 查询、两次 artifacts 列举、一次故意失败的 `artifacts.describe`;随后读两次 `meta.metrics`(第二次捕获第一次的记录),断言计数是精确值而非"至少":每个工具的 `calls_total` 与调用次数一致、唯有失败工具 `failures_total==1`、进程总计等于各工具计数之和、延迟分位是有序浮点、`recent` 环带出失败的 error_code 与会话级调用的 session_id;并钉住 `limit` 守护——越界 limit 由工具输入 schema 在分发前拒(协议级 `isError`),`limit=0` 合法且表示"只要总计不要 recent 样本"。只依赖检出与已装的包,恒跑不 skip。
