...
Agent 崩溃恢复新增经真实进程边界的端到端 Gate（`tests/integration/test_agent_crash_recovery_gate.py`）。无人值守意味着控制台终会在 run 正从 provider 拉流时毫无预警地死掉——OOM 击杀、宿主重启、监督者放弃。此时两件事必须成立,否则部署就需要人:死掉的 run 必须被**归档**而非永远留在 streaming(操作员读记录看到的是 `interrupted: service_restarted`,不是一条谎称还在工作的 run);被杀的是 run 而非目标,mission 必须回队列由下一个进程接着跑完。这两条性质都住在 `recover_after_restart` 里,只有接管数据库的进程才调用它——此前无跨进程证明。Gate 用首请求悬挂的假 provider 把 run 钉在流中,对控制台发 SIGKILL -9(无信号处理、无清理、无落盘机会),再在同一状态目录上启动第二个控制台:断言死 run 状态与错误如实、其事件轨迹里留有 status=interrupted 的 run.failed、幸存进程是台完整能用的控制台(同线程再起新 run 正常完成);mission 用例另断言被杀 run 有案可查、完成出自同预算内的另一个后续 run、runs_used 计数诚实。无真实 LLM、仅回环、纯 Python、POSIX。
