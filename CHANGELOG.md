...
工作区 profile 的工具面过滤新增经真实 MCP stdio 服务的端到端 Gate（`tests/integration/test_workspace_profile_surface_gate.py`）。profile 是操作者告诉服务「这套部署是干哪条线的」的方式，其承诺很具体：它收缩**客户端(LLM)** 经 `tools/list` 实际看到的工具面——Android 部署不该把 PE 脱壳器或纯 Web 的 DOM 工具亮给模型，PE 部署也不该亮出 Android 设备桥。单元测试能查前缀表，但只有跨真实传输的 gate 能证明**送出去的** `tools/list` 确实按 profile 缩小。本 gate 为每个 profile 各起一个 stdio 服务并比对其 `tools/list`：`full` 不隐藏任何工具、是每个 profile 的严格超集；每个非 full profile **恰好**移除其隐藏前缀下的工具、不多不少（`pe` 隐藏 Android+Web+共享 proxy 面，`android` 只隐藏 Web 静态面，`web` 只隐藏 Android 面），故 served 集恰等于 full 减去这些前缀、trimming 从不擦到核心工具；抓包 proxy.* 由 Android 与 Web 共享（web 会话抓浏览器流量，android 会话经 `proxy.ca.install_android` 推设备 CA），所以它在 android 与 web 都可见、仅 pe 隐藏——这是一道回归守卫：proxy 曾被归到 web 前缀下，于是整个 proxy 面（连带 android 专属的 `proxy.ca.install_android`）被从 android 方向错误地整体隐藏；session/static/artifacts/report/capabilities/meta 等核心工具在四个 profile 里都存活。隐藏前缀在测试里作为独立规格写死、不从被测代码 import，故前缀表一旦漂移 gate 立刻失败。纯 stdio 回环、无后端、任意平台。
