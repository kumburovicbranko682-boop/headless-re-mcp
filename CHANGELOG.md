...
- `js.strings`：JS 版的 `strings`,从 JavaScript 源码提取字符串字面量（硬编码的 URL、API 端点、路径、
  错误信息、内嵌密钥），纯 Python、**不需要 webcrack 或 Node**（`js.deobfuscate/beautify/unpack_bundle`
  都要 webcrack+Node）。一次带注释感知的扫描走遍 `'`/`"`/反引号 字面量,跳过 `//` 与 `/* */` 注释（引号型
  误报的最大来源），并解码反斜杠转义——被混淆成 `\x68\x74\x74\x70` 或 `\u002f` 的会还原成 `http` / 斜杠。
  它不完整词法分析 JS：不追踪正则字面量,故除法/正则歧义偶尔会误读一个（稳健的尽力扫描相对脆弱解析器的
...
- `js.capabilities`：JS 静态四件套的行为侧收口——`js.strings` 答"它说什么"、`js.endpoints` 答"它连谁"、
  `js.imports` 答"它依赖什么"，本工具一次调用答"它能做什么"：对固定的安全敏感 API 表计数并按类别分组——
  code_execution（`eval`、`Function`、`importScripts`、字符串实参的 `setTimeout`/`setInterval`）、network
  （`fetch`、`WebSocket`、`XMLHttpRequest`、`EventSource`）、storage（`localStorage`、`sessionStorage`、
  `indexedDB`、`.cookie`）、dom_injection（`.innerHTML`、`.outerHTML`、`.insertAdjacentHTML`）、encoding
  （`atob`/`btoa`）、messaging（`.postMessage`）与 wasm（`WebAssembly`），纯 Python、**不需要 webcrack 或
  Node**。复用 `js.imports` 的词法器，故字符串或注释里的名字不计数，且每个名字只在使其有意义的句法形态下
  才计：调用类须为全局调用、全局类须为非属性标识符（`x.eval` 不算 `eval`）、属性类须跟在点号后、定时器仅在
  首实参为字符串字面量的 eval 形态下计。这是对固定表的出现计数、不是恶意判定，动态到达的名字
...
- `wasm.functions`：node-free WASM 读取器的收官之作，把 type（1）、import（2）、function（3）三段
  ...
  拒绝，超过 16 MiB 按 `too_large` 拒绝。
- `wasm.strings`：WASM 版的 `strings`，从 data 段（模块初始化内存所在——字符串字面量、URL、路径、
  格式串、错误信息）提取可打印串，纯 Python、**不需要 wabt**。直接扫 data 段原始字节里的可打印 ASCII
  段（0x20..0x7e），而非解析各数据段的偏移表达式（其 LEB 立即数可能含 0x0B end 字节，naive 跳过会错位）——
  代价是负载间的少量结构字节可能粘到串两端。短于 `min_length`（默认 4）的段丢弃、长于 1024 字符的裁剪；
  结果去重并按首次出现顺序排列（把内存里相邻的串聚在一起）。返回 `has_data_section`（模块无 data 段时
...
- `wasm.data`：给出 data 段的加载映射——每个数据段落在线性内存的哪里，纯 Python、**不需要 wabt**，
  ...
  按 `too_large` 拒绝。
- `wasm.memory`：列出模块的线性内存声明——内存足迹，纯 Python、**不需要 wabt**，且比 `wasm.sections`
  （只报有无 memory 段）说得更多。把 import 段与 memory 段按内存索引空间合成一张表：每行是 `index`、
  `kind`（import/local）、`min`/`max`（以 64 KiB 页为单位的大小上下界，模块未设上界时 `max` 为
  null，即内存可无界增长）、`shared`（true 说明是 threads/atomics 共享内存——模块用了线程的强信号）与
  `index_type`（memory64 内存为 i64，否则 i32）。导入内存按规范排在前面并带 `module`/`name`，
...
- `wasm.callers`：`wasm.calls` 的逆向——给定目标函数索引，列出所有直接调用它的函数（反汇编器里的
  “xrefs to this function”），纯 Python、**不需要 wabt**。回答排查的第一个问题：谁能到达这个可疑
  导入或例程（`target` 与各 caller 索引可与 `wasm.functions` 联查名字）。走查每个函数体、在服务端完成
  过滤，无需把整张调用图翻页到客户端再筛。每行是 `index`（caller 的模块级函数索引）与 `call_sites`
  （该函数体内命中目标的 call 指令条数，调 3 次即计 3），并带 `decoded`——caller 函数体用到走查表外
  操作码时为 false，其计数可能偏低。间接调用天然不可见（`call_indirect` 不指名被调者），其候选目标由
  `wasm.elements` 枚举。返回 `target`（回显）、`has_code_section`（模块无 code 段时为 false，此时
  `callers` 空、`total` 0，而非报错）、`imported_count`、`undecoded_bodies`（无法完整走查的函数体数，
...
- `wasm.producers`：解码 "producers" 自定义段——模块的构建工具链指纹，纯 Python、**不需要 wabt**，
  ...
  非 WebAssembly 文件按 `invalid_params` 拒绝，超过 16 MiB 按 `too_large` 拒绝。
- `wasm.features`：解码 "target_features" 自定义段——模块被编译为使用的 WASM 特性/提案,纯 Python、
  **不需要 wabt**,是 `wasm.producers`(溯源)的能力侧伴侣。工具链(LLVM / wasm-ld)在此记录模块用到
  哪些 MVP 之外的提案:simd128、atomics(线程与共享内存)、exception-handling、bulk-memory、
  reference-types、tail-call、sign-ext、multivalue 等,由此可判断模块需要的运行时与需预期的现代指令集
  规模(`wasm.sections` 只报该段存在)。每行是 `name`(特性名)与 `prefix`(单字节标记:`+` 使用 /
  `-` 禁用 / `=` 精确要求,未知字节以十六进制呈现);wasm-ld 对模块实际使用的每个特性都写 `+`,故实践中
  各行即"已用特性集"。返回 `has_target_features_section`(模块无该段时为 false,此时 `features` 空、
