...
### 修复（设置面板探测模型列表会静默换掉已配置的模型）

- `webui` 设置面板在打开时（密钥已配置则自动探测）和点「拉取模型列表」时，若探测到的列表里
  没有当前配置的模型，会直接 `setModel(list[0])` 把表单里的模型换成列表第一项。而各家网关的
  `/models` 列表经常不全——别名、带日期的快照、微调模型常常缺席。于是最常见的事故路径是：
  用户通过 API 或导入配置好了模型，打开设置面板只是想换个密钥，模型下拉框在自动探测后已经
  静默指向了别的模型，点「保存模型」把原配置毁掉，全程无任何提示。现在两处都不再自动切换；
  下拉框在配置模型不在探测列表时把它并入选项首位，既能正常显示当前值，也能随时改选探测到
  的模型。
- 新增三条组件直测：自动探测返回不含配置模型的列表时，下拉框仍选中配置模型且它在选项里；
  「拉取模型列表」按钮同样不换模型；配置模型在列表内时选项不重复。修复前前两条用例都停在
  下拉框值变成 `model-a`。前端 66 条测试与 `tsc --noEmit` 均通过，提交的 SPA 产物已重建。
