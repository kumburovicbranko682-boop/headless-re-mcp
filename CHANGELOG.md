...
- **`apk.field_search`**：对字段名做全 App 大小写不敏感的子串搜索，是 `apk.method_search` /
  `apk.class_search` / `apk.string_search` 的字段版，用来在所有类里定位配置开关、baseUrl 持有者或
  硬编码的密钥字段（`API_KEY`、`sBaseUrl`）——这是 `apk.field_xrefs`（需精确名）做不到的。按字段简单名
  子串、大小写不敏感匹配。每行 `class_name`（声明类）、`name`、`descriptor`（字段类型）、`access`
  （标志串），常量与可变状态由此可分。返回 `query`、`fields`、`count`、`total`、`offset`、`has_more`；
  `total` 为收集数、封顶 2000、超出置 `scan_capped`；结果按 (class_name, name, descriptor) 去重排序。
  只读工具。
