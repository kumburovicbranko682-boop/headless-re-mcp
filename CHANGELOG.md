...
### 新增（设备：Settings provider 读取）

- `device.settings`（只读）：读取 Settings provider 的 global / secure / system 三个命名空间——
  这与 `device.properties`（getprop 构建属性）是不同的数据源，装的是运行期设备配置：`http_proxy`、
  `adb_enabled`、`development_settings_enabled`、`install_non_market_apps` 等，正是逆向关心的安全
  态势。每个 `settings list <ns>` 打印 `key=value`，三个命名空间各成一张 map。设备拒绝的命名空间
  （权限不足，或旧系统没有该命名空间）进 `unavailable` 而不是被悄悄丢掉，只有三个命名空间全失败
  才算整体失败。回带 `settings`（命名空间→键值表）、`count`、`has_more` 与 `unavailable`（有则带），
  条目总数封顶，装满上限的一次读取不会被当成全部配置。
