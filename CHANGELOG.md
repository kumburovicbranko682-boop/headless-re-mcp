...
### 新增（`apk.string_search` 按片段查 DEX 串）

- 新增 `apk.string_search`：与 `apk.class_search`、`apk.method_search` 凑齐搜索三件套。
  `apk.strings` 给整池，找一个词（"password"、"http"、"-----BEGIN"、某域名）就得把整池翻一遍再
  grep。此工具扫描同一池，只保留包含 query 的常量（大小写不敏感子串，不是正则），去重排序。区别于
  `apk.secrets`（固定的凭据形态）与 `apk.urls`（端点）。只扫 DEX 串池，不含 resources.arsc 或
  原生库。返回 `strings`、`query`、`count`、`total`、`offset`、`has_more`；`total` 为命中数，
  上限 5000，超出置 `scan_capped`。只读，工具面 265→**266（149 只读 / 117 写）**。
