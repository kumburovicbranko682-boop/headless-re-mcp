...
- `device.arp`（只读）：解析 `/proc/net/arp`，列出设备对本地网络的认知——把哪些 IP 解析到了哪些 MAC、
  在哪个网卡上、条目是否完整。与 `device.connections`（套接字）配对，这是网络侦察里「同网段还有谁」的
  邻居半边。`/proc/net/arp` 全局可读，无需 root。每条含 `ip`、`mac`、原始 `flags` 十六进制、由 ATF_COM
  位解码的 `complete`（未完成条目的 MAC 为占位 `00:00:00:00:00:00`），以及 `device`（网卡）。空表是
  诚实的结果（没有已解析邻居），不算错误；只有始终读不到内核表头（权限拒绝、文件不存在）才算错误。
  列表封顶 1024 并带 `has_more`。
