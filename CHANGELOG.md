...
### 新增（`r2.relocs` 列出 radare2 的重定位表）

- 新增只读工具 `r2.relocs`：白名单放行 `irj`，服务层跑 `irj`，经既有 `enrich_r2_payload`
  统一映射。这是重定位表——加载器把地址回填到哪里的记录，因此一个导入函数的 GOT 槽会显示为
  「该导入被接到某处」，这是 `r2.imports`（导入表）与 `r2.symbols`（符号表）都给不出的视角，
  对 native `.so` 逆向理解动态链接很有用。回 `items`，每项含 `name`（该条重定位绑定的符号，
  匿名重定位为空）、`type`（重定位类型）、`vaddr` 与 `address`（va/rva/module），外加 `count`；
  没有整数地址字段。满 4096 上限时给出 `items_truncated`、`items_total`、`items_limit`，不会把
  「截到上限」读成「表到此为止」；没有 `relocs`、`truncated` 或 `has_more` 字段。`r2.*` 只读线新增一个重定位表工具。
