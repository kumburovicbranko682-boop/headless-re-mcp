...
### 新增（`device.diskstats` 从 `/proc/diskstats` 报每块设备的 I/O 活动）

- 新增只读工具 `device.diskstats`，是 `device.partitions`（块存储布局）的活动搭档:布局看得到设备，
  却看不到它被用了多少。每个设备给出已完成的读/写次数、各自耗时、在途 I/O，以及最关键的读/写总
  字节数。`/proc/diskstats` 不论设备物理块大小，一律以固定 512 字节扇区计量传输，故 `read_bytes` /
  `write_bytes` = `sectors * 512`。诚实边界:只有列数达到内核完整格式、且 major/minor 与 11 个统计列
  都能解析为整数的行才被接受，故表头与 adb 主机错误文本都不贡献任何条目;活着的设备总有块设备，因此
  解析到零个即读取失败（文件缺失、权限拒绝、设备离线），报 `backend_error` 而非空结果。列表有界并在
  超过上限时置 `has_more`。新增 `tests/unit/test_device_diskstats_fields.py` 覆盖列解析与 512 字节
  换算、表头跳过、分页截断与零设备报错等分支。工具面 265 → 266（只读 148 → 149）。
