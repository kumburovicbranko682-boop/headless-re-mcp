...
`report.generate` 对「一条 finding 的字段被截断」现在与「finding 列表被截断」一样诚实。findings 表的取值单元格只显示前若干字段以保持一行，此前当某条 finding 携带的短字段多于上限时会悄悄漏掉:基于长度的 `…` 只在拼接文本较长时才出现，于是一条含六个单字符字段的取值只渲染前四个、无声丢弃两个——而报告从不展示的字段，与从未被记录的字段无从区分（正是 `_note_if_partial` 所守护的原则）。修复让取值单元格在字段多于上限时追加 `… (+N more)`，说明有几个没放下，而不是把前几个当成整个取值;恰好等于上限时不追加任何提示。
