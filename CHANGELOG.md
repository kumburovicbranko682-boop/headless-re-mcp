...
### 修复（抓包后端 mitmproxy 12.x 启动自杀）

- 内嵌抓包代理在 mitmproxy 12.x 上不再在启动时自杀。`DumpMaster` 默认装的是给 `mitmdump`
  CLI 用的插件:`errorcheck` 会在启动期任何一条错误日志上 `sys.exit(1)`,`keepserving` 的
  `running` 钩子读 `options.rfile`(readfile 插件注册的选项)并记一条错误,恰好被 `errorcheck`
  升级成致命;`readfile`/`readfilestdin` 还会在启动时从磁盘/stdin 读 flow。启动前先剥掉这四个
  插件(`_strip_embedded_hazards`),内嵌代理才不会被 CLI 专属的启动检查掀翻。(端口泄漏另由
  上文「mitmproxy 12 停止代理后监听端口不再泄漏」的 `Servers.update([])` 收尾覆盖。)
