...
- `device.cmdline`（只读）：解析 `/proc/cmdline`，即权威的内核启动命令行，也是设备启动态势的源头：
  `androidboot.verifiedbootstate`（green/yellow/orange 指示锁定/解锁）、`androidboot.veritymode` 与
  硬件标识等——这是与 `device.security`（SELinux/su）互补的「启动」维度，直接取自内核而非镜像到的
  getprop。`/proc/cmdline` 全局可读，无需 root。命令行按空白拆分：含 `=` 的 token 归入 `params`
  （重复键后者胜，与内核取最后值一致），无 `=` 的裸 token 归入 `flags`；`raw` 保留原始整行以免解析丢
  信息；两者均有上限，超长整行以 `truncated` 标记。读取结果为空（活设备不可能）会当作错误而非空 params。
