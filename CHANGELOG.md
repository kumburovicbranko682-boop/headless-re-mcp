...
抓包线的离线诚实契约新增经真实 MCP stdio 服务的端到端 Gate（`tests/integration/test_proxy_offline_honesty_gate.py`）。既有的 live 抓包 gate（`test_proxy_lifecycle_gate`）直调 `ProxyBackend` 证正向路径——start 即在监听、stop 即释放端口——但它在没装 mitmproxy 时**整体 skip**，于是留下一台无人值守的裸机每次不带 mitmproxy 启动时真正要兑现的契约无人钉：没有后端、也从没起过代理时，这条抓包面到底说什么？「什么都没抓到」和「没有后端可起」是两件事，只有后者才该让操作者去装 mitmproxy。本 gate 把这道分界钉在真实 stdio 传输上，且在 live gate 会 skip 的裸机上**照跑**：描述代理「不存在」的查询面根本不需要后端——`proxy.status` 只答 `running:false` 别无它字段（不是缺能力错，也不是一份可能被误读成「代理跑过、零流量」的空捕获），`proxy.stop` 对从没起过代理的会话是幂等诚实的空操作（`stopped:false` 带说明，既非报错也非谎称拆除了什么）；会交回**捕获数据**的读则在没有代理时以 `invalid_state` 拒绝——`proxy.flows` 与 `proxy.export_har` 绝不伪造空列表或空 HAR，「没有代理」不等于「一份零流量的捕获」；唯有 `proxy.start` 触碰后端，故只有它在 mitmproxy 缺失时降级为 `capability_unavailable`，把可修复的那一件事讲清楚，也只在需要它的那一次调用上讲；且代理是会话级运行态而非目标类型工具——查询面在 PE/APK/web 会话上表现完全一致、从不答 `target_mismatch`（与 PE-only 脱壳器「先拒非 PE 目标」恰成对照）。纯 stdlib 夹具、stdio 回环、无真后端、任意平台。
