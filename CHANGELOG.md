...
- **`apk.class_hierarchy`**：沿父类链把某个类从自身走到根。`apk.class_info` 只给出直接父类，
  本工具跟着走完整条链：`MainActivity -> AppCompatActivity -> ... -> Landroid/app/Activity;`。
  walk 会穿过 APK 内定义的类（含随包的 support-library 基类），在第一个「不在本包里」的父类
  （框架类如 `Landroid/app/Activity;` 或 `Ljava/lang/Object;`）处停下，该类即 `root`。起始类必须
  在 APK 内定义（只有它自己的 class def 才知道其父类），故未知或纯 external 名回 `not_found`；
  接受点名或 `Lsmali/` 形式。返回 `class_name`、`ancestors`（直接父类在前、根在后）、`depth`、
  `root`、`root_in_apk`（框架/Object 停止点为 false）与 `truncated`（walk 有环保护并封顶 128 层）。
  只读工具。
