...
任务韧性新增经真实 HTTP 的端到端 Gate（`tests/integration/test_mission_resilience_gate.py`）。承载任务机制的两个调度器主张此前无端到端证明:其一,花光工具轮次边界的 run 是用掉了预算而不是坏掉——假 provider 在"Run 1"期间每轮都提议廉价只读工具(`meta.metrics`),真实跑满 orchestrator 的 24 轮边界(收尾轮不给工具、拿纯文本总结)后 run 以 `maximum tool rounds exceeded` 记 `failed`,而任务不死:重回队列、第二个 run 以 `MISSION_COMPLETE` 开头收官,任务 `completed`、`runs_used == 2`——任务机制存在的意义恰是这种单 run 装不下的目标;其二,任务级取消必须穿透到在途 run——对着停滞 provider 的任务经 `POST /api/agent/missions/{id}/cancel` 取消后,调度器起的那个 run 真被停住(终态 `cancelled`、事件 `run.cancelled`),任务同落 `cancelled`,调度器本身存活继续排程,未知任务 id 如实 404。无真实 LLM、仅回环网络、纯 Python。
