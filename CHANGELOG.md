...
ELF 现在是一等目标类型，不再被当作"打不开的 PE"。`classify_target` 过去把所有非 PE/非 APK/非 Web 的文件都归为 PE，`session.create` 随即调用 `detect_pe_architecture` 抛「not a PE file」——于是一个 Linux ELF（radare2 与 Ghidra 这两条线在 Linux 上最自然的输入）连会话都开不了，跨平台工具根本没东西可跑。新增 `TargetKind.ELF`：`classify_target` 认 `\x7fELF` 魔数（`.so` 等无 APK/Web 后缀的文件也据此正确归类），`create_session` 走 `detect_elf_architecture`——它只把 EM_386/EM_X86_64 映射为 x86/x64，AArch64 等本工具不建模的机型返回 `None` 而非抛错（ELF 照样开，只是没有 PE 式机型可报），并按 ELF 头的字节序读 `e_machine`。路由随之正确：PE 专用后端（`static.open` 等，经 `require_pe`）对 ELF 报 `target_mismatch`，而 radare2/Ghidra（经 `require_binary`）能拿到 ELF——缺 CLI 时是 `capability_unavailable`，绝不会是 `target_mismatch`。新增经真实 MCP stdio 服务的 ELF 目标 Gate（`tests/integration/test_elf_target_gate.py`）：ELF 经 `session.create` 得 `target=elf` 与架构、`session.get` 回出同样事实、`static.open` 报 `target_mismatch`、`r2.open` 走到可用性检查。（Mach-O 目前仍走 PE 回退，可作为后续同法扩展。）
