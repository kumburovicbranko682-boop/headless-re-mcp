...
- **`apk.packages`**：把内部类按 Java 包归组并计数，给出 `apk.classes`（一份扁平类名列表）没有
  的鸟瞰——「这个 App 里都有什么」：第一方代码与随包的 SDK、广告/统计库、框架 shim 各自成一个包、
  各带 `class_count`，一眼可分。包名取类的点名去掉最后一段（`Lcom/example/Foo;` -> `com.example`；
  嵌套类 `Foo$Bar` 归其外层包），无名默认包记为 `""`，external 类跳过。返回 `packages`
  （`package`、`class_count`）、`count`、`total`（不同包数）、`total_classes`（计入的类数，封顶
  10000、超出置 `scan_capped`）、`offset`、`has_more`；按 `class_count` 降序、同数再按包名排序，
  最重的包在前。只读工具。
