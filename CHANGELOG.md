...
- **`apk.export_sources` 的 `java_files` 先截断再排序，返回的是伪装成有序的任意子集**。
  `_capped_java_listing` 只保留 rglob 走到的前 2000 个文件才 `sort()`，于是一个反编译出两千
  多个类的 APK，被丢下的是文件系统遍历顺序里排在 2000 名之后的那些、而不是字母序末尾的那些
  ——回包看着 a→z，缺口却在中间：按序在返回区间内找某个类的调用方会漏掉一个被悄悄剪掉的类，
  `has_more` 也读成"最后一名之后还有"，其实断层在中间。现在先收全（仍以 `_MAX_COUNTED_FILES`
  兜底，不让病态大树无界解析）、再 `sort()`、再取 `cap`，返回的这一页就是真正的字母序前缀。
