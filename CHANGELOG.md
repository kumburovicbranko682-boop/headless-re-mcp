...
### 新增（`apk.urls` 从 DEX 字符串池提取 URL 端点）

- 新增 `apk.urls`：从 DEX 字符串池里提取 http/https/ws/ftp URL。摸清一个 App 与哪些端点通信是
  首轮 triage 的常规动作，而 `apk.strings` 只回原始字符串池，调用方得把整池翻页再自己 grep。
  本工具扫同一字符串池、只留 URL 样式的 token（内嵌在更长字面量里的 URL 也能单独取出，如
  `base=https://api.example/v2 fallback` 提取出裸 URL），去重、排序、分页稳定；`total` 为收集到的
  去重 URL 数（上限 5000，超出置 `scan_capped`），`offset`/`limit`/`has_more` 与其余分页读取器一致。
  仅覆盖 DEX 字符串池，不触及 `resources.arsc` 与原生库，运行时由片段拼出的端点也不会出现。
