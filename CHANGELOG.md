...
### 新增（`r2.sections` 列出二进制的节/段布局）

- 新增只读工具 `r2.sections`：跑 `iSj`，把 ELF/.so（或 PE）的节/段布局列出来——正是把一个裸
  地址对上「落在哪个节」的那张地图，于是指向 `.text` 的指针读成代码、指向 `.data` 的读成数据。
  这是从 APK 里掏出 `apk.native_libs` 列的 `.so` 后，拿 r2 看它内部结构的基础一步，此前
  `r2.*` 有 info/functions/imports/exports/strings/disasm/xrefs 却独缺节表。回 `items`，每项含
  `name`、`size`、`vsize`、`paddr`、`vaddr`、`perm` 与 `address`（va/rva/module），外加 `count`；
  列表满 4096 上限时带 `items_truncated`、`items_total`、`items_limit`，不会把截断读成读完。
  命令经既有白名单校验（新增 `iSj`），映射复用通用的 `enrich_r2_payload`，与其它 `r2.*` 列表
  工具同一范式。radare2 静态线新增一个只读节表工具。
