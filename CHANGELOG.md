...
timeline 与 audit 两份取证日志的分工新增经真实 MCP stdio 服务的端到端 Gate（`tests/integration/test_timeline_audit_forensics_gate.py`）。无人值守跑完会留下两份日志，它们**故意**回答不同的问题：`timeline.list` 是单个会话的「留痕史」——它开了、关了、写了什么、驱动了哪个 UI——只读操作不留痕，所以一个分析了一小时却什么都没改的会话，timeline 里只有它的 open 与 close；`audit.list` 是跨会话的操作账本——哪些会话跑过、带什么参数、如何收尾。把两者混用（去 timeline 找一次读、去 audit 找一次静态改写、或读错字段名）会悄悄得出错误的取证结论。本 gate 在裸机无后端上把这道分界钉在 stdio 传输上：只读不留痕——一个会话在 open 之后跑纯查询（`capabilities.search`、`session.list`）、记 `knowledge.record`、生成 `report.generate`，其 timeline 仍只有 `session.created`、其 audit 仍只有 `session.create`，那些读和这些非静态写都不出现；open 与 close 在两份日志里都留痕，且 timeline 按时间先后（created 在 closed 之前）；audit 记下参数与结果（`params_summary`、真值 `ok`、`result_summary`），这正是它能回答「哪些会话跑过、如何收尾」的原因，而 timeline 不记这些、它回答「改了什么」；字段名也是契约——timeline 只给 `events`、绝不给 `entries`，audit 只给 `entries`、绝不给 `events`；对一个不存在的会话，两份日志按设计给出不同答复——`timeline.list` 是某个会话自己的历史，故一个从未存在的 id 是 `session_not_found`，而不是一份会被误读成「存在过、没改动」的空事件表；`audit.list` 是全局日志上的过滤器，故未知 id 只是匹配不到、返回空页而非报错，且不带 id 的全局 audit 里仍有该会话的记录。纯 stdlib PE 夹具、stdio 回环、无后端、任意平台。
