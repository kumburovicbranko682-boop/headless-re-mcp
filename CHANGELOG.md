...
Agent 取消与线程数据面新增经真实 HTTP 的端到端 Gate（`tests/integration/test_agent_cancel_http_gate.py`）。停不下来的 run 在 provider 卡死那一刻就是负债:操作者唯一的杠杆是 `POST /api/agent/runs/{id}/cancel`,它必须在 provider 连接仍开着、流着空气时起效。第一条用例把真实 `serve-web` 指向一个发出一个文本增量后故意挂住的假 OpenAI 服务器,确认请求确实在途后经 HTTP 取消,钉住:终态 `cancelled`、事件史含 `run.cancelled`、全程没有任何 `tool.*` 事件、`/api/sessions` 零副作用;对终态 run 重复取消无害(202)、未知 run 如实 404。第二条钉控制台 UI 依赖的线程数据面:创建/列表/读回全链路,空消息 400、超过 1 MiB 的消息 413,删除后线程真被遗忘(读 404、重删 404、向已删线程投递消息 404 而非静默重建)。无真实 LLM、仅回环、纯 Python。
