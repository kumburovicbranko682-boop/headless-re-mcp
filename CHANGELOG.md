...
Frida 线的离线诚实契约新增经真实 MCP stdio 服务的端到端 Gate（`tests/integration/test_frida_offline_honesty_gate.py`）。live Frida gate（`test_m11_frida_live_gate`）要一台真机和一个 frida-server，裸机上整体 skip，于是留下一台无人值守的 Android 部署在不带 frida、也没接设备时真正要兑现的契约：这条 Frida 面到底说什么，它有没有告诉调用方**该做的下一件对的事**？「去装个后端」与「你还没到这一步需要的状态」是两个不同的问题、有不同的修法，把两者混为一谈的工具，会把操作者打发去装一个本就帮不上忙的软件。本 gate 在 live gate 会 skip 的裸机上照跑，把 Frida 保持这道分界钉在真实 stdio 传输上，且在 PE 与 APK 会话上表现一致（层级由工具与会话状态决定，而非目标种类）：真正需要后端的操作以 `capability_unavailable` 失败并点名可修复的依赖——`frida.devices` 与 `frida.device.connect` 点名 frida Python 模块，而 `frida.server.ensure` 点名 **adbutils**（它自己的依赖）而非 frida，好让操作者去装那个真正解锁这次调用的东西；设备级读（`frida.applications`/`spawn`/`java.classes`/`java.methods`）以 `invalid_state` 失败并让调用方先 `frida.device.connect`——这是一个工作流步骤、不是一次安装，被这样告知的调用方不该跑去装 frida；调试对象探针（`frida.attach`/`modules`/`exports`/`memory.read`/`hook.template`）同样以 `invalid_state` 失败：它们读会话的活目标，没有目标时这是状态错误、不是缺安装。纯 stdlib 夹具、stdio 回环、无 frida/adbutils、任意平台。
