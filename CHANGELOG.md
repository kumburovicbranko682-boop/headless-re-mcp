...
### 新增（Android 静态：uses-library 依赖）

- `apk.uses_libraries`（只读）：列出 manifest 里的 `<uses-library>` / `<uses-native-library>`
  声明——应用运行时从设备加载的共享框架库与平台原生库，与 `apk.native_libs`（打进 APK 的
  `.so` 文件）是两回事，二者是不同的事实。每条带 `type`（java/native）与 `required`：manifest
  省略 `android:required` 时按 Android 规范默认 true，避免把可选依赖读成硬依赖。封顶 256 条并回带
  `has_more`，装满上限的列表不会被当成全部依赖。属性按 androguard 自身的命名空间解析
  （`{android}name` 回退裸 `name`）读取，与其访问器一致。
