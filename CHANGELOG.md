...
### 测试（web 溢出助手 `_spill_bytes` 的容量与路径闸门、`_spill_text` 预览的截字节边界）

- `_spill_bytes` 是 `_spill_text` 的二进制孪生:`web.network.get` 只在 base64 分支经它落盘,而夹具
  始终喂一小段未超限的 blob、文件名又是 `uuid4`,因此它的两道闸门从未真正触发过——超容量拒绝
  与文件名收敛都是「helper 有单测、真实调用路径没有」的死区。新增直接单测:超过容量上限的原始
  字节以 `too_large`(带真实字节 `size`/`cap`)拒绝,且在**建目录之前**就拒绝,超容量二进制体碰不到
  磁盘;越界文件名(`../../escaped.bin`)以 `invalid_params` 拒绝、目录内外都不落盘;未超限的正常
  路径把原始字节逐字节写入产物目录(不 base64、不重编码)。
- `_spill_text` 的内联预览是**字节切片**,容量上限可能落在多字节字符中间。既有预览测试都切在干净的
  多字节边界上,`errors="ignore"` 与 `errors="replace"` 解码结果一致——错误处理器的选择因此是惰性的。
  新增回归把 `ééé`(六字节)切在第五字节:悬挂的半个字符必须被丢弃、不得冒出 U+FFFD,落盘制品仍
  完整保留全部六字节。
- 变异验证:删掉 `_spill_bytes` 的原始字节容量判定、放松其文件名闸门、把预览的 `errors="ignore"`
  改成 `"replace"`,均分别只被对应新测试逮住;源树保持零改动(仅新增测试与本条目)。
