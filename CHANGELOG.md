...
MCP stdio 协议层新增无后端端到端 Gate（`tests/integration/test_mcp_stdio_core_gate.py`）。此前两个协议 Gate 都被重后端绑死——`test_mcp_static_idalib` 要 IDA、`test_mcp_dynamic_xdbg` 要 Windows + x64dbg——而适配器自身做的事（拉起 `python -m headless_re_mcp serve`、initialize 握手、工具列举、参数解码、把每个 `Result` 打进结构化信封）全是纯 Python,却在没有这些后端的机器上从未整链跑过;信封形状或工具绑定的回归能穿过全部单元测试,只在真实客户端里暴露。两条用例用官方 `mcp` 客户端对真实起的服务器进程说真协议（制品根经 `HEADLESS_RE_ARTIFACT_ROOT` 按测试隔离）:一条走完整分析往返——对提交的 PE 夹具建会话、`knowledge.record`/`query` 往返、`report.generate` 产出报告、`artifacts.describe` 的 size/sha256 与磁盘一致、`artifacts.read` 分两页 hex 读回并逐字节重组、`timeline.list` 见 `session.created`、关会话后 `audit.list` 依次见 close/create;另一条钉住敌意输入永远留在信封内——不存在的二进制、未知制品 id（`not_found`）、未知会话 id（`session_not_found`）、路径穿越型会话 id（`invalid_request`）各答 `ok=False` 的结构化错误而非协议层失败,且连接在这些敌意之后仍然可用。只依赖检出与已装的包,恒跑不 skip。
