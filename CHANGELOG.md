...
非 PE 后端（adb/frida/web/proxy/apk/jadx/apktool/jsre/r2/Ghidra）的超时现在如实标记为可重试。这些后端的错误经各服务层 `_as_rpc`（r2/Ghidra 则在 `service_ext` 内联同款包装）转成 `XdbgRpcError` 再进 `_failure` 信封，而 `XdbgRpcError.retryable` 默认 `False`、这些包装又都没传该标志——于是每个后端的 `timeout`（设备忙、页面加载慢、CLI 工具跑大输入慢等最典型的瞬时故障）都被报成 `retryable=False`，与 `_failure` 里其余所有超时（`BoundedCancelled` / `TimedOut`、die/exeinfope 的 `timeout`、`TimeoutError`）一律可重试相矛盾；照 "据 `code` 与 `retryable` 分支" 行事的无人值守调用方会拒绝重试一次本该重试的瞬时超时。现把映射收敛到 `core/results.rpc_from_backend_error`，以单一 `RETRYABLE_BACKEND_CODES={"timeout"}` 派生该标志（其余码——`capability_unavailable`、`invalid_params`、`invalid_state`、`not_found`、`permission_denied`、`too_large`，以及既可能是坏输入又可能是抖动、退避未必有用的兜底 `backend_error`——仍不可重试）；六个 `_as_rpc` 与 `service_ext` 的 21 处内联点统一委派给它，PE 线的 `service_ui` 不动。`test_backend_error_contract.py` 固定：任一后端的 `timeout` 穿过整条信封为可重试、七个永久码不可重试、每个 `_as_rpc` 都从 code 派生、共享 helper 覆盖 r2/Ghidra 的内联路径，并锁死可重试码集合恰为 `{timeout}`（防止将来悄悄放宽无人值守调用方会重放的范围）。

`AnalysisService.close_all` 回收非 PE 线持有的长生命周期资源（web 的浏览器与 driver、proxy 的 mitmproxy 子进程、adb 端口转发）时改为逐后端独立、尽力而为。过去它直线调用 `self._health.stop()` → `web.close_all()` → `proxy.close_all()`，只有 adb 那步套了 `suppress()`——前面任一步抛异常（如 driver 回收卡住）就跳过其后所有清理，遗留 mitmproxy 子进程与 adb 转发，并让异常逃出 `close_all`，而 `run_stdio` 的 finally 与 web 关停路径调用它时并不预期它会抛。现每步经一个基于 thunk 的小 helper 隔离执行，失败不再静默吞掉而是带 `component` 键记入信封的 `errors`（整体报 `close_all_failed`），让操作者看到究竟哪个资源可能泄漏，而不是收到一个假的 `{"closed": n}`；传 thunk 而非提前解析好的绑定方法，使某后端缺该方法（或属性访问本身抛错）也在此被捕获，与旧 `suppress()` 吸收的行为一致。`test_close_all_backend_isolation.py` 固定：web 清理失败仍回收 proxy 与 adb 并报 `close_all_failed` 且不抛、`health.stop` 失败不跳过后端、干净关停仍答 `{"closed": 0}`。
...
非 PE 线"结果过大"的错误码统一为 `too_large`。`device.pull` / `device.screenshot` 之前经 `service_device.refuse_oversized_device_file` 把删掉的超大文件报成 `output_too_large`，而整条非 PE 面（`results.py` 明列的分类里 `too_large` 就是"结果/请求过大"那一格，web/proxy 的 HAR 导出、apk/jadx/apktool 的解出目录树、jsre 的输入、以及 adb 后端*传输前* `sync.stat` 的拒绝）一律用 `too_large`——device 这条是唯一的异类。更糟的是同一次 `device.pull` 会因时序给出两个码：设备 `stat` 在拷贝前跑通并判超大时后端抛 `too_large`，而 `stat` 跑不通、只能在拷贝后按落地文件大小判定时服务层却回 `output_too_large`；对 agent 而言是同一个"文件太大、换小一点的路径"结局，却因一个它看不见的内部细节收到两个不同的码。现把该分支改回规范的 `too_large`，使 device 工具与其余非 PE 后端一致、且单个 `device.pull` 不再对同一结局给两种码。`test_device_artifacts.py` / `test_service_device_branches.py` 钉住该字面量并写明理由（防止有人再引入 device 专用的 `output_too_large`）；验证方式：把源码改回旧码，两处直测即转红。
...
新增 `web.har.read`：把磁盘上的 HAR 文件按请求逐页读回，作为 `web.har.export` 的对读侧。会话模型早已把 `.har` 归为 Web 目标（`classify_target` 按后缀归 WEB，`session.create` 因 `not is_http_url` 把文件系统路径存进 locator 并算好 `binary`/`sha256`），且 `web.har.export` 与 `proxy.export_har` 都会产出 HAR，但此前没有任何工具能消费一个 HAR——一个被建模成一等公民的目标类型其实是死路。新增 `backends/common/har.read_har_entries` 把 HAR 1.2 log 投影成与 `web.network.list` **逐字相同**的紧凑请求摘要键——即浏览器线 CDP 原生驼峰 `url`/`method`/`status`/`mimeType`/`resourceType`，外加一个无在跑侧对应、以 HAR 规范原名出现的 `startedDateTime`（静态 HAR 是这个时间戳唯一幸存之处）；`resourceType` 恒有，HAR 未带 Chrome `_resourceType`（proxy 产出的 HAR）时默认 `""`，与在跑侧读取器的空串默认一致，而非丢键。这样 agent 从存盘 HAR 里 `entry["mimeType"]` 与从在跑采集里取的是同一个键、同一套 `offset`/`has_more` 分页，而不必为存盘侧换用 snake_case 拼法（这正是这个读回工具的全部价值所在）；对畸形 entry 逐个降级而非整份读失败，只有顶层不是「带 entries 数组的 log」才抛 `HarReadError`（服务层映射为 `invalid_params`）。`WebBackend.read_har` 是这里唯一无需浏览器会话的读取器：解析路径后，缺失报 `not_found`、非 HAR 报 `invalid_params`、超过采集上限 `UNREGISTERED_CAPTURE_MAX_BYTES` 在 stat 阶段就报 `too_large`（且绝不删除调用方的输入——它不是本次运行写的 capture，`capped_file_size` 的「拒收即删」不适用），并沿用与 `network_list` 相同的 `_MAX_PAGE` 钳取，杜绝绕过 schema 的 agent 传输请求无界页。该工具在 `tools/catalog.py` 显式归为只读，全表面从 265 增至 **266（149 只读 / 117 写）**；`test_tool_schema_backend_bound_alignment.py` 把它的 `limit` 天花板钉到 `web._MAX_PAGE`，`test_web_har_read.py` 从解析器、后端到服务层三层固定（`serialize_har` → `read_har_entries` 往返读回同一批字段、`not_found`/`invalid_params`/`too_large`、`offset`/`has_more` 差一边界与 `limit` 钳取、无会话即可读），并加测一份带真实 header/body/timings 的外部（Chrome DevTools 风格）HAR 只投影出摘要字段、其余原样忽略。此外在 `test_har_export_spec.py` 里补齐两个生产者的完整读回闭环——`web.har.export` 与 `proxy.export_har` 各自写出的真实 HAR 都能被同一个 `read_har` 读回同批摘要字段：web 侧 `resourceType`（`Document`/`Script`）随往返保留，proxy 侧因 mitmproxy 无 Chrome 资源类型故不带 `_resourceType`、读回时 `resourceType` 恒以 `""` 呈现（而非缺席该键，故两个生产者的 entry 用同一种方式读），这也是把 HAR 收敛进 `common/har.py` 的对称性契约（两个生产者写的都能被这一个读取器读）。另加 `test_the_summary_keys_are_the_live_network_list_camelcase_keys` 把摘要键集恰好钉为 `{url, method, status, mimeType, resourceType, startedDateTime}`，防止有人日后把它改回 snake_case、悄悄破坏「存盘 HAR 与在跑采集同一套键」这条契约。最后补齐发现链路：`web.har.read` 能读两个生产者写的 HAR，却挂在 `web.*` 命名空间下，proxy 用户只有在导出器描述里点到它才会知道有这么个读回工具——而工具描述正是 agent 唯一的发现面。故在 `proxy.export_har` 与 `web.har.export` 的描述里都点名「导出后用 `web.har.read` 读回」，并加 `test_both_har_exporters_point_at_the_reader` 钉住这条交叉引用不被日后改描述时悄悄弄丢。
...
补齐 `WebBackend.read_har` 两条 I/O 故障分支的测试。对全部非 PE 后端跑覆盖率（`backends/{adb,frida,apk,web,proxy,jsre,jadx,apktool,r2,ghidra,common}`，整体 99%）后发现 `web/client.py` 里唯二未覆盖的行正是上一轮新增的 `read_har` 里那两个 `except OSError`：`is_file()` 通过之后显式 `stat()` 出错（竞态删除、权限变更）与随后 `read_text()` 出错（IO 错误、权限竞态）。两处都把 `OSError` 收敛成 `backend_error`（message 含「har unreadable」）而非任由其逸出、被信封渲染成 `internal_error`+落一条 incident。现加 `test_backend_reports_backend_error_when_the_stat_faults`（patch `Path.is_file` 恒真、`Path.stat` 抛 `OSError`）与 `test_backend_reports_backend_error_when_the_read_faults`（真实小文件过 is_file/stat，仅 patch `Path.read_text` 抛 `OSError`），断言收到 `backend_error`；两者天然非空洞——去掉 try/except 后裸 `OSError` 逸出，`pytest.raises(WebError)` 即失败。

`device.launch` / `device.force_stop` 失败时现在如实点名操作与包名，而非回一句泛化的「adb shell failed」。同一轮覆盖率排查暴露出这两个方法里各有一段**不可达**分支：它们把 `_device_shell(...)` 包在 `except AdbError: raise` + `except Exception: raise AdbError("backend_error", "launch failed: …", package=pkg)` 里，可 `_device_shell` 早已把一切失败归一成 `AdbError`（超时归 `timeout`、其余归 `backend_error`），于是 `except AdbError` 分支先命中、裸 `raise` 原样抛出 `_device_shell` 的泛化信息（「adb shell failed: …」、无 package），而带操作名与 package 的 `except Exception` 分支永远进不来——作者想给调用方的「哪个操作、对哪个包失败」的上下文其实一直是死代码。现把两处改成 `except AdbError as exc: raise AdbError(exc.code, f"launch failed: {exc.message}", **{**exc.details, "package": pkg})`：保留原 `code`（超时仍是可重试的 `timeout`、不被压平成 `backend_error`）、冠上操作名、并挂上 package。既让原先不可达的分支变成可达且被测，又把一条对 RE agent 实用的诊断信息真正送到调用方。`test_adb_backend_branches.py` 里 `test_launch_wraps_backend_failure` / `test_force_stop_wraps_backend_failure` 现额外断言 message 含「launch failed」/「force-stop failed」且 `details["package"]` 正确，并新增 `test_launch_failure_keeps_a_timeout_code`（`_device_shell` 抛 `timeout` 时 code 不被压平）；三者均非空洞——恢复成裸 `raise` 后「launch failed」与 `package` 断言即转红。至此非 PE 后端在这批模块上无未覆盖行。
...
`js.unpack_bundle` 的 docstring 现在如实点名它每次都返回的 `listing_truncated`——又一处与上一轮 `web.har.read` 的 `mimeType` 同类的「答案形状地图漏字段」缺陷。后端 `unpack_bundle` 的回包恒含 `output_dir`/`file_count`/`files`/`count`/`total`/`offset`/`has_more`/`listing_truncated`（非零退出时再加 `exit_code`/`tool_failed`/`stderr`），但 docstring 此前只列到 `has_more` 就停了，把 `listing_truncated` 漏在外面。这个字段并非可有可无：`has_more` 只表示「本页没走到 `total`」，而 `listing_truncated` 表示目录文件数本身撞上了 50,000 的计数上限——即 `total`/`file_count` 只是下界而非精确值；一个信任 `total` 为精确值的无人值守流程会把一个超大 bundle 读少。既有 `test_js_wasm_fields.py` 里 `test_unpack_bundle_says_when_the_file_list_was_cut` 断言了 `has_more`、`count`、`total` 等，却从未断言 `listing_truncated` 出现在回包或 docstring 里，于是这处遗漏无人守。现补 `test_the_unpack_bundle_docstring_names_every_field_it_returns`：真实跑一次 `unpack_bundle`（干净退出与非零退出两种回包形状都跑），把返回的每个键都断言必须逐字出现在 `js.unpack_bundle` 的 docstring 里——键从实时回包读、而非硬编码清单，故后端新增字段时该测试仍如实跟随；已验证非空洞（把 docstring 里所有 `listing_truncated` 字样去掉后，测试报「unpack_bundle returns 'listing_truncated' but the docstring never names it」而转红）。

`r2.functions` 的 docstring 现在与它的兄弟列举工具口径一致地点名截断三件套。r2 的列举类工具（`r2.strings`/`imports`/`exports`/`xrefs`）都把 `aflj`/`izj`/`iij`/`iEj`/`axj` 的 JSON 数组经 `enrich_r2_payload` 投影并在 4096 项处封顶，撞顶时回包加 `items_truncated`/`items_total`/`items_limit`——四者的 docstring 均如实写着「Read items_truncated, items_total and items_limit when the list filled the cap (4096)」，唯独同样跑 `aflj`、同样会撞同一个 4096 顶的 `r2.functions` 的 docstring 只写到 `items_truncated` 就停了，把 `items_total`（真实函数数）与 `items_limit` 漏在外面；一个在大二进制上读 `r2.functions` 的无人值守流程能看见「被截断」却没有 docstring 告诉它的字段去知道究竟有多少函数。根因是 `r2.functions` 是唯一没有字段测试的 r2 列举工具（`test_r2_{strings,imports,exports,xrefs,disasm,open}_fields.py` 齐备，独缺 functions），无人守着它的口径与兄弟对齐。现把 docstring 补齐为与兄弟同款措辞，并新增 `test_r2_functions_fields.py`：既用 `enrich_r2_payload` 跑 4099 项断言 `count==4096`/`items_truncated`/`items_total==4099`/`items_limit==4096`、且无 `truncated`/`has_more`/`functions` 字段、item 保留 `name`/`offset`/`size` 并映射出 `address`，又断言 docstring 逐字含 `items_truncated`/`items_total`/`items_limit`/`4096`/`no functions`。已验证非空洞——把 docstring 退回原样后 `items_total` 一项即 `assert 'items_total' in doc` 转红。

`web.har.export` 与 `proxy.export_har` 的 docstring 现在都点名它们各自后端每次都返回的 `size`——又一处（这次是同一族的两个工具）「答案形状地图漏字段」。两个后端的 `har_export`/`export_har` 都返回 `{path, entry_count, truncated, size}`（`size` 即 `serialize_har` 得到的 HAR 字节长度），服务层的 `_register_capture` 以 `{**payload, "artifact_id": …}` 原样透传，故 `size` 确实到达调用方；但两份 docstring 都只列到 `path`/`entry_count`/`truncated`（外加 `artifact_id`），把 `size` 漏在外面。这个字段不是可有可无：`web.har.read` 在 stat 阶段就按采集上限 `UNREGISTERED_CAPTURE_MAX_BYTES` 拒收过大的 HAR，一个刚导出就想读回的 agent 正需要 `size` 来判断读回会不会被 `too_large` 挡下。既有字段测试（`test_web_har_fields.py` 与 `test_proxy_fields.py` 各自的 export 用例）都只断言了 `entry_count`/`path`，从未碰 `size`，于是这处遗漏两头无人守。现两份 docstring 补上 `size` 及其含义，并在两个既有 export 字段测试里各加 `assert payload["size"] == <导出文件>.stat().st_size` 与 `assert "size" in doc`。已验证非空洞——把两份 docstring 里的 `size` 字样去掉后，两个用例的 `assert "size" in doc` 同时转红。

`ghidra.functions`/`symbols`/`xrefs`/`decompile` 四个工具的 docstring 现在都点名它们每次都返回的 `export_path`/`project_dir` 与（成功时几乎必有的）`artifact_id`——这是本轮同类里最大的一处「答案形状地图漏字段」（四个工具各漏三个）。ghidra 后端 `_export_unlocked` 会给每份回包无条件盖上 `payload["export_path"]` 与 `payload["project_dir"]`，服务层 `_ghidra_export` 又在导出文件登记成制品后加 `data["artifact_id"]=art["id"]` 并 `_success(data,…)` 原样返回，故三者都到达调用方；但四个工具的 docstring 只列到 `items`/`count`/`has_more`（decompile 是 `decompiled`/`truncated`/`found`）。其中 `artifact_id` 是把整份导出读回来的功能句柄（分页 `items` 之外的全量），而 `ghidra.analyze` 本就点名并返回 `project_dir`（同样是跑完即删的目录），所以在同族的兄弟工具上把这三者留作匿名字段，是 agent 面向契约不该有的不一致。现四份 docstring 各补上这三者及其含义（`artifact_id` 读回全量、`export_path` 磁盘上的原始导出 JSON、`project_dir` 分析所在目录且跑完即删）。因本机未装 analyzeHeadless 无法实跑，遂沿用本文件既有 `ghidra.analyze` 用例的「从源码固定回包形状」手法：`test_ghidra_listing_tools_name_the_export_handles` 断言后端源码里确有 `payload["export_path"]=str(out_path)` 与 `payload["project_dir"]=str(project_dir)`、服务层源码里确有 `data["artifact_id"]=art["id"]`，并断言四个工具 docstring 逐字含 `artifact_id`/`export_path`/`project_dir`（列举类再含 `count`/`has_more`、decompile 再含 `decompiled`/`found`）。已验证非空洞——把 `ghidra.functions` 那句补充删掉后，`assert "artifact_id" in doc` 即转红。
...
修好 JS/WASM 静态工具的一个「死区」数据丢失缺陷。两个传输层都会把每条工具回复过一遍 `bounded_tool_result`（预算为该工具的 `ResourcePolicy.max_result_bytes`，262_144 字节）——它不是把超预算的回复裁短，而是**整条替换**成一段约 16 KB 的文本摘要、连同信封一起丢掉。而 `js.deobfuscate`/`js.beautify` 与 `wasm.wat`/`wasm.info` 把工具输出内联在 `code`/`wat`/`objdump` 字段里返回，且不像 web/proxy 后端那样把溢出部分落盘成制品——jsre 后端只按自己的 `_MAX_INLINE` 截断并置 `truncated`。这个内联上限此前是 **400 KB**，比传输预算还大，于是留出一段死区：输出落在 262 KB 到 400 KB 之间时，jsre 认为没超（`truncated=False`）照单内联返回，传输层却因为整体超预算把这条回复连 `code` 字段一起摘要抹掉——调用方明明让 webcrack 产出了可用代码、也确实塞得进 jsre 的 400 KB 内联额度，最后却只拿到一段通用摘要。现把 `_MAX_INLINE` 降到 **200 KB**，与 web/proxy 后端的 `_MAX_INLINE_BODY` 对齐、稳落在 262 KB 预算之内（给 JSON 转义与信封留足余量），从而让 jsre 自己那份如实的截断（保留 `code`/`wat`/`objdump` 字段 + `truncated=True`）先于传输层的抹除生效。新增 `test_jsre_inline_fits_transport_budget.py`：对四个内联工具逐个断言 `_MAX_INLINE ≤ max_result_bytes`、断言三处内联后端的上限相等（防止某一处又漂回死区），并做一次行为核验——把一份超额输出经 `_bounded_output` 截到内联上限、裹进信封后过 `bounded_tool_result`，断言它**没有**被摘要抹掉、`code` 字段确实幸存（内容带大量换行以顺带压一压 JSON 转义）。已验证非空洞：把 `_MAX_INLINE` 调回 400 KB，结构不变式与行为核验双双转红（后者正是因为 400 KB 的内联超出 262 KB 预算而被整体摘要）。

接着上一条把同一「传输层截止时间够不着真实耗时」的缺陷补到那些**不暴露 `timeout` 参数、却在内部阻塞超过 60s** 的工具上——上一条的钉子只看得见 schema 里带 `timeout` 的工具，这批看不见。ADB 的文件传输统一以后端自己的 `_ADB_TRANSFER_TIMEOUT_S`（120s）为界：`device.install` 推一个 APK、`device.pull`/`device.push` 搬一个文件、`device.uninstall` 可能要清一个大应用的数据、`frida.server.ensure` 推 frida-server 二进制——它们都没有 `timeout` 入参，于是全部停在 60s 默认传输截止时间上，真实的大文件传输/安装常在 60s 被拦腰杀掉（后端本愿意跑到 120s）。现按各自的内部有界步骤把传输截止时间抬够：`device.install`（传输 120s + `pm path` 校验 shell 30s）、`device.uninstall`、`device.pull`（先 `stat` 8s 再拉 120s）、`device.push` 均设 180s；`frida.server.ensure` 因为是「探测 + 推二进制(120s) + chmod(30s) + 启动(8s) + 复探」的多步链路而设 240s。其余 ADB/Frida 工具的内部界都 ≤30s（shell/probe）或 ≤30s（`device.logcat` 为 `-d` dump 而非流式、`device.screenshot` 走 shell 界），稳在 60s 之内，不动；纯属挂起上限的 `_device()` socket 120s 也不动（健康设备秒回，真挂起时 60s 杀掉反而对）。`test_non_pe_timeout_budget.py` 新增一组针对这五个传输工具的断言，直接把它们的 `ResourcePolicy.timeout_seconds` 钉到从 adb 后端导入的 `_ADB_TRANSFER_TIMEOUT_S` 之上——将来谁调大这个传输常数却忘了同步策略，测试即转红。已验证非空洞：把 `device.install` 从 `_TOOL_TIMEOUTS` 移除退回 60s，其用例即因 60s < 120s 转红。

修好一个会让重量级非 PE 工具在真实输入上直接超时失败的实际缺陷：它们对外宣称的 `timeout` 够不着。每个工具的传输层截止时间 `ResourcePolicy.timeout_seconds` 是对**整次调用**的硬上限，MCP 适配器（以它为 `offload` 超时）与 agent 编排器（`min(tool_timeout, policy)`）双双据此强杀，默认 60s；`catalog.py` 里只给「首次打开大样本」的 `apk.open`/`dynamic.open`/`ghidra.analyze`/`static.open` 抬过这个上限。但真正吃时间的是那些同步阻塞在 CLI 子进程或浏览器导航上的工具：Android 的 apktool/jadx（`apk.decode`/`decompile`/`export_sources`/`repack`/`sign`，`timeout` 以秒计、默认 300–600s、上限 1800s）、Ghidra headless（`analyze`/`decompile`/`functions`/`symbols`/`xrefs`，每次导出都要重新导入二进制，默认 180s、上限 600s）、radare2/rizin（`r2.*`，上限 120s）、webcrack/wabt 的 node 工具（`js.beautify`/`deobfuscate` 上限 600s、`js.unpack_bundle` 上限 1200s、`wasm.info`/`wat` 上限 600s）、以及阻塞到页面落定的 `web.open`/`web.navigate`（上限 120s）——它们的 `timeout` 参数丈量的正是调用本身的时长，却全部落在 60s 的默认截止时间之下。后果是传输层先于工具自身的超时把调用杀掉，宣称的 `timeout` 永远够不着：光是**无参**调用就有 14 个工具当场破（`apk.decode` 默认 600s 却在 60s 被杀），按对外宣称的最大值算则有 25 个破（连已抬到 180s 的 `ghidra.analyze` 都够不着自己 600s 的上限）。毫秒计或「起个后台作业就返回」的参数（如 `ui.click`、`trace.start`）不在此列，故不能一刀切地按 schema 最大值抬。现把 `_TOOL_TIMEOUTS` 扩到覆盖这批同步非 PE 工具，令各自的传输截止时间不小于其对外宣称的 `timeout` 上限（apk.* → 1800、ghidra.* → 600 含把 `ghidra.analyze` 从 180 抬到 600、r2.* → 120、js 的 600/1200、wasm.* → 600、web.open/navigate → 120），全部落在 agent 硬顶 1800s 之内。新增 `test_non_pe_timeout_budget.py`：对每个带 `timeout` 参数的非 PE 工具断言其 `ResourcePolicy.timeout_seconds` ≥ schema 宣称的最大值、且该最大值 ≤ 1800s（一旦有毫秒计的界混进非 PE 面就转红提醒人工复核，而非默默要一个不可能的策略），并加一道发现性非空守卫（至少覆盖 `apk.decode`/`ghidra.functions`/`r2.open`/`web.open`）防止将来绑定或 schema 抽取失灵后测试变空洞。已验证非空洞：把 `apk.decode` 从 `_TOOL_TIMEOUTS` 移除、其策略退回 60s 默认，该工具的用例即因 60s < 1800s 转红。

给非 PE 工具面的读/写「效应」分类补上一份独立的正确性钉子。读/写边界的既有守卫由 `test_write_policy_surface.py` 全面固定——只读部署拒绝每个写工具、放行每个读工具、且被守卫集合恰等于写分类集合——但它的「写」是**从分类本身**派生的，因此抓不住「写工具被错填成 READ_ONLY」：这样的工具只会同时从被守卫集合与写分类集合里消失，等式照样成立。而这背后正是一个安全缺陷：一个有副作用的非 PE 工具（`apk.sign` 刷入重打包 APK、`device.push` 往设备写文件、`proxy.start` 开监听、`web.navigate` 驱动实时浏览器）若被错填成 `READ_ONLY`，就会在 `read_only` 部署里仍可调用——正是该模式要禁止的。`catalog.py` 构建期的策略检查只保证「全部已分类、无重复」，不保证分类**正确**。新增 `test_non_pe_tool_effect_policy.py`：一份经人工评审、把 83 个非 PE 工具逐个映射到其应有效应（`READ_ONLY` / `STATE_CHANGE` / `STATE_CHANGE+FILE_WRITE`）的独立真值表，逐工具断言目录里的分类与之相符、并把该映射与守卫（`spec.write`）挂钩（凡非 `READ_ONLY` 者必被守卫），使错填成为安全红灯而非仅仅样式问题。再加一道前缀普查（`apk.`/`device.`/`frida.`/`js.`/`wasm.`/`proxy.`/`web.`/`r2.`/`ghidra.`/`workspace.mode.`）：任何新增或改名、却没在此登记的非 PE 工具都会让测试转红，逼出读/写评审而非默默走默认。已验证非空洞：把 `apk.sign` 从 `_FILE_WRITE_NAMES` 挪到 `_READ_ONLY_NAMES`（总数仍 266、无重复，构建期检查照过），逐工具用例 `[apk.sign]` 与「写工具必被只读拒绝」用例即双双转红。

给两条静态分析非 PE 后端 r2 与 Ghidra 的**服务层**补上缺失的 fake-backend 分支测试。其余每条非 PE 服务层——web、apk、device、jsre、proxy 及设备版 Frida——都有一份用假后端驱动「成功路径 + 错误映射」的分支测试；唯独 `service_ext` 里的 r2/Ghidra 是例外：它俩仅有的服务级测试（`test_r2_closed_session`、`test_ghidra_closed_session`）只驱动了「拒绝」分支，一次完全成功的调用、`R2Error`/`GhidraError` → 结构化失败的映射、以及 Ghidra 落盘导出后登记 `artifact_id` 的路径，从未经服务层被跑到（`service_ext` 的 r2/Ghidra 段此前整片红）。新增 `test_service_ext_r2_ghidra_branches.py`：以假的 `R2Client`/`GhidraClient`（两者都是 `service_ext` 的模块级导入，改名即覆盖所有调用点）钉住——成功调用回报 `backend` 且原样透传后端载荷（`r2.open`/`info`/`functions`/`strings`/`imports`/`exports`/`disasm`/`xrefs`，`ghidra.analyze`/`functions`/`symbols`/`xrefs`/`decompile`）；后端错误变成携带其 code 的结构化失败；未预期异常仍被捕获；Ghidra 导出落盘则登记为制品并回 `artifact_id`、未落盘（如 decompile 只回源码）则不无中生有一个 `artifact_id`；`ghidra.xrefs`/`decompile` 缺 address、以及未知导出 mode 都是 `invalid_params`。并把 `r2.open`/Ghidra 早已享有的「关停竞态」保护对齐到此前没覆盖的 r2 兄弟入口（`r2.disasm`/`xrefs`/以及走 `_r2_request` 的白名单命令）：一个已 CLOSED 的会话、或在 r2 调用途中关闭的会话，都必须被拒绝且绝不把后端记到一个再也用不了它的会话上。这样 `service_ext` 的 r2/Ghidra 段（方法体与 `_r2_request`/`_ghidra_export` 两个 helper）由本测试与既有关停测试合力覆盖满。已验证非空洞：临时移除 `r2.disasm` 的运行后状态复查，`test_disasm_closing_mid_run_is_refused` 即因该调用对一个中途关闭的会话回 `ok=True`（正是该复查所防的 bug）而转红。

给所有轨道（含全部非 PE 工具）共用的 stdio 传输补丁 `stdio_server_with_parse_replies` 补上「关停竞态被吞掉、不冒充服务崩溃」这条收尾契约的钉子。这个 shim 是 MCP SDK `stdio_server` 的逐行镜像（把 SDK 那种「解析失败只往读流塞异常、服务端记 Internal Server Error 且永不回复」改成「带 id 的坏行回一条 JSON-RPC 错误」），reader 与 writer 两个协程各自用 `except anyio.ClosedResourceError: await checkpoint()` 把关停期的内存流关闭当作正常收尾——SDK 原版把这两行都标了 `# pragma: no cover`，我们的镜像照抄了逻辑却没照抄豁免，于是它们成了 `stdio_errors.py` 仅剩的未覆盖行。这不是可有可无的分支：agent 断开（stdin EOF）、服务端提前收流等日常关停都会让这些内存流在协程还在 `send`/写出时被关掉，若不吞掉 `ClosedResourceError` 就会经 task group 上抛、把一次正常关停显示成服务崩溃。既有 `test_stdio_errors.py` 只驱动了「转发合法行/回复带 id 的坏行/对无 id 垃圾保持沉默」这些正常路径，从未从关停竞态这扇门进来。新增两个用例，都在测试本就掌控的文件边界注入而非去戳 anyio 内部：`test_stdin_reader_swallows_a_closed_resource_on_shutdown` 让注入的 stdin `readline` 抛 `ClosedResourceError`（`_read_bounded_line` 在工作线程里跑它，异常照原样冒回读循环），`test_stdout_writer_swallows_a_closed_resource_on_shutdown` 喂一条带 id 的坏行产出一条回复、再让注入的 stdout `write` 抛 `ClosedResourceError`（writer 每条回复后都 flush，异常从写出里冒出来）——两者都断言上下文管理器干净退出、没有异常逃逸。已验证非空洞：把两个 `except anyio.ClosedResourceError` 改成只接一个不匹配的 anyio 错误类型后，两个用例都因 `ClosedResourceError` 经 task group 逃逸而转红。
...
把「未知会话」在非 PE 全线统一收敛为 `session_not_found`，不再泄漏后端的 `invalid_state`。`SessionNotFound` 是一个刻意独立的错误类型：它是唯一告诉无人值守调用方「这个会话 id 打错了或已丢失，去重建」的信号，而非「稍等再试」。但若干非 PE 服务方法在解析会话之前就先摸到了后端——web 的 `web.navigate`/`console`/`network.list`/`scripts`/`wasm.list`/`dom.snapshot` 全经 `_web_wrap` 直调后端、proxy 的 `proxy.flows`/`replay`/`status` 全经 `_proxy_wrap` 直调后端，两者都没先过 `registry.get`；frida 的 `frida.attach`/`modules`/`exports`/`memory.read` 则经 `_require_debuggee_pid`，它对一个「从不存在的会话」与「存在但没有 debuggee 的会话」一视同仁地回 `invalid_state`（`dynamic_state` 两种情况都回 `ok=False`，helper 把它们压成了同一个码）。于是一个打错的会话 id 收到的是后端的「没有活动页面」/「代理没在跑」/「没有活动 debuggee」——都报成 `invalid_state`，读起来像「会话存在、只是忙」，把调用方推进一个永远等不到的 wait/retry 循环，正是 `SessionNotFound` 这个类型当初要防的。同族里 `web.screenshot`（经 `_web_artifact_dir` 先 `registry.get`）、`proxy.flow.get`/`export_har`、以及 `r2.*`/`ghidra.*`（`_r2_request`/`_ghidra_export` 都先解析会话）本就答 `session_not_found`——这条修复把上述掉队的 13 个入口拉到同一契约上：`_web_wrap`/`_proxy_wrap` 在直调后端前先 `registry.get`，`_require_debuggee_pid` 先 `registry.get` 再做它自己的 debuggee 检查。由于已 CLOSED/FAILED 的会话仍被 registry 保留（`_retire_closed` 只按上限淘汰最旧的），`registry.get` 只对「真不存在」的 id 抛 `SessionNotFound`，而一个保留下来的关停会话仍能解析、仍会拿到后端的 `invalid_state`——即修复只重分类「未知」这一格，不动「存在但状态不对」那一格。附带把审计从 38 个 `session_not_found`/11 个 `invalid_state` 收敛到 57 个 `session_not_found`/0 个 `invalid_state`（另有 `proxy.stop`/`web.close` 两个幂等生命周期操作仍答 `ok`），并顺带让 `proxy.status` 与早已如此的 `web.status` 一致（未知会话答 `session_not_found` 而非「未运行」）。`_require_debuggee_pid` 是与 windbg live 工具（`windbg.attach`/`live_threads`/`live_modules`/`live_disasm`，PE 线）共享的 helper，故这些工具也一并获得同样正确的「未知会话→`session_not_found`」行为——是一致性改进而非回归。新增 `test_unknown_session_uniform_not_found.py`：对这 13 个入口逐个断言未知会话答 `session_not_found`；并加三道「不过度」守卫——用一个可达但无页面的假 web 后端、一个可达但未运行的假 proxy 后端、以及一个真实但从未 attach 过 debuggee 的会话，断言它们在会话确实存在时仍答 `invalid_state`（web 那道还额外核验一个被强制 CLOSED 的保留会话依然答 `invalid_state` 而非 `session_not_found`），证明修复没把真正的「状态不对」也一并抹成「不存在」。已验证非空洞：还原任一处 `registry.get`，对应入口即因未知会话回 `invalid_state` 而转红。

给 `service_ext` 里的探针版 Frida 服务方法补上缺失的 fake-backend 分支测试，与上一轮 r2/Ghidra 那条同源。上一轮 `test_service_ext_r2_ghidra_branches` 给 r2 与 Ghidra 的服务方法补了「成功路径 + 错误映射」的假后端覆盖，但同一文件里紧挨着的探针 Frida 方法——`frida.attach`/`frida.modules`/`frida.exports`/`frida.memory.read` 以及 `frida.hook.template` 的 PE 分支——仍只跑到了「拒绝」路径：它们既要真的 frida、又要一个活的被调试 pid，于是成功包装（`_success(..., backend="frida")`）、`FridaError`→结构化失败的映射、以及 `except BaseException` 兜底这三段从未经服务层被执行（`service_ext` 的 462–563 行整片红）。根因是这些探针方法先要过 `_require_debuggee_pid`（读 `dynamic_state` 里的 `debuggee_pid`，来自 x64dbg 动态后端），没有真后端时它必然在调用 `FridaClient` 之前就抛 `invalid_state`，把方法体挡在覆盖之外。新增 `test_service_ext_frida_branches.py`：以假的 `FridaClient`（`service_ext` 的模块级导入，改名即覆盖所有调用点）顶替探针后端，并把 `dynamic_state` 打桩成回一个带 `debuggee_pid` 的成功 Result——这不绕开 `_require_debuggee_pid`，而是让它真的跑一遍（含上一轮新加的 `registry.get` 守卫与 pid 提取，仅把无法在单测里造出的 x64dbg 动态态读取这一步替掉）。钉住：四个探针成功时都回 `backend="frida"` 且原样透传后端载荷、并确实用上了 `_require_debuggee_pid` 解析出的那个 pid（断言回包 `pid==4321`）；`FridaError`（如 `permission_denied`/`invalid_params`）变成携带其 code 的结构化失败；未预期的 `RuntimeError` 仍被兜成 `ok=False`；以及 `frida.hook.template` 在会话没有 `frida_authorized` 元数据时走 PE 分支、解析 debuggee 后本地 `hook_template` 成功（设备分支早由 `test_frida_hook_template_closed_session` 覆盖）。已用 term-missing 复核：补测后 `service_ext` 的缺失行从 462 之后直接跳到 572（windbg，PE 线，另论），即探针 Frida 方法体 462–563 已被覆盖满。

把「非 PE 分页工具 schema 广告的 `limit` 上限 == 后端实际钳制的上限」这条不变量固化成测试。每个非 PE 列表端点都在 schema 里以 `Field(ge=1, le=N)` 声明 `limit`（`device.logcat` 是 `lines`），而每个后端又各自用 `max(1, min(limit, _MAX_...))` 再钳一次——后者存在的原因是 agent 与 OpenAI-bridge 两条传输会直调 handler、跳过 pydantic 校验（只有 MCP 一条路会跑），所以只信 schema 的后端会把一个未设上限的页面交给一个已经绕过 schema 的传输，正是 `apk._clamp_page`、`ghidra._MAX_LIST_PAGE` 当初长出守卫的同一动机。于是每个工具就有两个必须相等的数：agent 读来决定单页取多大的 schema 上限，和后端实际钳到的常量。二者一旦漂移，工具就会悄悄误导 agent——schema 抬到 4000 而后端仍钳在 2000，就等于告诉 agent 一页能取 4000 行，可它永远只会拿到 2000（多出来的读起来像「没有了」，或逼出本不必要的 offset 翻页）；反过来后端上限抬过了过期的 schema，则白白浪费一段 agent 被告知不可用的容量。apk、ghidra、jsre 三个后端都写了注释承诺自己的上限「与 schema 保持相等」，但没有任何东西在强制它。新增 `test_non_pe_limit_cap_alignment.py` 覆盖整条非 PE 线（web/proxy/frida/apk/js/ghidra/device 共 22 个分页工具），逐个断言 schema 的 `maximum` 恰好等于后端那个 `_MAX_*` 常量（常量直接从对应后端 import，任一侧改动都会破坏相等），并顺带钉住下界为 1（后端都以 `max(1, ...)` 钳制）；`size`（`frida.memory.read`）与 `count`（`r2.disasm`）刻意排除在外——它们表达的是调用方想要的确切数量，其后端对越界值是「拒绝」而非「钳到页上限」，契约不同。另加两道守卫：一是非空洞守卫，断言发现集覆盖 `web.network.list`/`frida.modules`/`apk.classes`/`ghidra.functions`/`device.logcat` 等代表；二是完备性守卫，从 catalog 反向枚举所有带分页参数的非 PE 工具，要求它们与 `_EXPECTED` 一一对上——这样新增或改名一个分页端点若不落到本表，测试即转红，不变量无法被悄悄绕过。已验证非空洞：把任一 schema 的 `le=` 抬 1（如 `web.console` 2000→2001），对应用例即以「广告 2001 但后端钳到 2000」转红。
