...
### 新增（`apk.intent_filters`：导出组件的另一半——什么 intent 能敲开这扇门）

- APK 工具面能列组件、能(在姊妹分支里)标注哪些导出,但**没有结构化出口回答"什么 intent 触发这个组件"**:
  deep-link 的 scheme/host(`myapp://pay` 这类,常是恶意样本的 C2 触发点或钓鱼入口)、receiver 监听的
  action(如 `BOOT_COMPLETED` 开机自启),此前只能从 `apk.manifest` 的原始 XML 里自己解析。导出组件是"门开着",
  intent-filter 是"什么钥匙能开"——攻击面地图缺了这后半张。
- 新增只读工具 `apk.intent_filters`(工具面 265 → 266,读 148 → 149):遍历 activity/service/receiver(provider
  按 authorities/授权 URI 触发、无 intent-filter,故排除),每条回 component/type/actions/categories/data,
  其中 data 是逐条 `{scheme/host/port/path/mimeType 等存在的属性}` 字典。分页(offset/limit,上限 1000,源头
  钳制),回 intent_filters/count/total/offset/has_more;无 filter 的组件不出现在列表里(列表即"可达集"而非
  组件花名册)。诚实性:组件名扫描触顶时带 `names_capped`,某组件的 filter 解析抛错(敌意 manifest)时计入
  `parse_errors` 并跳过而非无声吞掉——空页或短页不该被读成攻击面比实际小。
- 三层接线齐全(client → `service_apk.apk_intent_filters` → MCP 工具 + catalog 只读声明 + MCP 面期望清单),
  README 与 catalog 计数守卫同步。新增回归:deep-link 活动 + 开机 receiver 如实映射(含 scheme/host、类型、
  无 filter 组件被省略)、分页 has_more 如实、某组件 walk 抛错 → 计入 `parse_errors` 且不消失、干净扫描不带
  `parse_errors`/`names_capped`。
