...
- `SECURITY.md` 增加「安全开关速查」：把 `local_full_access` 与三个 autonomy 配置键
  ...
  两条易踩坑规则。
- 修正文档口径：README 里「敌意输入下全部返回信封」的工具数从过时的 262 改为 269（=全部
  270 个 MCP 工具减去会真删数据的 `artifacts.gc`），并改述为「绑定工具数 − 1」的不变式，
...
- **全表面资源策略有界**：全部 270 个工具的 `resource_policy` 都有有限且为正的超时与为正的
...
- **Cursor 下划线别名解析 + 全表面无碰撞**：Cursor 以 `static_functions` 调用而 catalog 注册的是
  ...
  新增契约:钉住出厂全表面 270 个 MCP 名折叠后无碰撞,并直测别名解析(点名/下划线/多段名都命中同一
