...
- **`apk.largest_classes`**：按体量给内部类排名，让承载主要逻辑的「god class」浮到前面，而不是
  埋在 `apk.classes` 的字母序里。`apk.classes` 列名、`apk.packages` 按包计数，本工具则给类本身排序：
  每行 `class_name`、`num_methods`、`num_fields`，按方法数降序、再字段数降序、再类名排序，最重者在前、
  同数可稳定分页。external 类跳过（其成员不在本包定义）。返回 `classes`、`count`、`total`（收集数、
  封顶 10000、超出置 `scan_capped`）、`offset`、`has_more`。只读工具。
