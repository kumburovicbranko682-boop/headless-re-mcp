...
- 新增 `reparse_cut_output`:仅在 `truncated` 时启用,只信任**首个结构字符**处的值——
  完整解开则照常使用(切点在其后);是未闭合的根数组则按序打捞其完整的顶层条目
  (逐条 `raw_decode`,在切尾处停止),payload 标注 `items_salvaged: True`;其余情况
  诚实报 `parsed: False` 并保留 raw,绝不采用更深处侥幸存活的片段。非截断路径行为不变。
- 四个列表工具(functions/strings/imports/exports)的文档注明 `items_salvaged` 语义。
  新增 `tests/unit/test_r2_truncated_output_salvage.py`:实测到的 callrefs 捏造形态、
  dict 误入 info、打捞与 4096 cap 叠加、根后切点照常解析、非截断路径不变、run()→enrich
  端到端接缝;真机复验(切中列表打捞出 6/12 个完整函数条目,切进首条目内诚实拒绝),
  变异测试(去掉截断分支)确认防线有效。
