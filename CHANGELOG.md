...
- `device.security`（只读）：一个聚焦的态势探测（不是完整审计），报告两个别的工具都没有的权威事实：
  SELinux 强制模式（`getenforce`）与 `su` 是否在 PATH 上可达（尽力而为的 root 迹象）。逆向据此在信任
  一份抓包之前，快速区分「加固的原厂机」与「permissive 的已 root 机」。`selinux` 归一化为
  Enforcing/Permissive/Disabled，缺失或返回异常时为 null，并保留 `selinux_raw` 以便原样可见；
  `su_on_path` 为布尔、`su_path` 给出解析到的位置——该检测尽力而为，隐藏 root（Magisk deny-list）不会
  出现在 PATH 上。被设备拒绝的探测进 `unavailable` 而非臆测，只有两个探测都失败才算整体失败。
