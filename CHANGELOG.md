...
- **`apk.class_info`**：一次给出某个类的继承与形态——`superclass`（extends 的 smali 名，
  辨认 Activity/Service 或自定义基类最快的一手）、`interfaces`（实现的接口名，即回调与
  IPC 契约）、`access`（标志串）、`is_external`（仅被引用、成员不在本 APK）、
  `is_android_api`（androguard 判定的框架类）、`num_methods` 与 `num_fields`。类名在 DEX
  内唯一，故只描述一个类或回 `not_found`；`interfaces` 排序并封顶，`interfaces_count`
  给真实总数、`interfaces_truncated` 标注是否截断。接受点名或 `Lsmali/` 形式。只读工具。
