...
### 新增（`apk.exported_components` 枚举可被其它应用触达的攻击面）

- 新增 `apk.exported_components`：`apk.components` 按名字列出全部组件，此工具收窄到"导出"的那些
  并说明缘由。判定规则依 Android 默认：`android:exported="true"` 为导出；属性缺省但声明了
  `intent-filter` 也视为导出；显式 `exported="false"` 一律排除。每行给出 `type`
  （activity/activity-alias/service/receiver/provider）、`name`（原始 `android:name`）、
  `class`（按包名解析，前导 `.` 或裸名补全为全限定）、`explicit`（`exported="true"` 显式置位为
  true，仅凭 intent-filter 默认导出为 false）、`has_intent_filter`、`actions`（intent-filter
  的 action 名，去重排序，按 `_MAX_ACTIONS` 上限截断并置 `actions_truncated`）。直接解析
  `apk.manifest` 的清单 XML，无需 DEX 分析。返回 `components`、`package`、`count`、`total`、
  `offset`、`has_more`，按 (type, class) 排序；`total` 为导出数，上限 256，超出置 `scan_capped`。
