...
- `device.services`（只读）：通过 `service list` 列出设备已注册的 binder 系统服务——正在运行的
  各 manager 以及每个服务暴露的 AIDL 接口，这是 Android 的 hook/攻击面。`service list` 在
  `Found N services:` 头之后每行打印 `<序号> <名字>: [<接口>]`；没有公开接口的服务打印 `[]`，
  这里以空 interface 收进来而不是丢弃。回带 `services`（每条含 `name` 与 `interface`）、`count`、
  `has_more` 与 `reported_total`（设备自报的 Found N 计数），封顶 1000、按名字排序；装满上限的一页
  不会被当成全部服务。一行 adb 报错（设备离线/掉线）按失败处理，而不是空注册表。
