...
`describe_apk`（会话创建时不依赖 androguard 的 stdlib 身份读取）现在识别 APK Signature Scheme v2/v3，不再只看 v1。旧实现只在 `META-INF/` 里找 `.RSA/.DSA/.EC`，那是 v1（JAR）方案；而 Android 7 起默认的 v2/v3 签名数据位于 ZIP 条目与中央目录之间的 **APK Signing Block**、不在 META-INF 下——于是一个按现代方案正确签名的包会报 `signed_v1=false` 且再无其他签名信号，在 `session.get` 里与真正未签名的包无从区分。`apk` 身份元数据新增 `signed_v2`/`signed_v3` 两个布尔（v3.1 归入 v3）：解析定位到 EOCD→中央目录偏移→签名块尾部的 `APK Sig Block 42` 魔数，并用块首尾两处长度字段互校，避免把某个文件条目里恰好出现的魔数误判为签名块；ZIP64、越界或畸形布局一律如实报「两者皆无」而非抛错（这是身份线索，不是验签）。新增经真实 MCP stdio 服务的签名方案身份 Gate（`tests/integration/test_apk_signature_scheme_gate.py`）：字节里带真实签名块的 v2+v3 包经 `session.create`/`session.get` 报 `signed_v2`/`signed_v3` 为真、`signed_v1` 为假，真正未签名的包三个标志全假。
