...
### 新增（`apk.string_xrefs` 反查“这个字符串被哪些方法引用”）

- `apk.strings` 能列出 DEX 里的字符串常量、`apk.xrefs` 能查一个方法的调用方,但都答不上逆向里
  最常问的那句“这个硬编码的 URL / 密钥 / 报错串到底在哪儿用了”。新增只读工具
  `apk.string_xrefs`:经 androguard `Analysis.get_strings()` 定位到值完全相等的
  `StringAnalysis`(字符串常量里前后空白有意义,故不做 strip),取其 `get_xref_from()` 给出的
  `(ClassAnalysis, MethodAnalysis)`,读出引用方的 `class` / `method` / `descriptor`。引用方去重
  并排序,`window` 稳定可分页(`limit` ≤ 1000,与 `apk.xrefs` 同用 `_MAX_XREFS_PAGE`),回传
  `count` / `total` / `has_more`;去重集合本身在 5000 处封顶时置 `scan_capped`(此时 `total` 是下界)。
  `matched` 区分“字符串在池中但无人引用”与“池中根本没有这个字符串”,避免把后者误读成前者。
  空 `value` 报 `invalid_params`。归类只读(`agent_auto_execute`),工具面 265 → **266**
  (只读 148 → 149,写不变),`tools/catalog.py` 计数、README 三处工具算术与计数守卫同步更新。
  新增 `test_apk_string_xrefs_fields.py` 覆盖命中/去重排序/`matched` 两态/`limit` 封顶/空值报错,
  并把 `apk.string_xrefs` 纳入既有的 `limit` 与页上限一致性守卫。
