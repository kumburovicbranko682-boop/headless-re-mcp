...
### 新增（`frida.threads` 枚举目标进程的活动线程）

- 新增只读工具 `frida.threads`：`_ENUM_SCRIPT` 增加 `threads` RPC，短生命周期探针跑
  `Process.enumerateThreads()`。这是继静态清单（`frida.modules`/`frida.exports`）之后的动态一侧——
  此刻进程里有哪些线程、各自执行到哪里，对 Android/原生动态逆向（认出 JNI 工作线程、原生加解密线程等）
  很有用。回 `threads`，每项含 `id`、`state`（OS 调度状态，如 running/waiting）与 `pc`/`sp`
  （程序计数器与栈指针，跨架构别名；线程 context 读不到时留空而非丢行——正在跑的线程仍是线程），
  外加本页 `count`、`total` 与 `has_more`，满上限的一页不会被读成全部线程。仅限会话 debuggee pid。
  `frida.*` 只读线补上活动线程枚举。
