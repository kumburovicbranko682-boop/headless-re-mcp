...
### 修复（apktool `decode`/`sign` 与 jadx `_run` 把 apk 校验挪到能力门之前——缺文件/非 zip/zip 炸弹在未装 apktool/jadx(缺 JRE)的机器上曾被伪装成 `capability_unavailable`,而非各自的 `not_found`/`invalid_params`/`too_large`)

- 同一类顺序错的第四、五处(前三处见上面 `web.open`、adb `_device`、jsre `_require_input`),把这条约定在整条非 PE 线上补齐。jadx 的 `decompile` 早已把 class_name 校验放到能力门之前(并带注释点名这一 masking),但 apk 路径本身的校验(在 `_run` 里,`export_sources`/`decompile` 都经它)以及 apktool 的 `decode`/`sign` 仍是“先判能力再看 apk”。于是在没装 jadx、或没配 apktool/apksigner(缺 JRE)的主机上,一个不存在的 apk、非 zip 文件或申报为炸弹的 zip 统一报 `capability_unavailable`(“去装工具”),而真正的错分别是 `not_found`/`invalid_params`/`too_large`——都是调用方可自查、与工具是否安装无关的输入错。把 `apk.is_file()`、`_require_apk_zip`、`check_zip_expansion`(都是纯文件读,炸弹检查仍在 JVM 启动之前)一并提到能力门之前。新增三条测试:`test_decode_validates_the_apk_before_the_capability_gate`、`test_sign_validates_the_apk_before_the_capability_gate`(均以未配置的 `ApktoolClient(None, None)` 让门是活的)、`test_jadx_validates_the_apk_before_the_capability_gate`(`JadxClient` 指向不存在的可执行文件)。带外验证:把三处顺序改回“先判能力” → 这三条新测试各自失败(缺文件处 `capability_unavailable` != `not_found`/`invalid_params`),其余用配置好的 client 跑的输入校验用例仍绿。

### 修复（jsre `WasmClient._require_input` 把文件校验挪到 wabt 能力门之前——缺文件/超大/非模块在未配置 wabt 的机器上曾被伪装成 `capability_unavailable`,而非各自的 `not_found`/`too_large`/`invalid_params`)

- 同一类顺序错的第三处(前两处见上面 `web.open`、adb `_device`)。`_require_input` 过去先判 `tool is None`(wabt 能力门)再看文件。于是在未配置 wabt 的主机上,`wat`/`info` 收到一个不存在的路径、超限的大文件或非 wasm 文件时,统一报 `capability_unavailable`(“去配 wabt”),而真正的错分别是 `not_found`/`too_large`/`invalid_params`——都是调用方可自查的输入错,与 wabt 是否安装无关,agent 按 code 路由。`_require_existing_file`(含尺寸上限)与 `_looks_like_wasm`(读 `\0asm` 魔数)都是纯函数,现前置;wabt 门只在输入确是一个真模块后才触发。新增 `test_wasm_validates_the_input_before_the_wabt_gate`:把两个工具强制为未配置(门是活的),断言缺文件报 `not_found`、非模块报 `invalid_params`。带外验证:把 `tool is None` 挪回最前 → 唯独这条新测试失败(缺文件处 `capability_unavailable == not_found`),其余 11 条仍绿。
...
- `web.open` 先跑 `_check_available()`(无 Playwright 即抛 `capability_unavailable`),再跑 `_require_http_url(url)`(非 http/超长即抛 `invalid_params`)。于是在没装 Playwright 的主机上,`web.open(s, "file:///etc/passwd")` 报的是“去装后端”,而真正的错是“改 url”——agent 按 code 路由,两者是截然不同的修法。这与本仓已确立的约定相反:`web.navigate`、`web.click` 等都先校验入参再碰会话,`proxy.start`/`frida.spawn`/`jadx.decompile`/`apk.methods` 也都把畸形入参的 `invalid_params` 放在能力门之前(见上一条 proxy 用例的说明)。`open` 是唯一的例外。改为先 `_require_http_url` + `_bound_nav_timeout`(两者都是纯函数,前置零成本)再 `_check_available`。老测试 `test_web_open_refuses_a_file_url_before_reserving_the_slot` 把 `_check_available` 打桩成空操作,故只证明了 url 校验早于“槽位预留”,对“能力门顺序”视而不见——它的文档声称“即便没装 Playwright 也成立”其实从未被测(且在修复前为假)。新增 `test_web_open_rejects_a_non_web_url_before_the_capability_gate`:不打桩、把 `_available` 强制为 False(模拟无 Playwright、门是活的),断言非 http url 仍报 `invalid_params`。带外验证:把顺序改回“先 `_check_available`” → 唯独这条新测试失败(`capability_unavailable == invalid_params`),含被打桩的兄弟在内的其余 19 条仍绿,精确坐实兄弟测试漏掉的正是这一点。
