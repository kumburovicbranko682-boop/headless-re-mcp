...
Android 捆绑与容器格式（`.aab`/`.apks`/`.xapk`）现在能建会话，而不再以误导性报错失败。`classify_target` 一直把这四种后缀都判为 APK 目标，但 `describe_apk`（会话创建时跑的 stdlib 身份读取）要求 `AndroidManifest.xml` 位于压缩包根目录——只有普通 `.apk` 如此：App Bundle 的清单在 `base/manifest/` 下，`.apks`/`.xapk` 则是内含真实 APK 的容器。于是四种已宣称支持的格式里有三种在会话创建时直接抛「archive has no AndroidManifest.xml」，读起来像文件损坏，把一个完全合法的 Android 制品挡在所有后续工具之外。现在 `describe_apk` 识别四种打包：`apk`（根清单）、`aab`（`BundleConfig.pb` 或 `*/manifest/AndroidManifest.xml`）、`apks`/`xapk`（含 `.apk` 条目的容器，用后缀与 `manifest.json` 区分），并在 `apk` 身份里给出 `format` 标记与尽力而为的事实；`native_abis` 改为匹配任意 `lib/` 段后跟*已知 ABI*（覆盖 bundle 的 `base/lib/…`，又不会把 `assets/lib/<非 ABI>/` 误当架构），容器格式另报 `apk_count`、不谎称其外层带原生库或签名。新增经真实 MCP stdio 服务的捆绑/容器建会话 Gate（`tests/integration/test_apk_bundle_formats_gate.py`）：`.aab`/`.apks`/`.xapk` 各自 `session.create` 成功、`session.get` 回出对应 `format` 与身份，普通 zip 仍如实拒绝——放宽接受面没有变成「任何 zip 都是 APK」。
