...
### 新增（`web.storage.origins` 跨 origin 快照 localStorage）

- 新增 `web.storage.origins`：快照浏览器 context 里**所有已访问 origin** 的 localStorage。
  页面级的存储读取只覆盖顶层 frame 的 origin，而一段登录流程可能把 token 存到另一个
  origin（SSO/IdP 主机、内嵌 widget）下，切回当前页后就再也够不着。本工具走 Playwright
  的 `context.storage_state`，按 origin 汇总整份 context 的 localStorage。每个 origin 是一行，
  带其 localStorage 条目（name/value）、`entry_count`/`entry_total`，单个 origin 条目超过上限时
  置 `entries_truncated`；origin 之间排序、组内按 name 排序，分页稳定。仅限 localStorage——
  sessionStorage 属于单个页面/标签、不在 `storage_state` 内；Cookie 也刻意不带（那是
  `web.cookies`）。值按上限截断并在被截条目上标 `value_truncated`，origin 数封顶 500 并以
  `origins_capped` 如实上报，`offset`/`limit`/`total`/`has_more` 与其余分页读取器一致。
  只读，工具面 265→**266（149 只读 / 117 写）**。
