...
- **`device.logcat` 字符截断会把半行日志当成整行回**。转储超过 200k 字符时按「留最后
  N 字符」切一刀，切口落在行中间，于是最旧那行只剩尾巴、开头被削掉，而 `truncated`
  读起来像「有旧行没了」，不像「你看到的第一行是半行」。现在截断后丢掉到首个换行为止
  的残行，让回包里每一行要么完整、要么没有；整行本身就超过上限、没有换行可切时保留该
  碎片而不是回空。
