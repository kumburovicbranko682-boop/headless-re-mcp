...
### 新增（`apk.method_search` 按名字片段跨类查方法）

- 新增 `apk.method_search`：callers/callees 这对工具的入口。`apk.methods` 需要精确的类名，
  `apk.xrefs` / `apk.callees` 需要精确的方法名，但排查往往从一个片段起步（"decrypt"、
  "token"、"sign"）且不知道它在哪个类。此工具扫描每个非外部方法，保留名字包含 query 的那些
  （大小写不敏感的子串，不是正则），供调用方随后把精确名字喂给 xref 工具。只匹配方法名，不匹配
  类名或描述符。返回 `methods`（class/method/descriptor）、`query`、`count`、`total`、
  `offset`、`has_more`，按 (class, method, descriptor) 排序以保证分页稳定；`total` 为命中数，
  上限 2000，超出置 `scan_capped`。只读，工具面 265→**266（149 只读 / 117 写）**。
