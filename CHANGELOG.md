...
Mach-O 现在是一等目标类型，不再被当作"打不开的 PE"。`classify_target` 过去把所有非 PE/APK/Web 的文件都归为 PE，`session.create` 随即抛「not a PE file」——于是 macOS/iOS 的原生镜像（radare2 与 Ghidra 正为这类原生二进制而设）连会话都开不了。新增 `TargetKind.MACHO`：`classify_target` 识别单架构（thin，MH_MAGIC/MH_CIGAM 两种字节序、32/64 位）与胖/通用（fat）镜像；`create_session` 走 `detect_macho_architecture`——thin 镜像按 magic 的字节序读 `cputype`，`CPU_TYPE_X86`/`X86_64` 映射为 x86/x64，arm64/ppc 等本工具不建模的机型与 fat（多架构）一律返回 `None` 而非抛错。**关键：fat 魔数 `0xCAFEBABE` 同时是 Java class 文件的魔数**；胖头把架构数放在其后的大端字，而 Java 把 `major_version` 放在同样的字节、其值恒 ≥ 45（Java 1.1），因此只在该字 `< 45` 时才认作 Mach-O fat，Java `.class`（含带 preview 次版本的）绝不会被误判为 Mach-O。路由随之正确：PE 专用后端经 `require_pe` 对 Mach-O 报 `target_mismatch`，radare2/Ghidra 经 `require_binary` 能拿到它（缺 CLI 时 `capability_unavailable`，绝非 `target_mismatch`）。新增经真实 MCP stdio 服务的 Mach-O 目标 Gate（`tests/integration/test_macho_target_gate.py`）：thin x86-64 镜像经 `session.create` 得 `target=macho` 与 `x64`、`session.get` 回出同样事实、`static.open` 报 `target_mismatch`、`r2.open` 走到可用性检查。
