...
### 新增（`r2.symbols` 列出 radare2 的完整符号表）

- 新增只读工具 `r2.symbols`：白名单放行 `isj`，服务层跑 `isj`，经既有 `enrich_r2_payload`
  统一映射。这是完整的 `.symtab`/`.dynsym`，不同于 `r2.imports`（导入表 `iij`）、
  `r2.exports`（导出表 `iEj`）与 `r2.functions`（`aflj` 分析发现的函数）——它把未导出的
  本地函数与数据符号也点名，正是「`sub_1234` 还是真名」的分野，对从 APK 里掏出的 native
  `.so` 逆向尤其有用。回 `items`，每项含 `name`、`type`（FUNC/OBJECT/…）、`bind`
  （LOCAL/GLOBAL/WEAK）、`size`、`vaddr` 与 `address`（va/rva/module），外加 `count`；没有整数
  地址字段。满 4096 上限时给出 `items_truncated`、`items_total`、`items_limit`，不会把「截到上限」
  读成「表到此为止」；没有 `symbols`、`truncated` 或 `has_more` 字段。`r2.*` 只读线新增一个符号表工具。
