...
### 修复（切换会话绑定失败后界面仍指着没绑上的会话）

- `webui` 侧栏点会话切换绑定时，`useWorkbench.changeSession()` 先乐观地 `setSessionId(next)`、
  清掉「样本已断开」状态，再发 PATCH 把对话绑到新会话。PATCH 失败（后端正在重启、对话被并发
  删除返回 404、网络断）时 `catch` 只弹了错误横幅，乐观值原样留下：顶栏样本标签、检查器的
  监控/时间线/产物面板全都盯着 `next`，而服务端的对话绑定没动——助手的会话级工具继续操作的
  是**旧**会话。用户看着 B 的监控画面，让助手「打个断点」，断点落在 A 上。此外若切换前正显示
  「样本已断开」卡片（旧会话丢失），这个状态也被提前清掉、失败后不恢复，重连入口就此消失。
  现失败时把 `sessionId` 回滚到切换前的值（用函数式更新，用户已再点了更新的选择时不覆盖）、
  恢复之前的断开卡片（期间没有新的丢失事件时），错误横幅照旧。
- 新增三条 `useWorkbench` hook 直测：PATCH 拒绝时绑定显示回滚到旧会话且横幅给出原因；PATCH
  成功时保持新会话（守住快乐路径）；从「已断开」样本切走失败时断开卡片连同旧绑定一起恢复。
  修复前两条失败用例都停在 `sessionId === "sess-b"`，正对应线上「界面指错会话」的表现。前端
  66 条测试与 `tsc --noEmit` 均通过，提交的 SPA 产物已重建。
