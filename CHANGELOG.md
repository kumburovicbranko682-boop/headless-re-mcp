...
Web 会话现在能看见页面打开的 WebSocket 及其帧。CDP 里 WebSocket 走 `Network.webSocketCreated` 而不是 `requestWillBeSent`，捕获此前一个都没接：连接在 `web.network.list` 里完全不存在，逐帧推送的 `webSocketFrameSent/Received` 也全部丢弃——而这些帧恰恰是逆向 Web 应用时要读的应用层协议。现在连接以 `resourceType=WebSocket`、`status=101` 的行入列，带 `websocket=true`、`ws_messages`（数据帧数，控制帧不计）与 `ws_bytes`（解码后字节数，permessage-deflate 之后）；`web.network.get` 对该行不再去调必然失败的 `Network.getResponseBody`，而是返回按序保留的帧（`from_client`+`size`，短 UTF-8 帧含 `text`，二进制/超长帧以 `omitted` 标注），并给出真实总数 `websocket_message_count` 与 `websocket_truncated`。帧由全局有界环持有（跨连接共 2000 帧、单帧 8KB），高频 socket 不会撑爆会话内存。`web.har.export` 现在也把 socket 的帧随导出带走：WebSocket 以条目形式写入 HAR，其保留帧作为 Chrome DevTools 的 `_webSocketMessages` 扩展数组（每条 `{type: send/receive, opcode, data}`）附在该条目上，而不再只是一个孤零零的 101 握手——分析者把 HAR 交给别的查看器时不会再丢掉 WebSocket 会话内容。socket 关闭现在也被记录：新接 `Network.webSocketClosed`，行上多出 `ws_closed=true`，被服务端踢掉的连接不再看起来仍在传输（CDP 不带关闭码，代理侧 mitmproxy 抓包才有；这里如实只给旗标）。追查该关闭旗标偶发不出现时挖出一个更深的坑：sync Playwright 只在某个调用泵其 greenlet 循环时才派发排队的 CDP 事件，而 `web.network.list` / `web.network.get`（WS 行）/ `web.console` / `web.scripts` / `web.har.export` 只读 Python 字典、从不触碰 Playwright——于是最后一次 Playwright 调用之后到达的一切（迟到的响应、WebSocket 帧、关闭事件）永远停在传输层不被派发，轮询 list 的调用者看到的是冻结在上次调用时刻的视图（实测：帧到了 handler、关闭事件 10 秒都不来；换用原生 CDP 轮询则 5/5 全到）。修法：runner 新增 `try_call`（尽力而为，慢了不将会话标记为 wedged——正在长导航的 runner 本来就在泵，排队的泵稍后执行即可），上述读路径读取前先在 runner 线程上等 1ms 把积压事件全部派发。live gate 现在让页面在收到回显后主动 close，并单独轮询断言 `ws_closed` 落到行上——修前约半数运行 20 秒等不到关闭事件，修后 6/6 稳定通过。另修二进制帧死角：此前短二进制帧一律报 `omitted: "binary"`、把负载整个丢掉，而二进制 WebSocket 协议（protobuf、msgpack）恰是逆向最要读的，且 CDP 的 `webSocketFrame*` 事件本就把二进制 `payloadData` 以 base64 送来。现改为短二进制帧（解码后不超过每帧 8 KiB）以 `base64` 字段带回真实字节（超限的才仍报 `omitted: "too_large"`），`web.har.export` 里对应帧按 Chrome DevTools 的 `_webSocketMessages` 格式写成 opcode 2、`data` 为该 base64——这本就是 Chrome 自己对二进制帧的表示，比先前写空串更贴合格式。CI 新增 `linux-web-websocket` gate：真实 Chromium 打开与 websockets 回显源通信的页面，断言双向文本帧在 `web.network.list`、`web.network.get` 与导出的 HAR 里都被捕获、页面主动关闭后 `ws_closed` 落到行上、且额外互发的一条非 UTF-8 二进制帧经 `web.network.get` 与 HAR 都能按 base64 完整取回（skip ≠ pass）。再补上逐帧时间：CDP 的 Network 事件时间戳是单调时钟（`timestamp`），只有 `requestWillBeSent` 额外带 `wallTime`（Unix 墙钟）；此前整段丢弃、不给 `time`。现从第一条同时带 `timestamp` 与 `wallTime` 的请求学到"单调→墙钟"的偏移（`wallTime - timestamp`，会话内稳定），再用它把每条 WebSocket 帧的单调时间戳换算成墙钟 `time`——正是 Chrome DevTools 给自己 HAR 帧打时间的做法。`web.network.get` 与导出 HAR 的每帧现都带 `time`，让分析者能读心跳间隔与请求/响应延迟；偏移学不到时（socket 之前没有任何请求）则如实不给 `time`，绝不伪造。单测覆盖偏移换算（墙钟 1_700_000_000@单调 100.0 定标，帧在单调 101.5 即 1_700_000_001.5）、无偏移时不带 `time`、以及 `time` 透传进 HAR；`linux-web-websocket` gate 额外断言所有帧带正向递增、落在合理纪元的墙钟 `time`。
