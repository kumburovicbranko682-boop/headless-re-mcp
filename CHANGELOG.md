...
纯 Python 的会话账本面（`artifacts.list/describe/read/gc` + `timeline.list` + `audit.list`）新增端到端 Gate（`tests/integration/test_session_ledger_gate.py`）。产品的每次捕获——报告、解包产物、HAR、进程 dump——都登记为制品,这三组读面就是 Agent 不借 shell 检视自身历史的唯一入口,此前却没有独立守护。六条用例以 `report.generate` 产出真实文件后对着提交的 PE 夹具跑:`describe` 的 size/sha256 必须与磁盘字节逐一吻合;`artifacts.read` 全量读回与原文件逐字节相等、两窗分页可精确重组、limit 钳到至少 1 字节、越过 EOF 诚实返回空数据与真实 size;`artifacts.gc` 在 1 字节预算下按最旧先删、但**永不回收最新制品**（刚产出的路径调用方还握着）,删除后 `describe` 转 `not_found`、文件确实消失,宽裕预算则为无操作;时间线记录 `session.created`/`session.closed`（含 `ok` 明细）并按 offset/limit 走同一文件分页,未知会话答 `session_not_found`;审计记录 `session.create`/`session.close`（最新在前）,按会话过滤不泄漏邻居、全局视图跨会话可见,并钉住与时间线的语义不对称——审计是全局日志的过滤器,未知 id 是空页不是错误;守护面上未知制品 id 的 `describe`/`read` 答 `not_found`,GC 预算为 0 或 bool 与路径穿越型会话 id（`../escape`）各以 `invalid_request` 关闭失败。测试期无需任何工具链,故此 Gate 恒跑不 skip。
