...
### 新增（`proxy.hosts` 按主机聚合抓包，看清目标的网络足迹）

- 新增只读工具 `proxy.hosts`：把仍保留在环形缓冲里的抓包流按 host 聚合成一行一主机，
  让分析者不必翻整份流日志就能看清目标在跟哪些后端说话。每行含 `host`、`flows`（去过多少次）、
  `errors`（从未完成、status 为 null 的流）、`response_bytes`（解码后响应体字节之和，即下载足迹），
  以及见过的 distinct `methods` 与 `statuses`。只答复失败的主机会是 `errors` 有值而 `statuses`
  里没有 2xx，于是不会被读成「提供过内容」；已完成流才贡献其 status。回 `hosts`、`count`、`total`、
  `offset`、`has_more`，按 `flows` 降序、host 名作稳定次键排序，翻页确定。它是对 `proxy.flows`
  所翻同一环形缓冲的聚合——只反映仍保留的流，不是整个会话历史。聚合器是对 `recorder.snapshot()`
  的纯函数，可脱离真实代理钉死。抓包线 `proxy.*` 工具随之 8 → 9。
...
- **抓包**：`proxy.*` 9 个工具，mitmproxy 以 addon 形式跑在独立线程，Web 与 Android 共用，
  含 `proxy.ca.install_android` 与按主机聚合的 `proxy.hosts`。
