...
### 新增（`apk.class_search` 按名字片段查类）

- 新增 `apk.class_search`：`apk.method_search` 的同伴。`apk.classes` 列出整张类表，按一个片段
  （"Login"、"Crypto"、"okhttp"）定位某个类就得把整表翻一遍再 grep。此工具扫描同一张表，只保留
  名字包含 query 的类（对 smali 名 `Lcom/example/Foo;` 做大小写不敏感子串匹配，不是正则），
  供调用方把精确名字喂给 `apk.methods`。返回 `classes`、`query`、`count`、`total`、`offset`、
  `has_more`，按名排序以保证分页稳定；`total` 为命中数，上限 10000，超出置 `scan_capped`。与
  `apk.classes` 一致排除外部（框架）类。只读，工具面 265→**266（149 只读 / 117 写）**。
