...
- `js.secrets`：扫描 JS（或任意文本）文件里的硬编码凭据，纯 Python、只读、不需要 Node。它是
  `apk.secrets` 扫 DEX 字符串池在前端侧的对应物——面向下载/压缩过的 bundle，那是活的 API key
  最常泄漏的地方之一。只匹配一组知名且高精度的服务商格式（AWS access key id、Google API key 与
  OAuth client id、Slack/GitHub/Stripe token、JWT、PEM 私钥头），因此命中几乎不会误报，`type`
  直接说明泄漏了什么；不使用熵启发（自定义/通用 key 会漏），也从不启动 webcrack。返回
  `findings`（每条含 `type` 与截断后的 `value`）、`count/total/offset/has_more`，`total` 上限
  2000、越限置 `scan_capped`；输入超过 16 MiB 按 `too_large` 拒绝。
