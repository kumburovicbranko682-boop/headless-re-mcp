...
- `apk.hashes`（只读）：给样本一个稳定身份——APK 文件的 sha256/sha1/md5 与大小（VirusTotal 与多数
  样本库据以索引的键），外加每个 DEX 的 sha256 与大小，便于把重打包了同一份代码的两个构建对上。
  md5/sha1 只用于检索，不作安全用途。文件按块流式哈希，大 APK 不会整体读进内存；DEX 列表封顶 64
  并回带 `has_more`，重度 multidex 的应用不会被当成已完整枚举。回带 `file`（sha256、sha1、md5、
  size）、`dexes`（每条 name、sha256、size）、`dex_count` 与 `has_more`；整包摘要在 `file` 下，没有
  顶层 hash/digest 字段。
