...
- `device.mounts`（只读）：解析 `/proc/mounts`，列出设备上已挂载的文件系统——挂在哪里、什么类型、
  带哪些选项，供逆向查看 `/system` 是否只读、外部存储在何处、哪些挂载点带 `noexec`/`nosuid`。
  `/proc/mounts` 全局可读，无需 root。每条含 `device`、`mountpoint`、`fstype`、完整 `options` 列表与
  便捷的 `readonly` 标志；路径里的八进制转义（如空格 `\040`）会被还原，避免把 `/mnt/My Card` 误报成
  `/mnt/My\040Card`。回带 `mounts`、`count`、`has_more`，封顶 1000；装满上限的一次读取不会被当成全部
  挂载，而读取结果为空（活设备不可能没有挂载）会当作错误而非空列表。
