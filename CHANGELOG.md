...
### 新增（`apk.native_methods` 列出 DEX 的 native(JNI) 方法）

- 新增只读工具 `apk.native_methods`：扫描整个 DEX、挑出带 `native` 访问标志的方法，即 Java 层
  与 `.so` 原生库之间的 JNI 边界——正是拿到 `apk.native_libs` 列出的原生库后要去里面找的入口点。
  回 `methods`（`class` 为 `Lsmali/;` 形式、`method`、`descriptor`）、`count`、`total`、`offset`、
  `has_more`，满页不会被误读成全量；`total` 为收集数、上限 5000，超过时 `scan_capped=true`。
  `methods` 为空且 `total=0` 表示该应用未声明任何 native 方法，而非扫描失败。native 判定优先取
  androguard 的访问标志字符串（`ACCESS_FLAGS[0x100] -> "native"`），取不到时回落到底层
  `EncodedMethod` 的整型标志；单个畸形方法的取值失败按「非 native」处理，不会中断整轮扫描。
  Android 静态 `apk.*` 工具随之 12 → 13。
...
- **静态**：`apk.*` 13 个工具，androguard 进程内解析 manifest/权限/证书/组件/DEX 类与方法/
  native(JNI) 方法/字符串/xrefs，jadx CLI 负责 `apk.decompile` 与 `apk.export_sources`。
