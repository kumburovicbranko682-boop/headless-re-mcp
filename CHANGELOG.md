...
- **线程消息只读有上限、写入没有**。80 条 × 50_000 字符把 `agent.db` 堆到 4.1 MiB，
  ...
  窗口硬顶对齐）。
- **XVLKC 探针超时只杀启动器**。包装脚本起 sleeper 时 0.40 秒返回失败，子进程仍是 S，
  下一次脱壳在探针的孩子还占着机器时就开始。现在走有界执行。
- **de4dot 探针把挂起报成可用**。三个 argv 各超时 0.4 秒后仍返回 True 且空文本，并留下
...
- **`apk.classes` 停在上限却不说还有**。250 个类、limit=100 时有 total 没有
  `has_more`。现在带上。
