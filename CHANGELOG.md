...
- 新增回归（`apk.strings` 的去重/长度上限/唯一计数几处被同质夹具照不到）。DEX 字符串池并不是
  一份整洁的短唯一值清单：同一字面量被引用成千上万次，池里也可能塞进几 MiB 的 blob。`strings`
  把它折叠成有界、稳定的一页：`seen` 是 `set`（去重）、`seen.add(str(v)[:_MAX_STRING_LEN])`（先按
  2000 字符截断再去重）、`if len(seen) >= _MAX_STRINGS_COLLECT`（上限按**唯一**条数计）、
  `sorted(seen)`（稳定分页序）。既有 `strings` 测试喂的是 25 条本就互异的短值（`s0`..`s24`），于是
  这些行为全 inert——无重复则 `set` 与 `list` 无从区分、无超长值可截、5000 上限够不着、`s0`..`s24`
  也近乎有序。新增回归：重复值折叠为一条且 `total` 记唯一数；超长值先截到 2000 再去重（共享 2000
  前缀的两条因此并成一条，存值恰为 2000 而非原 2003）；采集上限按唯一计（同一值刷 100 次不触发、
  cap=3 时 5 条互异才触发 `scan_capped`）；页面有序；分页 total 按唯一值算（五次出现的两条按两条翻页）。
  两处变异——去掉 `[:_MAX_STRING_LEN]` 截断、把去重 `set` 换成 `list`——各由相应测试捕获，既有
  `apk.strings` 字段/分页测试仍全绿。
