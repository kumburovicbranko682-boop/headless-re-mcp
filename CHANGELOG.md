...
- **`apk.native_methods`**：列出所有声明为 `native` 的方法，即 JNI 入口点。`apk.native_libs`
  给出随包的 `.so` 文件，本工具给出边界的 Java 一侧——每个访问标志含 `native`、方法体因而在
  native 代码里、经 JNI 到达的方法；两者对读即可定位托管代码在何处交给随包的原生库。每行是
  `class_name`、`name`、`descriptor`、`access`。external（仅被引用、非本 DEX 定义）方法跳过，
  因为只有本地定义的方法才有 native 实现。结果按（类、名、描述符）去重排序，`total` 为收集数、
  封顶 2000（超出置 `scan_capped`），`offset`/`has_more` 分页。只读工具。
