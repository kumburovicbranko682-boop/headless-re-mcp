...
### 新增（`apk.deep_links` 枚举 URI 入口）

- 新增 `apk.deep_links`：列出应用注册的 URI 入口（VIEW intent-filter）。深链是网页或别的应用把
  URL 交给本应用的通道，其暴露的 scheme/host/path 是首看攻击面。扫描所有组件里带 VIEW action
  且至少有一个 `<data>` scheme 的 intent-filter，每个 filter 一行：`type`、`name`、`class`
  （按包名解析）、`browsable`（含 BROWSABLE 类别，网页而非仅别的应用即可触发）、`auto_verify`
  （`android:autoVerify`，即对 host 校验过的 App Link）、以及该 filter 的 `schemes`、`hosts`、
  `paths`（path / pathPrefix / pathPattern / pathSuffix 合并）、`mime_types`、`uris`
  （scheme×host 叉积 `scheme://host`，作标识速览，非穷举 URL）。内层列表去重排序并按上限截断，
  置 `values_truncated`。直接解析清单，无需 DEX 分析。返回 `deep_links`、`package`、`count`、
  `total`、`offset`、`has_more`，按 (type, class) 排序；`total` 为 filter 数，上限 256，超出
  置 `scan_capped`。只读，工具面 265→**266（149 只读 / 117 写）**。
