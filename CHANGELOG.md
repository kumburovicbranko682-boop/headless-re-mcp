...
- `max_heap` 被拼进 `JAVA_TOOL_OPTIONS`（`-Xmx<max_heap>`），是请求自身的属性，但它的正则校验一直只在
  ...
  能力门、再**调到 `_run_headless`。于是在**客户端 API 这一层**直接调用时，没配 Ghidra 的机器上一个
  `2G -XX:OnOutOfMemoryError=touch …` 之类的恶意 `max_heap` 得到的是 `capability_unavailable` 而非
  `invalid_params`：同一个坏输入，配了 Ghidra 的机器判 `invalid_params`、没配的机器判
  `capability_unavailable`，两地不一致。需要说清楚的是：`max_heap` 当前只存在于客户端签名、默认恒为
  `2G`，服务层与工具面都没有把它接出来，所以这条**目前并非经 MCP 工具面可达的缺陷**——但 CHANGELOG 里
  `frida.server.ensure` 那条早已把"Ghidra 的 max_heap"列为"输入先于门"的同类处理，代码却没真正做到，这是
  文档与实现的名实不符；注入本身也从未被放过（正则总在 spawn 前跑，且只有能力门通过才会 spawn）。现抽出
  `_validate_max_heap`，在 `analyze_binary` 与 `_export`（覆盖 functions/symbols/xrefs/decompile 四种模式）
  进门之前先校验，`_run_headless` 处保留同一校验作为贴着 env 拼接点的纵深防御——将来一旦工具面把堆大小接给
  调用方，其排序已按构造正确。与 `proxy.start` 的 port、`frida.server.ensure` 的 port、`java_enumerate` 的
  mode/class_name、`apk.methods/xrefs` 的必填名同属一致处理。回归在"没装 Ghidra"的客户端上钉住：analyze 与
  四种导出模式对恶意/空 `max_heap` 都在能力门之前回 `invalid_params`，且从不 spawn analyzeHeadless。纯排序
  修复，返回结构不变。
...
- 新增只读工具 `web.cookies`：列出 Web 会话浏览器上下文当前持有的 cookie。做 Web 逆向时
  认证/会话 token 常以 cookie 形式存在，此前的 12 个 `web.*` 工具只能从 `web.network.get`
  的响应头里碰运气翻 `Set-Cookie`，拿不到上下文里的最终状态（含 JS 写入与跨请求累积）。
  返回 `name/value/domain/path/expires/http_only/secure/same_site`，字段名按本仓库惯例用
  下划线而非 Playwright 的驼峰。页面可以塞任意多、任意大的 cookie，所以列表封顶 1000 条、
