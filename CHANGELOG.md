...
Web 目标身份与浏览器工具降级新增经真实 MCP stdio 服务的端到端 Gate（`tests/integration/test_web_target_identity_gate.py`），是 APK 身份 gate 的 Web 线对称物。Web 线许下与 Android 线相同的承诺、也曾有相同的缺口:目标必须在没有浏览器驱动的机器上**打得开**,而真正需要浏览器的工具要精确说明、而非崩溃或假装。既有 target-triage gate 只用服务直连证了 URL 会话能开,没有在真实 MCP 上覆盖资产会话与浏览器工具降级。本 gate 证四条:下载的 `.js` 与 `.wasm` 各开成绑定磁盘字节且带 sha256 的 `web` 会话、http(s) URL 开成无 binary 且保留 URL 为 locator 的浏览器无关会话;`web.open`(启动浏览器)在 Playwright 驱动缺席时降级为 `capability_unavailable`(装了驱动则允许启动,故只在能力缺席处断言该码);未开浏览器就调 `web.navigate`/`web.screenshot` 答 `invalid_state`——诚实的次序错误,而非 `target_mismatch`、更非崩溃;跨后端守卫依旧成立——web 会话不是 Android 目标,故 `apk.open` 拒为 `target_mismatch` 而不去够反编译器。纯 Python、stdio 回环、任意平台。
