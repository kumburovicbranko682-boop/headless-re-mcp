...
Agent 控制面新增经真实 HTTP+SSE 的端到端 Gate（`tests/integration/test_agent_http_sse_gate.py`）。工作台故事立足于四个此前只有单测守护的 HTTP 主张:`POST /api/agent/runs` 起的 run 真会驱动 provider 循环;`GET /api/agent/runs/{id}/events` 以 SSE 流出 run 的全程;LLM 提议的写工具在授权允许时对真实 `AnalysisService` 分发(留下可审计的 `approval.auto` 事件);`request` 模式下同一调用停在 `approval.required` 直到有人经 HTTP 批准或拒绝。三条用例拉起真实 `serve-web` 子进程(隔离配置家目录、预置 token),provider 指向本地假 OpenAI 服务器(讲 chat-completions SSE 方言:首轮对提交在库的 PE 夹具提议 `session.create`,对话里出现 `tool` 角色消息后答终稿):全程 SSE 帧序列钉住 tool.proposed→approval.auto→tool.started→tool.completed→run.completed,分发是真的(`/api/sessions` 出现恰为该夹具的分析会话,线程留下 user/tool/assistant 完整往来);request 模式停批后,错 digest 先被 409 拒绝、对 digest 批准则 run 完成(approval.approved 且无 approval.auto),新线程再停批后拒绝则 run 落 `rejected` 且无 tool.started;provider 控制面列表绝不回吐已存的 api_key、模型探针对假服务器返回真实列表、死端点以可诊断的 `provider_probe_failed:` 502 关闭失败、匿名访问 401。无真实 LLM、仅回环网络、纯 Python。
