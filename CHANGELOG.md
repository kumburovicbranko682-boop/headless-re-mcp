...
- **`apk.interfaces`**：把全 App 实现的接口做成计数普查——`implements` 关系的聚合视图。
  `apk.subclasses` 回答「谁实现了这一个接口」，本工具则列出被内部类实现的每个接口连同实现它的类数，
  勾勒 App 的回调与契约面：`Ljava/lang/Runnable;`、某监听器或自定义接口被用得有多广。每行 `interface`
  （smali 名）与 `implementor_count`，external 类跳过（接口名本身可能是框架或内部）。返回 `interfaces`、
  `count`、`total`（不同接口数）、`classes_scanned`（examined 类数、封顶 10000、超出置 `scan_capped`）、
  `offset`、`has_more`；按 `implementor_count` 降序、同数再按接口名排序，最常用者在前。只读工具。
