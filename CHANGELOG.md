...
- **停在上限的列表看起来和"到此为止"完全一样**。四处：r2 载荷最多保留 4096 个条目、
  ...
  如实披露，这几处只是补齐同一条规矩。
- **`apk.xrefs` 只封了第一屏调用点，后面的够不着**。上一条给它补了 `has_more`，但它仍只保留
  按图遍历顺序的前 `limit` 个调用者、没有 `offset`：一个被调用次数超过一页的方法（常见的加解密
  函数就是），第一页之后的调用者永远翻不到，也数不出到底有多少。现在和
  `apk.classes`/`methods`/`strings` 一样是分页读取——采集上限 5000 个调用点、按（类, 方法）
  排序后按 `offset`/`limit` 切片，回 `total`/`offset`/`scan_capped`。`has_more` 只表示还有已
  采集的行，`scan_capped` 表示采集本身停在上限、上游可能还有更多调用者。
