...
### 修复（`device.logcat` 不再把半行日志当整行交回、也不再让调用方数行数）

- `device.logcat` 用设备侧 `-t N` 限行,再叠一道 20 万字符上限防单行过长撑爆内存。字符上限触发时
  代码保留**最新**的那批字符——而这一刀正好落在最老一条存活日志的中间,于是回包里的第一行是半截
  日志。无人值守的 agent 把这半行当作完整条目去解析(比如按列切标签/PID),就会读进一条损坏记录。
- 现在字符截断后先丢掉这条残缺的首行(截断点后的第一个换行即那半行的结尾),保证交回的每一行都是完整的;
  `truncated` 仍为真以示有丢失。另新增 `count`(实际返回的行数),调用方不必再从 `lines` 长度反推,
  也不会把「设备日志本就不足 N 行」误当成被截断。
- 新增回归:普通取用报 `count` 且各行完整、`count` 恒等于 `lines` 长度、字符截断丢掉半截首行只留整行、
  以及无换行的单条超长行仍保留(照旧标 `truncated`)。`device.logcat` 描述点名 `count` 与首行处理。
