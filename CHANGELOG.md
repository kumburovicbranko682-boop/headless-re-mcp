...
### 测试（`classify_target` 扩展名优先于魔数的路由合同）

- `core/session.py` 的 `classify_target` 明确规定“扩展名优先，其次才对没有扩展名的文件嗅探魔数”——
  扩展名是调用方声明的意图，胜过文件实际内容。但既有分类测试的夹具扩展名与内容总是一致(或内容没有
  可识别魔数),因此两种信号之间的**优先级**从未被检验:把魔数嗅探挪到扩展名判断之前、或去掉扩展名短路
  让每个路径都被打开,现有用例仍全绿,却会悄悄把伪装/错标输入路由到错误的后端线(PE / APK / WEB)。
  新增 `tests/unit/test_classify_target_extension_precedence.py`:给每个夹具一个与字节不符的扩展名,并
  用磁盘上不存在的路径逼迫扩展名分支在任何读取前定线——`.js/.mjs/.cjs/.wasm/.html/.htm/.har` 内容为 PE
  仍判 WEB、真 APK 改名 `.wasm` 仍判 WEB、`.apk/.aab/.apks/.xapk` 内容为 wasm 或 PE 仍判 APK、不存在的
  `.apk`/`.js` 无需读盘即定线、无扩展名的缺失文件按文档回退 PE。变异验证:去掉扩展名短路使分类退化为纯
  魔数后 16 例中 15 例失败(唯一存活的是无扩展名回退 PE 的对照)。仅测试与 CHANGELOG,未改动产品代码。
