...
- **`doctor` 的 radare2 探针只看 PATH，无视配置的 `HEADLESS_RE_R2`**。它用的是只查
  ...
  改用 `probe_optional_tool("radare2", …, "r2", ("r2","rizin","radare2"))`，与 adb / jadx /
  apktool / webcrack / wabt 一致：先认配置路径，再回落 PATH。
- **`Settings.load` 与 `doctor` 探针都漏了 `radare2` 这个二进制名**。`R2Client._discover` 认
  `("r2", "rizin", "radare2")` 三个名字,但加载器只按 `r2`→`rizin` 顺序 `shutil.which`,
  doctor 的 radare2 探针候选也只有 `("r2","rizin")`。于是只装了全名 `radare2`（PATH 上没有
  `r2`/`rizin` 别名)的机器把 `settings.r2` 留成 None、doctor 还报 radare2 缺失——客户端
  `_discover` 仍能发现它、工具照跑,可配置层与 doctor 都在“什么名字算 radare2”上与客户端不
  一致（一个潜在维护陷阱、一个 doctor 假阴性）。默认值补上 `shutil.which("radare2")`、doctor
  探针候选补上 `radare2`,三层对齐；新增回归用只有 `radare2` 的 PATH 断言 `Settings.load`
  会把 `settings.r2` 填上。
