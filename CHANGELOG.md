...
### 新增（`ghidra.strings` 列出 Ghidra 定义的字符串）

- 新增只读工具 `ghidra.strings`：跑 `analyzeHeadless` 的 `ExportJson.py` 新增 `strings` 模式，
  用 `DefinedDataIterator.definedStrings` 把 Ghidra 分析中被定义为字符串数据的项列出来——与
  `r2.strings` 互为佐证，出自不同引擎，对 APK 里掏出的 `.so`（及 WASM）native 分析尤其有用。
  回 `items`，每项含 `address`、`value`（截断到 2048 字符）与 `length`（该数据的字节长度），
  外加 `count` 与 `has_more`，满页不会被读成全量。导出失败是错误信封，而非「这个二进制没有
  字符串」的空列表——沿用既有「非零退出但已写出内容视为成功、空内容加非零退出才是失败」的判定。
  字符串取值用 `u"%s"` 强转，跨 Jython 2 与 PyGhidra 3 都不会因非 ASCII 而在扫描中途炸掉。
  `ghidra.*` 静态线新增一个只读字符串工具。
