...
APK 纯 stdlib 身份识别新增经真实 MCP stdio 服务的端到端 Gate（`tests/integration/test_apk_identity_gate.py`）。Android 线立在一条参考机(装了 androguard)从没测过的承诺上:一个包必须在裸机上**打得开**、带上 triage 需要的身份事实(native ABI、dex 数、v1 签名),仅用标准库读出,并在 androguard 缺席时降级为"开了但不能反编译"而非拒掉整个目标——若会话创建需要 androguard,一个可选依赖的缺失就会拖垮整个 Android 面而非仅一个工具。Gate 用测试内构造的合成包(无夹具、无外部工具)在真实 MCP 上证三条:格式良好的 APK 开成 `apk` 会话且元数据带准确的 stdlib 身份(ABI 取自 `lib/<abi>/`、`.dex` 计数、`META-INF` 的 v1 签名标志);识别靠**内容**而非文件名——同样的字节去掉 `.apk` 扩展名仍开成 APK(含 `AndroidManifest.xml` 的 ZIP 就是 APK),而无 manifest 的纯 ZIP 按其回退到的非 PE 被拒(manifest 是决定性事实);androguard 缺席时 `apk.open`(其反编译器支撑的身份解析)答 `capability_unavailable` 而非 `target_mismatch`、更非崩溃——目标已被理解,只是可选工具缺席。纯 Python、stdio 回环、任意平台。
