...
### 新增（`web.cookies`：读浏览器 cookie 罐,含 document.cookie 看不到的 httpOnly 会话令牌)

- Web 工具面能看网络、控制台、脚本、DOM、HAR,但**没有办法读 cookie**——会话/鉴权令牌、跟踪标识这些
  Web 逆向最关心的状态。而工具刻意不开放任意 JS 求值(`document.cookie` 那条路本就走不通),且 `document.cookie`
  连 **httpOnly** 的会话 cookie 都看不见(那恰恰是最要害的一类)。Playwright 的 `context.cookies()` 是有界只读
  接口、能拿到 httpOnly,正好补上这个能力又不破坏"无任意求值"的边界。
- 新增只读工具 `web.cookies`(工具面 265 → 266,读 148 → 149):在浏览器 runner 线程上读 cookie 罐,按
  (domain, path, name) 排序保证分页稳定,每条回 name/value/domain/path/expires/http_only/secure/same_site。
  分页(offset/limit,上限 500,源头钳制),回 cookies/count/total/offset/has_more;单条 value 在每-cookie 上限
  (4 KiB,约浏览器单 cookie 上限)处截断并标 `value_truncated`,一条畸形 cookie 撑不爆返回。不含 localStorage
  (描述点明),不改变现有工具。
- 三层接线齐全(client → `service_web.web_cookies` → MCP 工具 + catalog 只读声明 + MCP 面期望清单),README 与
  catalog 计数守卫同步。新增回归:httpOnly+secure 会话 cookie 如实透出、描述点名 http_only;jar 按
  (domain,path,name) 排序分页且 has_more 如实;超长 value 截断并标 `value_truncated` 而非丢弃。
