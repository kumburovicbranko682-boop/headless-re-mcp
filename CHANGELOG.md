...
- **`proxy.export_har` 现在像同族的 `web.har_export` 一样对输出定界并回报 `size` / `truncated`**。
  抓包环形缓冲已限制流条数与 URL 长度,所以当前导出落在采集上限之内;但那条上限是**这次写入
  自身**的性质,不该只是环形缓冲限额的巧合——一旦后者被调高,无界导出会静默写出超过采集上限的
  文件,而 web 一侧会拒绝。现按与 `web.har_export` 相同的方向与步长丢弃最新的流摘要直到装得下,
  仍超限(连空骨架都放不下)时报 `too_large` 而非硬写。这也补齐了它是非 PE 各线里唯一一个既不定界
  输出、又不回报体积的落盘导出:其它落盘工具(`web.screenshot`/`adb.pull`/`proxy.flow.get` 等)
  都回报 `size`。新增回归测试覆盖装得下、超限丢弃、以及连空骨架都超限被拒三条路径。
