...
- **`report.generate` 里发现值的摘要只显示前 4 个字段，多出来的悄悄丢掉**。渲染器为了让表格
  一行读得下，只取 value 字典的前 4 个 `key=value`（`_summarize_value`）。于是一条记了 6 个字段
  的发现，在这份对外留存、别人据以行动的报告里读起来就像只有 4 个字段——和「Recent actions」段
  被截断却不声明是同一类静默省略，只是下沉了一层。现在超过上限时补一句 `(+N more)`，未超出则不
  加任何标记；单字段的长值仍照旧由单元格的 `…` 表示被裁。
