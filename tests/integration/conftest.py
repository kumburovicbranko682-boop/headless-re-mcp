...
def elf_fixture(tmp_path_factory: pytest.TempPathFactory) -> Path:
    ...
    than binaries, so the portable static gates (radare2 today, the Ghidra Linux
    gate here) have no ELF to analyse out of the box. Building it here keeps
    "skip != pass" honest: a skip means there is no compiler, not that the
