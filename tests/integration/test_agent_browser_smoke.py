...
def test_browser_agent_workbench_smoke(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    try:
        with sync_playwright() as playwright:
            # Launch the browser Playwright ships, exactly as WebBackend does, so
            # this gate runs on any platform that ran `playwright install
            # chromium` rather than only a Windows box with Chrome at a fixed
            # path. A missing browser skips honestly (skip != pass).
            try:
            ...
            page.goto(f"http://127.0.0.1:{port}/?token={token}")
            # The reworked workbench opens on its empty conversation state.
            expect(page.get_by_role("heading", name="有什么想分析的？")).to_be_visible()
            assert "token=" not in page.url

            # The default autonomy auto-approves state_change effects for
            # unattended packed-PE work, which would run the dangerous tool below
            # without prompting. Explicitly select "request" so writes always ask
            # -- selecting the mode clears the standing auto-approvals -- which is
            # the approve/reject path this gate exists to exercise.
            page.get_by_role("button", name="请求批准").click()
            with page.expect_response(
                lambda response: response.url.endswith("/api/agent/autonomy")
                and response.request.method == "PUT"
            ):
                page.get_by_role("dialog", name="应如何批准 Agent 操作？").get_by_role(
                    "option", name="请求批准"
                ).click()
            ...
            # seeing it proves the read-only tool round actually executed. Match
            # the assistant paragraph exactly: a raw-event pane also echoes the
            # same delta as JSON, which would otherwise be a strict-mode clash.
            expect(page.get_by_text("tool round finished", exact=True)).to_be_visible(
                timeout=_ROUND_TRIP_MS
            )

            page.get_by_role("button", name="设置").click()
            settings = page.get_by_role("dialog", name="模型与设置")
            settings.get_by_label("接口地址").fill("https://example.invalid/v1")
            settings.get_by_label("模型").fill("browser-fake")
            settings.get_by_label("API 密钥").fill(secret)
            settings.get_by_role("button", name="保存模型").click()
            # Saving keeps the dialog open; it reports success, then we dismiss it.
            expect(settings.get_by_text("已保存到本机服务")).to_be_visible(timeout=_ROUND_TRIP_MS)
            settings.get_by_role("button", name="×").click()
            ...
            # Approving must actually run the tool. After a reload no thread is
            # re-selected, so the persisted tool message is not re-fetched; the
            # durable proof the tool executed is a tool.completed event, which the
            # inspector's event tab lists and which never appears when rejected.
            ...
            # The reworked UI surfaces the humanised rejection, not the raw event.
            expect(page.get_by_text("写操作被").first).to_be_visible(timeout=_ROUND_TRIP_MS)
