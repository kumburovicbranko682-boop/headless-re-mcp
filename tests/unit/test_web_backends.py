...
class TestJsDeobfuscateSaysWhenCodeWasCut:
    """A deobfuscate that hit the inline cap used to look complete if unread.
    ...
    def test_a_cut_listing_is_marked(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        result = JsClient(Path("/bin/true")).deobfuscate(source)
        assert len(result["code"]) == 400_000
        assert result["truncated"] is True
        assert result["bytes"] == 500_000

    def test_a_short_listing_is_not_labelled_partial(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        from headless_re_mcp.backends.jsre import client as jsre
        ...
        source = tmp_path / "a.js"
        source.write_text("x", encoding="utf-8")
        monkeypatch.setattr(jsre, "_run", lambda cmd, timeout=0: ("var a=1;", "", 0))
        result = JsClient(Path("/bin/true")).deobfuscate(source)
    ...
    def test_the_tool_description_says_to_read_truncated(self) -> None:
        ...
        block = source.split("def js_deobfuscate(")[1].split("def js_beautify(")[0]
...
class TestWasmWatSaysWhenTextWasCut:
    """A WAT dump that hit the inline cap used to look complete if unread.

    Measured: 500_000 characters came back as 400_000 with truncated=True,
    while the tool text omitted truncated. An unattended agent that trusted
    the description treated the fragment as the whole module.
    """
    ...
    def test_a_cut_listing_is_marked(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        tool.write_bytes(b"")
        monkeypatch.setattr(jsre, "_run", lambda cmd, timeout=0: ("w" * 500_000, "", 0))
        result = WasmClient(tool).wat(module)
        assert len(result["wat"]) == 400_000
        assert result["truncated"] is True
    ...
    def test_the_tool_description_says_to_read_truncated(self) -> None:
        ...
        block = source.split("def wasm_wat(")[1].split("def wasm_info(")[0]
        assert "truncated" in block
