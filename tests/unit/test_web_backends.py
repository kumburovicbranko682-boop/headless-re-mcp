...
class TestWebScreenshotDoesNotReportAGhost:
    ...
    An unattended agent then reads a capture that was never taken.
    """
...
def test_the_wasm_info_tool_names_a_cut_listing() -> None:
    """The reply already set truncated; the description did not.

    An agent that only reads the tool text treats a cut objdump field as
    the whole dump.
    """
    import ast
    import inspect
...
def test_the_beautify_tool_names_a_cut_listing() -> None:
    """The reply already set truncated; the description did not.

    An agent that only reads the tool text treats a cut code field as the
    whole beautified file.
    """
    import ast
    import inspect
    ...
    assert docs["js_beautify"]
    assert "truncated" in docs["js_beautify"]
