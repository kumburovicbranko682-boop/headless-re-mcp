...
def test_imports_scan_search_size_schema_matches_native_scan_cap() -> None:
    """The catalog accepted an unbounded IAT search window.

    Measured: input schema search_size had no maximum. Native ScanImports
    clamps and rejects above MaxImportScanBytes (16 MiB). A caller that asks
    to scan 2 GiB still occupies a worker until the adapter refuses.
    """
    from headless_re_mcp.tools.binding import input_schema_for
