...
@pytest.mark.parametrize(
    "wrong", [TargetKind.ELF, TargetKind.MACHO, TargetKind.APK, TargetKind.WEB]
)
...
@pytest.mark.parametrize("kind", [TargetKind.ELF, TargetKind.MACHO])
def test_require_binary_serves_a_non_pe_local_binary(kind: TargetKind, tmp_path: Path) -> None:
    """An ELF or Mach-O session is file-backed, so require_binary returns it --
    the r2 and Ghidra tool surfaces reach those targets without a PE-only guard.
    """
    binary = tmp_path / "a.out"
    session = _session(kind, binary=binary)
...
def test_require_binary_explains_a_locator_only_session() -> None:
    ...
    # A PE, an ELF, a Mach-O or an APK is the thing that would have a local file.
    assert set(caught.value.details["expected_targets"]) == {
        TargetKind.PE.value,
        TargetKind.ELF.value,
        TargetKind.MACHO.value,
