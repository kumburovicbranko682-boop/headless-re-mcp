...
class TestAdbArgumentValidation:
    ...
    def test_push_does_not_wait_on_adb_forever(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """adbutils sync.push used to run with no deadline.

        Measured: push() was invoked with no kwargs; a 0.8s sleep held
        device.push for 0.8s.
        """
        import headless_re_mcp.backends.adb.client as adb_client
...
class TestFridaJavaMethodsDescription:
    def test_java_methods_description_says_to_read_has_more(self) -> None:
        """frida.java.methods already paged but its tool text never said so.
...
class TestFridaServerEnsureIsHonest:
    ...
    def test_ensure_does_not_wait_on_ps_forever(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """ensure used adbutils shell for ps, which connects with a 600s default.

        Measured: a 0.8s sleep in device.shell held ensure 0.8s per ps
        hop; five hops held the worker 4.0s. shell(timeout=5) still
        opened the transport with the library default first.
        """
        import headless_re_mcp.backends.adb.client as adb_client

        monkeypatch.setattr(adb_client, "_FRIDA_PS_TIMEOUT", 0.4)
        ...
        t0 = time.monotonic()
        with pytest.raises(AdbError) as caught:
            _Backend(adb).ensure_frida_server("emulator-5554")
        elapsed = time.monotonic() - t0
        assert elapsed < 2.0
        assert caught.value.code == "timeout"
    ...
    def test_ensure_does_not_wait_on_launch_forever(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """ensure used adbutils shell for su/nohup, which connects with a 600s default.

        Measured: a 0.8s sleep in device.shell held ensure 1.07s on launch
        after ps already returned. shell(timeout=8) still opened the
        transport with the library default first.
        """
        import headless_re_mcp.backends.adb.client as adb_client

        monkeypatch.setattr(adb_client, "_FRIDA_LAUNCH_TIMEOUT", 0.4)
        ...
        adb.write_text(
            ...
            "import time\n"
            "joined = ' '.join(sys.argv)\n"
            "if 'nohup' in joined or 'su -c' in joined:\n"
            "    time.sleep(30)\n"
            "if 'shell' in sys.argv:\n"
            "    print('init')\n"
            "    raise SystemExit(0)\n"
        ...
        adb.chmod(0o755)
        t0 = time.monotonic()
        result = _Backend(_Dev(), adb).ensure_frida_server("emulator-5554")
        elapsed = time.monotonic() - t0
        assert elapsed < 2.0
        assert result["running"] is False
