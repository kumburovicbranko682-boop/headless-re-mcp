...
def test_native_entrypoint_or_missing_ilonly_is_mixed_mode(tmp_path: Path, flags: int) -> None:
    blob = _metadata_root({})
...
def test_tables_without_a_strings_heap_still_report_stats() -> None:
    ...
    module_name, assembly_name, stats = _parse_tables_and_names(tables, {"#~": (0, len(tables))})
