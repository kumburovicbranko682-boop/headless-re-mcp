...
def _tables_stream() -> bytes:
    ...
    typedefs = (
        _u32(0x100000) + _u16(_sidx(b"App")) + _u16(_sidx(b"Ns"))
        + _u16(0) + _u16(1) + _u16(1)
    ) + (
        _u32(0x100001) + _u16(_sidx(b"Helper")) + _u16(0)
        + _u16(0) + _u16(1) + _u16(1)
    )
...
def test_metadata_truncated_before_the_stream_count_is_refused(
    ...
    version = b"v2\0\0"
    blob = (
        b"BSJB" + struct.pack("<HHI", 1, 1, 0) + struct.pack("<I", len(version))
        + version
    )
...
def test_a_truncated_stream_header_stops_the_stream_walk(tmp_path: Path) -> None:
    ...
    blob = (
        b"BSJB" + struct.pack("<HHI", 1, 1, 0) + struct.pack("<I", len(version))
        + version + struct.pack("<HH", 0, 2) + b"\0\0\0\0"
    )
...
def test_an_unterminated_stream_name_stops_the_stream_walk(tmp_path: Path) -> None:
    ...
    blob = (
        b"BSJB" + struct.pack("<HHI", 1, 1, 0) + struct.pack("<I", len(version))
        + version + struct.pack("<HH", 0, 1) + struct.pack("<II", 0, 0) + b"#~"
    )
