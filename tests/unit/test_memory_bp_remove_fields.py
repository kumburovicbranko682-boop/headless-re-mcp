"""breakpoints.memory.remove must name the removed flag it returns."""
...
def _remove_mem_return() -> str:
    source = Path(r"native/xdbg-headless-rpc/rpc_methods.cpp").read_text(encoding="utf-8")
    start = source.index("Outcome RemoveMemoryBreakpointRpc(")
    chunk = source[start : source.index("Outcome ListMemoryBreakpoints", start)]
    return chunk[chunk.rindex("auto result = JsonObject()") :]


def test_memory_remove_answers_with_removed_not_set() -> None:
    """The catalog said remove and never named the flag.

    Measured: RemoveMemoryBreakpointRpc JsonSets address and removed.
    There is no set field. Software remove echoes set false; looking
    for that after a memory delete treats a live clear as still armed.
    """
    returned = _remove_mem_return()
    ...
    described = _tool_docstring("breakpoints.memory.remove")
