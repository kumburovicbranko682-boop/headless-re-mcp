...
def test_search_methods_forward_the_optional_start_and_end_window(tmp_path: Path) -> None:
    ...
    assert service.static_search_text(session_id, text="hello", start=0x1000, end=0x2000).ok
    assert service.static_search_immediate(session_id, value=42, start=0x1000, end=0x2000).ok
...
def test_spill_leaves_a_non_string_field_untouched(tmp_path: Path) -> None:
    ...
    out = service._maybe_spill_static_text(session_id, payload, kind="decompile", text_key="code")
