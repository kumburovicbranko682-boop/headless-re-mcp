...
def test_light_parse_is_cached_per_path(
    tmp_path: Path, fake_androguard: SimpleNamespace
) -> None:
    built: list[str] = []
...
def test_release_swallows_a_resolve_error(
...
def test_methods_reports_an_unknown_class(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
class _MethodClass:
    def __init__(self, name: str, method_names: list[str]) -> None:
        self.name = name
        self._methods = [
            SimpleNamespace(name=m, descriptor="()V", access="public")
...
def test_strings_report_when_the_scan_hit_the_collect_cap(
    ...
    client = ApkClient()
    monkeypatch.setattr(
        ApkClient, "_parsed", lambda self, path: _StringAnalysis(["alpha", "beta"])
    )
