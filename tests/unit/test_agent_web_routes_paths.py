...
def test_mission_guards(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert client.get("/api/agent/missions?status=weird", headers=_HEADERS).status_code == 400
    # cancel of a missing mission -> 404.
    assert client.post("/api/agent/missions/missing/cancel", headers=_HEADERS).status_code == 404
...
def test_save_provider_over_corrupt_profile_starts_fresh(
    ...
    (tmp_path / "providers.json").write_text(
        json.dumps({"profiles": {"corrupt": "not-a-mapping"}})
    )
    saved = client.put(
...
def test_probe_models_reports_missing_profile_and_provider_failure(
    ...
    config_path.write_text(json.dumps({"profiles": {"broken": "not-a-mapping"}}))
