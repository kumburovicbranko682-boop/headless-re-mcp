"""A retained CLOSED session must not start NETReactorSlayer."""
...
def test_dotnet_reactor_unpack_on_a_closed_session_does_not_start_the_cli(
    tmp_path: Path,
) -> None:
    """A retained CLOSED session still resolved, so a late unpack ran the CLI.

    Measured: after close_session, dotnet.reactor.unpack returned ok=True, the
    runner ran once, and artifact_root/dotnet/<id> was created. session.close
    cannot reap a NETReactorSlayer process that started after it returned. The
    model then treats the dead session as cleaned.
    """
    runs: list[Path] = []
    ...
    try:
        ...
        assert runs == []
        out_dir = (tmp_path / "artifacts").resolve() / "dotnet" / session_id
        assert not out_dir.exists()
    finally:
        service.close_all()


def test_dotnet_reactor_unpack_does_not_report_success_if_the_session_closes_during_run(
    tmp_path: Path,
) -> None:
    """A close mid-unpack used to return ok=True for a session that cannot use it."""
    binary = tmp_path / "managed.exe"
    _write_verified_clr_pe(binary)
