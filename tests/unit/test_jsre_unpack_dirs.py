...
from headless_re_mcp.core.service_jsre import JsReAnalysisMixin, JsReError
...
class _FailingJs:
    """Writes a partial tree, then fails -- like an unpack that died mid-run."""
    ...
    def unpack_bundle(
        ...
        *,
        ...
        del path, timeout, offset, limit
        _fill_unpack(out_dir, files=10, size=10 * 1024)
        raise JsReError("backend_error", "webcrack unpack failed")


def test_a_failing_unpack_loop_still_honors_the_byte_cap(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """The failure path used to bound only the tree count, not the bytes.

    Each run leaves a partial tree before it raises; with the prune moved into
    finally the jsre root stays under the byte ceiling on the failure path too.
    Keep the entry cap out of the way so the byte cap is the binding limit.
    """
    monkeypatch.setattr("headless_re_mcp.core.service_jsre.JsClient", _FailingJs)
    monkeypatch.setattr("headless_re_mcp.core.service_jsre.JSRE_UNPACK_MAX_ENTRIES", 1000)
    cap = 250 * 1024
    monkeypatch.setattr("headless_re_mcp.core.service_jsre.JSRE_UNPACK_MAX_BYTES", cap)
    harness = _Harness(tmp_path)
    bundle = tmp_path / "app.js"
    bundle.write_text("bundle", encoding="utf-8")
    for _ in range(20):
