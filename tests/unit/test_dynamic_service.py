...
def test_workflow_navigation_times_out_acquiring_a_busy_runtime_lock(
    ...
    """The initial runtime-lock wait ignored the 100ms navigation timeout."""
    from threading import Event, Thread
...
def test_workflow_status_does_not_wait_for_a_busy_runtime_lock(tmp_path: Path) -> None:
    ...
    outcome: dict[str, object] = {}
    status = Thread(
        target=lambda: outcome.setdefault("result", service.workflow_status(session_id)),
        daemon=True,
    )
    status.start()
...
def test_dynamic_events_times_out_acquiring_a_busy_runtime_lock(tmp_path: Path) -> None:
    """A 100ms event poll queued indefinitely behind another worker request."""
    from threading import Event, Thread
    ...
    poll = Thread(
        target=lambda: outcome.setdefault(
            "result",
            service.dynamic_events(session_id, timeout=0.1),
        ),
        daemon=True,
    )
    poll.start()
