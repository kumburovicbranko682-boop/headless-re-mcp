...
# The index-width helpers moved from private metadata_enum functions into the
# shared public dotnet.tables module when clr_inspect grew its own table walk.
from headless_re_mcp.dotnet.tables import coded_index_size, simple_index_size
...
def test_simple_index_widens_to_four_bytes_at_two_to_the_sixteen(rows: int, expected: int) -> None:
    ...
    """
    assert simple_index_size({0x02: rows}, 0x02) == expected
...
def test_coded_index_width_follows_the_tag_bit_budget(
    ...
    """
    assert coded_index_size({tables[0]: rows}, tables, tag_bits) == expected
...
def test_coded_index_width_is_driven_by_the_largest_referenced_table() -> None:
    ...
    """
    assert coded_index_size({0x06: 1, 0x0A: 5000}, (0x06, 0x0A), 5) == 4
