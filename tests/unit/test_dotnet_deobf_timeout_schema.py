"""The .NET deobfuscation tools must advertise the deadline the service enforces.

``dotnet.deobfuscate`` (de4dot) and ``dotnet.reactor.unpack`` (NETReactorSlayer)
both run their ``timeout`` through ``_detection_timeout``, which raises for
anything over ``MAX_WORKFLOW_TIMEOUT`` (300s). The schemas used to advertise
``le=600``, so an in-schema ``timeout=400`` passed MCP validation only to be
refused downstream as "timeout must be greater than 0 and at most 300 seconds"
-- a ceiling the caller was never told about. Pin the advertised maximum to the
enforced one so the two cannot drift apart again, mirroring the external-unpacker
CLI tools that share the same gate.
"""
...
_DETECTION_CAPPED = ("dotnet.deobfuscate", "dotnet.reactor.unpack")


def _timeout_schema(name: str) -> dict[str, object]:
    handler = next(
        binding.handler
        for binding in build_dotnet_tools(object())  # type: ignore[arg-type]
