...
def test_each_mixin_method_wraps_an_unexpected_error_in_a_failure(
    ...
    del data
    monkeypatch.setattr(
        service_jsre, client_name, _stub_client(client_method, ValueError("boom"))
    )
    harness = _Harness(tmp_path)
...
def test_unpack_bundle_wraps_an_unexpected_error_and_still_prunes(
    ...
    monkeypatch.setattr(
        service_jsre, "JsClient", _stub_client("unpack_bundle", ValueError("boom"))
    )
    harness = _Harness(tmp_path)
...
def test_unpack_that_fails_without_files_is_backend_error(
    ...
    monkeypatch.setattr(
        jsre_client, "_run", lambda cmd, **kwargs: ("", "webcrack blew up", 1)
    )
