...
def test_wasm_info_puts_the_dump_in_objdump_not_sections(tmp_path: Path) -> None:
    ...
    # bytes matches wat/deobfuscate so a truncated dump is not read as whole.
    assert payload["bytes"] == len(text.encode("utf-8"))
    assert payload["truncated"] is False
    assert "Answers with objdump" in _tool_docstring("wasm.info")


def test_wasm_info_reports_full_bytes_when_the_dump_is_cut(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """A large module's objdump over the inline cap must still report its size.

    Before, wasm.info omitted bytes, so a truncated dump read the same
    whether 1 KB or 1 MB was dropped -- unlike its siblings wat/deobfuscate.
    """
    ...
    assert len(str(payload["objdump"])) == 10
