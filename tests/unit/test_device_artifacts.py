...
def test_a_screenshot_loop_cannot_grow_the_device_directory_without_bound(
    ...
    and the bound is the live cap the production path actually enforces
    (prune_capped_dir with UNREGISTERED_CAPTURE_MAX_ENTRIES) -- assert against
    that constant, not a private copy, so a change to the cap moves the guard
    with it instead of passing by coincidence.
    """
