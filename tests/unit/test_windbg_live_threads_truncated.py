"""windbg.live_threads description must name truncated when cdb text was cut."""
...
def test_windbg_live_threads_says_when_cdb_text_was_cut(
    tmp_path: Path, monkeypatch: Any
) -> None:
    """The catalog named threads and never named the cut.

    Measured: 500-char cdb stdout, cap 64 -> truncated True, output_chars
    500, returned_chars 64, threads 64 chars. Looking at threads after a
    ...
    assert payload["threads"] == "A" * 64
    assert payload["pid"] == 4242
    doc = _tool_docstring("windbg.live_threads")
