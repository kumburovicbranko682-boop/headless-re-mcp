...
def test_success_without_an_output_file_is_output_missing(
    ...
    exe, source, destination, sha = _sample(tmp_path)
    monkeypatch.setattr(
        de4dot, "_capture_process", lambda argv, **kwargs: _capture()
    )
...
def test_creation_options_on_windows_hide_the_console_window(
    ...
    monkeypatch.setattr(de4dot.os, "name", "nt")
    monkeypatch.setattr(
        de4dot.subprocess, "STARTUPINFO", _FakeStartupInfo, raising=False
    )
...
def test_creation_options_on_windows_without_a_startupinfo_type(
    ...
    monkeypatch.setattr(de4dot.os, "name", "nt")
    monkeypatch.delattr(de4dot.subprocess, "STARTUPINFO", raising=False)
...
def test_a_process_without_pipes_is_a_structured_failure(
    ...
    killed: list[object] = []
    monkeypatch.setattr(
        de4dot.subprocess, "Popen", lambda argv, **options: _PipelessProcess()
    )
...
def test_the_windows_exit_path_checks_descendants(
    ...
    monkeypatch.setattr(de4dot.os, "name", "nt")
    monkeypatch.setattr(
        process_group_module, "assign_to_process_group", lambda pid: False
    )
    walked: list[int] = []
...
def test_the_windows_exit_path_terminates_leftover_descendants(
    ...
    monkeypatch.setattr(
        process_tree_module, "collect_descendants", lambda pid: [999_999]
    )
    killed: list[object] = []
    monkeypatch.setattr(de4dot, "_terminate_process", killed.append)
    argv = [sys.executable, "-c", "print('done')"]
...
def test_probe_accepts_a_later_argv_variant(
    ...
    monkeypatch.setattr(
        de4dot, "run_bounded", lambda argv, **kwargs: replies.pop(0)
    )
