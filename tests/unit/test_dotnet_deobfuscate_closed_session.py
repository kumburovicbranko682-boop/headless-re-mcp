"""A retained CLOSED session must not start de4dot."""
...
def _service(tmp_path: Path, runner: Any) -> AnalysisService:
    ...
    return AnalysisService(
        ...
        de4dot_runner=runner,
    )


def test_dotnet_deobfuscate_on_a_closed_session_does_not_start_de4dot(
    tmp_path: Path,
) -> None:
    """A retained CLOSED session still resolved, so a late deobfuscate ran de4dot.

    Measured: after close_session, dotnet.deobfuscate returned ok=True, the
    runner ran once, and artifact_root/dotnet/<id> was created. session.close
    cannot reap a de4dot process that started after it returned. The model then
    treats the dead session as cleaned.
    """
    runs: list[Path] = []
...
def test_dotnet_deobfuscate_does_not_report_success_if_the_session_closes_during_run(
    tmp_path: Path,
) -> None:
    """A close mid-de4dot used to return ok=True for a session that cannot use it."""
    binary = tmp_path / "managed.exe"
    _write_verified_clr_pe(binary)
