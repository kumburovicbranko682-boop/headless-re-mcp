...
def test_invalid_ui_capture_session_cannot_create_directories_outside_artifacts(
    ...
    try:
        ...
        # On Windows the path guard rejects the traversal id directly; off
        # Windows the Win32-only capture refuses earlier with the platform
        # guard. Either way the traversal must never create a directory outside
        # the artifact root -- that property is what this test protects.
        if is_windows_host():
