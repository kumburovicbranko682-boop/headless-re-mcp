...
def _pin_release(monkeypatch: pytest.MonkeyPatch, archive: Path, *, tag: str = "bundle-v1") -> None:
...
def test_one_click_setup_on_windows_runs_the_bundle_pipeline(
    ...
    monkeypatch.setattr(web_setup, "configure_ida", lambda **kwargs: {"ok": True, "saved": True})
...
def test_one_click_setup_fails_when_ida_configuration_fails(
    ...
    ida.mkdir()
    (ida / ida_library_names()[0]).write_bytes(b"MZ")
    monkeypatch.setattr(web_setup, "configure_ida", lambda **kwargs: {"ok": False, "saved": True})

    with pytest.raises(InstallError, match="IDA configuration failed"):
        run_one_click_setup(download_release=False, non_interactive=True, ida_home=ida)
