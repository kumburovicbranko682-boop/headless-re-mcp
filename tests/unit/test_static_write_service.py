...
def test_the_exports_tool_says_a_page_is_only_a_page() -> None:
    """The worker already set has_more; the description did not.

    An agent that only reads the tool text treats one page as every export
    in the database.
    """
    import ast
    import inspect
...
def test_the_xrefs_from_tool_says_a_page_is_only_a_page() -> None:
    """The worker already set has_more; the description did not.

    An agent that only reads the tool text treats one page as every outgoing
    reference.
    """
    import ast
    import inspect
    ...
    assert docs["static_xrefs_from"]
    assert "has_more" in docs["static_xrefs_from"]


def test_the_disassemble_tool_names_a_short_walk() -> None:
    """The worker already set partial; the description did not.

    An agent that only reads the tool text treats a walk that stopped at
    max_bytes as count instructions.
    """
    import ast
    import inspect
...
def test_the_bytes_read_tool_names_a_short_buffer() -> None:
    """The worker already set truncated; the description did not.

    An agent that only reads the tool text treats a short read as the
    requested size.
    """
    import ast
    import inspect
...
def test_the_search_text_tool_says_a_page_is_only_a_page() -> None:
    """The worker already set has_more; the description did not.

    An agent that only reads the tool text treats one page as every text
    match in the database.
    """
    import ast
    import inspect
    ...
    assert docs["static_search_text"]
    assert "has_more" in docs["static_search_text"]


def test_the_search_immediate_tool_says_a_page_is_only_a_page() -> None:
    """The worker already set has_more; the description did not.

    An agent that only reads the tool text treats one page as every
    immediate match in the database.
    """
    import ast
    import inspect
    ...
    assert docs["static_search_immediate"]
    assert "has_more" in docs["static_search_immediate"]


def test_the_callers_tool_says_a_page_is_only_a_page() -> None:
    """The worker already set has_more; the description did not.

    An agent that only reads the tool text treats one page as every caller.
    """
    import ast
    import inspect
    ...
    assert docs["static_callers"]
    assert "has_more" in docs["static_callers"]


def test_the_callees_tool_says_a_page_is_only_a_page() -> None:
    """The worker already set has_more; the description did not.

    An agent that only reads the tool text treats one page as every callee.
    """
    import ast
    import inspect
    ...
    assert docs["static_callees"]
    assert "has_more" in docs["static_callees"]


def test_the_entrypoints_tool_says_a_page_is_only_a_page() -> None:
    """The worker already set has_more; the description did not.

    An agent that only reads the tool text treats one page as every entry
    ...
    assert docs["static_entrypoints"]
    assert "has_more" in docs["static_entrypoints"]


def test_the_names_tool_says_a_page_is_only_a_page() -> None:
    """The worker already set has_more; the description did not.

    An agent that only reads the tool text treats one page as every name
    in the database.
    """
    import ast
    import inspect
    ...
    assert docs["static_names"]
    assert "has_more" in docs["static_names"]


def test_the_globals_tool_says_a_page_is_only_a_page() -> None:
    """The worker already set has_more; the description did not.

    An agent that only reads the tool text treats one page as every global.
    """
    import ast
    import inspect
...
def test_the_structs_tool_says_a_page_is_only_a_page() -> None:
    """The worker already set has_more; the description did not.

    An agent that only reads the tool text treats one page as every struct.
    """
    import ast
    import inspect
    ...
    assert docs["static_structs"]
    assert "has_more" in docs["static_structs"]


def test_the_enums_tool_says_a_page_is_only_a_page() -> None:
    """The worker already set has_more; the description did not.

    An agent that only reads the tool text treats one page as every enum.
    """
    import ast
    import inspect
    ...
    assert docs["static_enums"]
    assert "has_more" in docs["static_enums"]


def test_the_basic_blocks_tool_says_a_page_is_only_a_page() -> None:
    """The worker already set has_more; the description did not.

    An agent that only reads the tool text treats one page as every basic
