...
class TestStaticSearchBytesDescriptionMatchesTheCut:
    """static.search.bytes now pages with has_more, but the tool text hid that.

    Measured: 250 hits, limit 100, has_more=true, while the description
    said "search for a binary pattern" -- so a model treats a page as every
    match.
    """
    ...
    def test_the_tool_text_says_to_check_has_more(self) -> None:
        ...
        try:
            ...
            doc = tools["static.search.bytes"].handler.__doc__ or ""
        finally:
            service.close_all()
        assert "has_more" in doc


class TestStaticSearchTextDescriptionMatchesTheCut:
    """static.search.text now pages with has_more, but the tool text hid that.

    Measured: 250 hits, limit 100, has_more=true, while the description
    said "search for text" -- so a model treats a page as every match.
    """
    ...
    def test_the_tool_text_says_to_check_has_more(self) -> None:
        ...
        try:
            tools = {item.name: item for item in build_static_extended_tools(service)}
            doc = tools["static.search.text"].handler.__doc__ or ""
        finally:
            service.close_all()
        assert "has_more" in doc


class TestStaticSearchImmediateDescriptionMatchesTheCut:
    """static.search.immediate now pages with has_more, but the tool text hid that.

    Measured: 250 hits, limit 100, has_more=true, while the description
    said "search for an immediate" -- so a model treats a page as every
    match.
    """
    ...
    def test_the_tool_text_says_to_check_has_more(self) -> None:
        ...
        try:
            tools = {item.name: item for item in build_static_extended_tools(service)}
            doc = tools["static.search.immediate"].handler.__doc__ or ""
        finally:
            service.close_all()
        assert "has_more" in doc
