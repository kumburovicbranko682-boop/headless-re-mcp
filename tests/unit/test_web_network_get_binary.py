...
def test_invalid_base64_body_reports_an_error_rather_than_lying(
    ...
    Like the no-body path, this failure branch must keep the documented
    body/base64_encoded/body_truncated shape: it used to return only the request
    metadata plus body_error, so a caller reading result["body"] hit a missing
    key on exactly the failure it was meant to be told about.
    """
    ...
    assert payload["body_truncated"] is False
    # Request metadata still rides along, and nothing was spilled.
