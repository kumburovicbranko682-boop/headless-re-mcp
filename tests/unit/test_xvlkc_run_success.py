...
def test_run_publishes_the_newest_pe_when_several_exist(
...
def test_run_fails_closed_when_no_pe_output_appears(
    tmp_path: Path, monkeypatch: Any
) -> None:
    exe, source, destination, sha = _prepare(tmp_path)
...
def test_run_rejects_a_malformed_mz_stub_as_output(
    tmp_path: Path, monkeypatch: Any
) -> None:
...
def test_run_fails_closed_when_pes_share_the_newest_mtime(
    tmp_path: Path, monkeypatch: Any
) -> None:
    exe, source, destination, sha = _prepare(tmp_path)
...
def test_run_remaps_a_generic_capture_failure(
    tmp_path: Path, monkeypatch: Any
) -> None:
    exe, source, destination, sha = _prepare(tmp_path)
