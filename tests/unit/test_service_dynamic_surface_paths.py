...
def test_attach_reports_a_dead_pid(
...
def test_attach_returns_the_backend_failure(
    ...
    monkeypatch.setattr(service_mod, "is_pid_alive", lambda pid: True)
    service, session_id = _dynamic_session(
        tmp_path, _FailOnCommandWorker("debug.attach")
    )
...
def test_launch_forwards_the_working_directory(tmp_path: Path) -> None:
    ...
    result = service.dynamic_launch(
        session_id, arguments="--go", working_directory="/tmp/work"
    )
...
def test_launch_returns_the_open_failure_when_implicit_open_fails(
    ...
    failure = Result[JsonObject](ok=False, error=service_mod.RpcError(
        code="backend_unavailable", message="cannot open"
    ))
    monkeypatch.setattr(
...
def test_launch_returns_the_backend_failure(tmp_path: Path) -> None:
    service, session_id = _dynamic_session(
        tmp_path, _FailOnCommandWorker("debug.launch")
    )
...
def test_launch_returns_a_failing_post_breakpoint_resume(tmp_path: Path) -> None:
    service, session_id = _dynamic_session(
        tmp_path, _FailOnCommandWorker("debug.resume")
    )
