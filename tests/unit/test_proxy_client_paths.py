...
def test_instance_start_reports_a_thread_that_fails_to_launch_mitmproxy(
    ...
    # Force the lazy import in _run to fail so the real _run raises ImportError,
    # sets _error, and start() surfaces it as backend_error -- exercising _run's
    # except/finally whether or not mitmproxy is installed. With a real mitmproxy
    # present _run would instead launch a live proxy that never accepts (the
    # patched _port_accepts), so start() would time out rather than fail here.
...
def test_backend_check_available_reports_and_caches(monkeypatch: pytest.MonkeyPatch) -> None:
    # Force the lazy ``import mitmproxy`` in ``_check_available`` to fail so the
    # capability-unavailable path runs whether or not mitmproxy is installed. The
    # proxy extra is a supported configuration; with it present ``start`` would
    # otherwise get past the gate and try to bind a live proxy.
    monkeypatch.setitem(sys.modules, "mitmproxy", None)
