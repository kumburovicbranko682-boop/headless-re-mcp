...
def test_drain_proxy_servers_returns_early_and_swallows_errors() -> None:
    ...
    _drain_proxy_servers(SimpleNamespace(addons=SimpleNamespace(get=boom)), asyncio.new_event_loop())
...
def _flow(
    *,
    ...
    response = (
        SimpleNamespace(status_code=status, headers={"content-type": content_type}, raw_content=body)
...
def test_recorder_evicts_to_stay_under_the_retained_ceiling(
    ...
    recorder = _FlowRecorder()
    for i in range(6):
        recorder.response(_flow(flow_id=f"f{i}", url=f"http://x/{i}", body=b"payload-bytes"))
    # The earliest retained flows were omitted to keep the total under the cap.
    assert recorder.retained_bytes() <= 40
    omitted = [e for e in recorder.snapshot() if e.get("body_omitted")]
    assert omitted  # at least one older flow lost its retained body
...
def test_export_har_refuses_over_the_cap(
...
def test_export_har_writes_a_bounded_file(tmp_path: Path) -> None:
    recorder = SimpleNamespace(
        snapshot=lambda: [
            {"method": "GET", "url": "http://x/1", "status": 200, "content_type": "text/html",
             "response_size": 10}
        ]
    )
    backend = ProxyBackend()
...
def test_ca_cert_path_finds_a_cert_or_none(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
