...
def test_instance_start_reports_a_thread_that_fails_to_launch_mitmproxy(
    ...
    # Force mitmproxy absent so the real _run raises ImportError, sets _error,
    # and start() surfaces it as backend_error -- exercising _run's
    # except/finally. Relying on the dependency being uninstalled made this pass
    # only in CI; with the [proxy] extra present _run would launch a real proxy
    # instead. A None parent makes ``from mitmproxy import ...`` raise regardless.
...
def test_backend_check_available_reports_and_caches(monkeypatch: pytest.MonkeyPatch) -> None:
    # Force absence: with the [proxy] extra installed the import succeeds and
    # start() would try to bind a real proxy instead of reporting the missing
    # capability this arc pins.
    monkeypatch.setitem(sys.modules, "mitmproxy", None)
