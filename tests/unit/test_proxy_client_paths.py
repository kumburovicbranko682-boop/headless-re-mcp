...
def test_instance_start_reports_a_thread_that_fails_to_launch_mitmproxy(
    ...
    # With mitmproxy forced absent the real _run raises ImportError, sets
    # _error, and start() surfaces it as backend_error -- exercising _run's
    # except/finally. Forced, not assumed: with the proxy extra installed the
    # unmocked _run would launch a real DumpMaster on port 8080 and this test
    # became order-dependent (green in a full run, red in isolation).
...
def test_backend_check_available_reports_and_caches(monkeypatch: pytest.MonkeyPatch) -> None:
    # Force the absence instead of assuming it: with the proxy extra installed
    # the probe's `import mitmproxy` succeeds and start() proceeds past the
    # guard this test exists to pin. A None sys.modules entry halts that
    # import regardless of what the machine has.
    monkeypatch.setitem(sys.modules, "mitmproxy", None)
