...
def _flow(
    *,
    ...
    request = SimpleNamespace(
        method=method, pretty_url=url, host=host, headers={}, raw_content=b""
    )
    response = (
        SimpleNamespace(
...
def _install_fake_mitmproxy(
    monkeypatch: pytest.MonkeyPatch, dump_master: type
) -> None:
    import types
...
def test_flow_get_flags_a_truncated_request_side(tmp_path: Path) -> None:
    ...
    request = SimpleNamespace(
        method="GET", pretty_url="http://x/1", headers=many, raw_content=b""
    )
