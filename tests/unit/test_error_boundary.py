...
def _keyword_literal(fragment: str) -> str:
    ...
    so anything else fails the test rather than being skipped silently.
    """
...
def test_every_shared_redaction_keyword_is_scrubbed_inline(
    ...
    lists, and they drifted: ``authorization`` (outside the Bearer form) and
    ``providerApiKeys`` were masked in payloads but reached the incident log and
    the HTTP 500 body in the clear when the same value landed in an exception
    message. The patterns are now built from redaction's own tuple; this walks
    that tuple, so a keyword added there is exercised here without anyone
    remembering to update a matrix.
    """
...
def test_a_bare_bearer_token_is_scrubbed_without_its_header_name() -> None:
    ...
    inline scrubber let it through until it, too, ran BEARER_VALUE.
    """
