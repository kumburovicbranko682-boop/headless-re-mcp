"""r2.exports must name the address object on each exported symbol."""
...
def test_r2_exports_puts_the_eat_va_under_address_object(tmp_path: Path) -> None:
    """The catalog said address and meant a VA integer.

    Measured: enrich_r2_payload puts {va, rva, module} on each item.address.
    Looking for an integer address after a successful export list reads as
    radare2 returning no EAT VA.
    """
    ...
    payload = enrich_r2_payload(
        {
            "raw": json.dumps(
                [{"name": "DllGetClassObject", "vaddr": 0x140001000}]
            ),
            "commands": ["iEj"],
        },
        binary=pe,
    ...
    described = _tool_docstring("r2.exports")
