...
def test_unpack_xvlkc_unpack_on_a_closed_session_does_not_start_the_cli(
    tmp_path: Path,
) -> None:
    runs: list[Path] = []
...
def test_unpack_xvlkc_unpack_does_not_report_success_if_the_session_closes_during_run(
    tmp_path: Path,
) -> None:
    binary = tmp_path / "sample.exe"
    _write_pe(binary)
