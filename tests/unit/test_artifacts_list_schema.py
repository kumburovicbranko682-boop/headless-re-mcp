"""artifacts.list must refuse a negative offset at the tool schema."""
...
def test_artifacts_list_schema_refuses_a_negative_offset() -> None:
    ...
    as the negative page it asked for silently reread the newest artifacts.
    """
    handler = next(
        binding.handler
        for binding in build_meta_tools(object())  # type: ignore[arg-type]
        if binding.name == "artifacts.list"
    )
