...
def _tables_stream(
    ...
    *,
    ...
    counts = b"".join(
        struct.pack("<I", (declared or {}).get(bit, row_counts[bit]))
...
def _standard_image(tmp_path: Path, name: str = "crafted.exe") -> Path:
    ...
    tables = _tables_stream(
        {0x00: 1, 0x02: 1, 0x04: 1, 0x06: 3, 0x0A: 1, 0x28: 1}, rows
    )
...
def test_bad_arguments_are_rejected(
    tmp_path: Path, kind: str, offset: int, limit: int
) -> None:
    path = _standard_image(tmp_path)
...
def test_a_forged_row_count_is_bounded_by_the_stream(tmp_path: Path) -> None:
    ...
    tables = _tables_stream(
        {0x00: 1, 0x02: 2}, rows, declared={0x02: 0x7FFFFFFF}
    )
