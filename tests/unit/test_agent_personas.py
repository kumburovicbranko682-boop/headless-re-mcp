...
def test_importing_a_leading_underscore_title_yields_a_valid_id(tmp_path: Path) -> None:
    # The id is auto-generated by _slug, then fed to _body_path, whose regex
    # requires an alphanumeric first character. Underscores survive slug
    # sanitisation, so a draft-style name like "_notes.md" used to slug to
    # "_notes-<digest>" and fail import with the opaque persona_id_invalid --
    # even though the caller supplied a file, not an id.
    store = PersonaStore(tmp_path / "personas", seed_paths=())

    for title in ("_notes", "__agent__", "_2024_persona"):
        listed = store.import_markdown(title=title, body=f"body for {title}")
        current = listed["current"]
        assert current[0].isalnum()
        assert (store.root / f"{current}.md").is_file()
    # An internal underscore is content, not a leading-character problem, and
    # must be preserved rather than stripped wholesale.
    listed = store.import_markdown(title="_2024_persona", body="body for _2024_persona")
    assert listed["current"].startswith("2024_persona-")

    source = tmp_path / "_draft.md"
    source.write_text("draft persona body", encoding="utf-8")
    imported = store.import_path(source)
    assert imported["current"][0].isalnum()
    assert imported["current"].startswith("draft-")
