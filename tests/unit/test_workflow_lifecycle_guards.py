...
def test_tracked_module_rejects_invalid_fields(overrides: dict[str, object], match: str) -> None:
...
def test_unload_event_for_an_already_unloaded_module_is_not_reinvalidated() -> None:
    ...
    again = consume_module_events(unloaded, _batch(1, _event(2, "module.unloaded", {"base": base})))
...
def test_debug_stopped_still_reports_bases_of_already_unloaded_modules() -> None:
    ...
    stopped = consume_module_events(unloaded, _batch(1, _event(2, "debug.stopped", {})))
...
def test_nameless_load_event_falls_back_to_the_runtime_base() -> None:
    ...
    hit = consume_module_events(state, _batch(0, _event(1, "module.loaded", {"base": base})))
    assert hit.invalidated_keys == {"payload"}
...
def test_path_selector_matches_on_the_path_basename() -> None:
    state = _tracked(selector=ModuleSelector(path=r"C:\sample\fixtures\Event_Fixture.DLL"))
