...
_READY = (
    ...
    '{"capabilities": ["ida.decompile"], "backend": "ida"}})\n'
)
...
def _spawn(
    ...
    *,
    ...
    return IdaWorkerClient(
        _binary(tmp_path), _settings(tmp_path), startup_timeout=startup_timeout
    )
...
def test_non_list_capabilities_stay_empty(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_close_after_worker_death_is_quiet(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    client = _spawn(tmp_path, monkeypatch, _READY + "sys.exit(0)\n")
    _wait_for(lambda: client.exit_code is not None)
