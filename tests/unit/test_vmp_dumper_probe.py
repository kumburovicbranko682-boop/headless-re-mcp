"""VMPDump capability probe must not leave what a launcher started."""
...
def _launcher(tmp_path: Path) -> tuple[Path, Path]:
    ...
    script = tmp_path / "vmp_launcher.py"
    marker = script.with_suffix(".child")
    if os.name == "nt":
        script.write_text(body, encoding="utf-8")
        wrapper = tmp_path / "vmpdump.cmd"
...
def test_a_timeout_kills_what_the_probe_started(tmp_path: Path) -> None:
    ...
    child = 0
    try:
        ok, output = probe_vmp_dumper(stub, timeout=0.5)
        assert ok is False
        assert output == ""
...
def test_a_probe_that_prints_usage_is_ready(tmp_path: Path) -> None:
    if os.name == "nt":
        stub = tmp_path / "vmpdump.cmd"
        stub.write_text("@echo off\r\necho VMPDump usage -ep=\r\n", encoding="utf-8")
    else:
        stub = tmp_path / "vmpdump"
        stub.write_text(
            "#!/usr/bin/env python3\nimport sys\nprint('VMPDump usage -ep=')\nsys.exit(0)\n",
            encoding="utf-8",
        )
        stub.chmod(0o755)
    ok, output = probe_vmp_dumper(stub, timeout=5.0)
    assert ok is True
