"""NETReactorSlayer capability probe must not leave what a launcher started."""
...
def _launcher(tmp_path: Path) -> tuple[Path, Path]:
    ...
    script = tmp_path / "nrs_launcher.py"
    marker = script.with_suffix(".child")
    if os.name == "nt":
        script.write_text(body, encoding="utf-8")
        wrapper = tmp_path / "NETReactorSlayer.cmd"
...
def test_a_timeout_kills_what_the_probe_started(tmp_path: Path) -> None:
    ...
    child = 0
    try:
        ok, output = probe_net_reactor_slayer(stub, timeout=0.5)
        assert ok is False
        assert output == ""
...
def test_a_probe_that_prints_usage_is_ready(tmp_path: Path) -> None:
    if os.name == "nt":
        stub = tmp_path / "NETReactorSlayer.cmd"
        stub.write_text("@echo off\r\necho NETReactorSlayer --no-pause\r\n", encoding="utf-8")
    else:
        stub = tmp_path / "NETReactorSlayer"
        stub.write_text(
            "#!/usr/bin/env python3\n"
            "import sys\n"
            "print('NETReactorSlayer --no-pause')\n"
            "sys.exit(0)\n",
            encoding="utf-8",
        )
        stub.chmod(0o755)
    ok, output = probe_net_reactor_slayer(stub, timeout=5.0)
    assert ok is True
    assert "netreactorslayer" in output.casefold()
