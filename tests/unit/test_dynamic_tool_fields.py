...
def test_dynamic_memory_read_schema_matches_native_address_floor() -> None:
    """The catalog accepted a negative address.

    Measured: input schema address has no minimum. Native ReadMemory reads
    address as unsigned. A negative address still occupies a worker until the
    adapter refuses, and the catalog never said the read would be rejected.
    """
    from headless_re_mcp.tools.binding import input_schema_for
...
def test_dynamic_breakpoint_set_schema_matches_native_address_floor() -> None:
    """The catalog accepted a negative breakpoint address.

    Measured: input schema address has no minimum. Native ChangeBreakpoint
    reads address as unsigned before building SetBPX. A negative address still
    occupies a worker until the adapter refuses, and the catalog never said the
    breakpoint would be rejected.
    """
    from headless_re_mcp.tools.binding import input_schema_for
    ...
    start = native.index("Outcome ChangeBreakpoint")
    chunk = native[start : native.index("bool ReadOptionalUnsigned", start)]
    assert (
        'ReadUnsigned(params, "address", address, error, std::numeric_limits<duint>::max())'
        in chunk
    )
    assert 'std::string(set ? "SetBPX " : "DeleteBPX ")' in chunk
    handler = next(
        binding.handler
        for binding in build_dynamic_tools(object())  # type: ignore[arg-type]
        if binding.name == "dynamic.breakpoint.set"
    )
    props = input_schema_for(handler)["properties"]
    assert props["address"]["minimum"] == 0


def test_dynamic_breakpoint_remove_schema_matches_native_address_floor() -> None:
    """The catalog accepted a negative address on remove.

    Measured: input schema address has no minimum. ChangeBreakpoint serves both
    set and remove and reads address as unsigned before building DeleteBPX. A
    negative address still occupies a worker until the adapter refuses, and the
    catalog never said the removal would be rejected.
    """
    from headless_re_mcp.tools.binding import input_schema_for
    ...
    assert '"DeleteBPX "' in chunk
    handler = next(
        binding.handler
        for binding in build_dynamic_tools(object())  # type: ignore[arg-type]
        if binding.name == "dynamic.breakpoint.remove"
    )
    props = input_schema_for(handler)["properties"]
    assert props["address"]["minimum"] == 0
