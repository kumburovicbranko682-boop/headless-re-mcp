...
def test_a_timed_out_xvlkc_probe_kills_what_it_started(tmp_path: Path) -> None:
    """subprocess.run left the child of a probe wrapper running.
    ...
    fake = tmp_path / ("xvlkc.cmd" if os.name == "nt" else "xvlkc")
    body = tmp_path / "fake_xvlkc.py"
    body.write_text(
    ...
    ok, _text = probe_xvlkc(fake, timeout=0.4)
...
def test_a_timed_out_scylla_probe_kills_what_it_started(tmp_path: Path) -> None:
    """GUI Scylla is allowed to time out; its child is not allowed to stay.

    Measured: a wrapper that spawned a sleeper returned True in 0.40s
    (timeout_after_start) and left the child in state S.
    """
    import os
    import stat
    import sys
    ...
    marker = tmp_path / "child.pid"
    fake = tmp_path / ("scylla.cmd" if os.name == "nt" else "scylla")
    body = tmp_path / "fake_scylla.py"
    body.write_text(
        "import subprocess, sys, time\n"
        f"marker = {str(marker)!r}\n"
        "child = subprocess.Popen([sys.executable, '-c', "
    ...
    started = time.monotonic()
    ok, text = probe_scylla(fake, timeout=0.4)
    elapsed = time.monotonic() - started
