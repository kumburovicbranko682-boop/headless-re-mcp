...
def test_module_and_import_wrappers_dispatch_expected_rpc() -> None:
    ...
    client.imports_scan(0x400000, search_start=1, search_size=2, max_candidates=3, mode="strict")
...
def test_trace_status_skips_validation_when_uninitialized() -> None:
    client = _bare_client()
    client.request = lambda method, params=None, *, timeout=10.0: {"initialized": False}  # type: ignore[method-assign]
...
def test_note_debuggee_pid_parses_or_ignores(payload: JsonObject, expected: Any) -> None:
    client = _bare_client()
    # An int sentinel keeps the attribute's declared type; no real pid is -999.
    client._debuggee_pid = -999
    client._note_debuggee_pid(payload)
    if expected == "unchanged":
        assert client._debuggee_pid == -999
    else:
...
def test_monitor_windows_records_and_suppresses(monkeypatch: pytest.MonkeyPatch) -> None:
    client = _bare_client()
    client._desktop = object()  # type: ignore[assignment]
...
def test_request_exit_writes_exit_then_closes() -> None:
    ...
    stdin = cast(FakeStdin, client._process.stdin)
    assert stdin.value == "exit\n"
...
def test_request_exit_returns_when_the_process_is_gone() -> None:
    ...
    client._request_exit()
    assert cast(FakeStdin, client._process.stdin).value == ""


def test_request_exit_swallows_a_broken_pipe() -> None:
    client = _bare_client()
    client._process.stdin = FakeStdin([], fail=BrokenPipeError("gone"))  # type: ignore[assignment]
...
def test_finish_threads_joins_and_cleans_up() -> None:
    ...
    assert cast(FakeThread, client._window_thread).joined
    assert cast(FakeThread, client._stdout_thread).joined
    assert cast(FakeThread, client._stderr_thread).joined
...
def test_close_stops_trace_and_debugging_before_exit(
    ...
    assert cast(FakeStdin, client._process.stdin).value == "exit\n"
    assert transport.closed is True
    assert client._closed is True
...
def test_connect_transport_closes_a_previous_transport(
    ...
    assert cast(object, client._transport) is new
