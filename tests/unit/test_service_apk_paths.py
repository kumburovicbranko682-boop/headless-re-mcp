...
class _FakeApktool:
    ...
    def sign(self, source: Path, out_apk: Path, *, keystore: Any, keystore_password: str,
             key_alias: str, timeout: float) -> Any:
        out_apk.write_bytes(b"PK\x03\x04")
        return {"signed": True}
...
def _apk_session(service: AnalysisService, tmp_path: Path) -> str:
    ...
    session = Session(
        target=TargetKind.APK, binary=apk, locator=str(apk), state=SessionState.READY
    )
...
def test_refuse_oversized_tree_returns_when_sizing_raises(
...
def test_apk_call_backed_methods_succeed_and_map_errors(
    tmp_path: Path, monkeypatch: Any
) -> None:
    service = _service(tmp_path)
    try:
...
def test_apk_decompile_rolls_back_when_the_session_closes(
    tmp_path: Path, monkeypatch: Any
) -> None:
...
def test_apk_export_sources_rolls_back_when_the_session_closes(
    ...
    try:
        ...
        class _ClosingJadx:
            ...
            def export_sources(self, binary: Path, out_dir: Path, *, timeout: float,
                               no_imports: bool) -> Any:
                service.registry.transition(session_id, SessionState.FAILED)
                return {"exported": True}
