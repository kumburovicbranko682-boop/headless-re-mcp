...
def test_a_cut_live_module_list_says_it_was_cut(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A live module listing that hit the buffer used to look complete if unread.

    Measured: 500 characters of live ``lm`` came back as 64 with
    truncated=True on modules, while the tool text omitted truncated. An
    unattended agent that trusted the description treated the fragment as
    every module.
    """
    from headless_re_mcp.backends.common.bounded_run import Completed
...
def test_live_modules_tool_description_says_to_read_truncated() -> None:
    ...
    block = source.split("def windbg_live_modules(")[1].split("def windbg_live_disasm(")[0]
    assert "truncated" in block
