"""A retained CLOSED session must not run detection."""
...
def test_detect_scan_on_a_closed_session_is_refused(tmp_path: Path) -> None:
    """A retained CLOSED session still resolved, so a late scan returned findings.

    Measured: after close_session, detect.scan returned ok=True with 4 findings
    and diec/exeinfope sources. The model then treats the dead session as
    classified and follows unpack.plan / unpack.start.
    """
...
def test_packer_classify_on_a_closed_session_is_refused(tmp_path: Path) -> None:
    """classify goes through detect.scan, which used to succeed after close."""
    binary = tmp_path / "managed.exe"
    _write_verified_clr_pe(binary)
    service = _service(tmp_path)
    try:
        ...
        result = service.packer_classify(session_id, use_die=False)
        assert result.ok is False
        assert result.error is not None
        assert result.error.code == "invalid_request"
        assert "closed" in result.error.message
