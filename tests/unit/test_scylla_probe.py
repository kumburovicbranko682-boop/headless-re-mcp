"""Scylla capability probe must not leave what a launcher started."""
...
def _launcher(tmp_path: Path) -> tuple[Path, Path]:
    """A stub Scylla that starts a child and then hangs, like a GUI wrapper."""
    body = (
    ...
    script = tmp_path / "scylla_launcher.py"
    marker = script.with_suffix(".child")
    if os.name == "nt":
        script.write_text(body, encoding="utf-8")
        wrapper = tmp_path / "Scylla.cmd"
...
def test_a_timeout_kills_what_the_probe_started(tmp_path: Path) -> None:
    """subprocess.run kills only the stub. Measured: 0.6s timeout, child still alive."""
    stub, marker = _launcher(tmp_path)
    child = 0
    try:
        ok, output = probe_scylla(stub, timeout=0.5)
        assert ok is True
...
def test_a_probe_that_finishes_is_still_ready(tmp_path: Path) -> None:
    if os.name == "nt":
        stub = tmp_path / "Scylla.cmd"
        ...
        stub.chmod(0o755)
    ok, output = probe_scylla(stub, timeout=5.0)
    assert ok is True
    assert "scylla" in output.casefold()
