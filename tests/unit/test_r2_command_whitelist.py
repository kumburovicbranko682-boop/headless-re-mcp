...
@pytest.mark.parametrize(
    ...
    [
        ...
        "i anything",
        "pdj 513 @ 0",
        # axj ignores the seek and dumps the whole xref database, so the
        # "axj @ N" form reads as address-scoped while the address is inert.
        # Neither it nor the bare listing may come back quietly.
        "axj @ 0",
        "axj",
    ],
)
...
def test_r2_whitelist_keeps_bounded_disasm_and_xref_forms(
    ...
    monkeypatch.setattr(r2_module, "run_bounded", fake_run)
    result = client.run(binary, ["pdj 32 @ 4198400", "axtj @ 0x401000", "axfj @ 16"])

    assert result["commands"] == ["pdj 32 @ 4198400", "axtj @ 0x401000", "axfj @ 16"]
