...
def test_detection_returns_the_cached_hint(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_detection_ignores_an_unrecognized_classification(
    ...
    monkeypatch.setattr(
        service,
        "packer_classify",
        lambda *a, **k: Result[JsonObject](ok=True, data={"stealth_profile": "nonsense!!"}),
    )
