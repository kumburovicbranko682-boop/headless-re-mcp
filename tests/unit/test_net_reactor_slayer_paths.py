...
def _inputs(tmp_path: Path, *, contents: bytes = b"managed-assembly-bytes") -> tuple[
    Path, Path, Path, str
]:
...
def test_a_missing_executable_is_reported(tmp_path: Path) -> None:
    ...
    with pytest.raises(NetReactorSlayerError) as caught:
        run_net_reactor_slayer(
            tmp_path / "gone.exe", source, destination, input_sha256=sha
        )
...
def test_an_oversized_input_is_refused(tmp_path: Path) -> None:
    ...
    with pytest.raises(NetReactorSlayerError) as caught:
        run_net_reactor_slayer(
            exe, source, destination, input_sha256=sha, max_file_size=4
        )
...
def test_an_input_that_is_not_a_file_is_refused(tmp_path: Path) -> None:
    ...
    with pytest.raises(NetReactorSlayerError) as caught:
        run_net_reactor_slayer(
            exe, source_dir, tmp_path / "out.exe", input_sha256="0" * 64
        )
...
def test_a_sha_that_changed_before_the_run_is_refused(tmp_path: Path) -> None:
    ...
    with pytest.raises(NetReactorSlayerError) as caught:
        run_net_reactor_slayer(
            exe, source, destination, input_sha256="0" * 64
        )
...
def test_probe_recognizes_the_tool_banner(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    exe = tmp_path / "nrs.exe"
    exe.write_bytes(b"x")
...
def test_probe_reports_empty_output_as_unavailable(
    ...
    exe.write_bytes(b"x")
    monkeypatch.setattr(
        nrs_mod, "run_bounded", lambda cmd, **kwargs: Completed(0, b"", b"")
    )
