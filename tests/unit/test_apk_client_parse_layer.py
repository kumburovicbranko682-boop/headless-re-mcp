...
def test_available_is_false_when_androguard_is_absent(monkeypatch: pytest.MonkeyPatch) -> None:
    # Force the lazy ``import androguard`` in ``__init__`` to fail so the
    # unavailable path runs whether or not androguard is installed. The android
    # extra is a documented, supported configuration; hard-asserting its absence
    # made this test fail on any machine that actually has it installed.
