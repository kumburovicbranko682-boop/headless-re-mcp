...
def probe_scylla(executable: Path, *, timeout: float = 5.0) -> tuple[bool, str]:
    ...
    try:
        # Measured: subprocess.run(timeout=) killed the Scylla launcher and
        # left the process it started alive. Doctor then reported READY
        # (timeout_after_start) while that child kept a core for the rest of
        # the service's life.
