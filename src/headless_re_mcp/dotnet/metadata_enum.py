...
def _table_row_size(meta: _MetaCtx, table: int) -> int:
    ...
    sizes: dict[int, int] = {
        ...
        0x1E: 4 + 4,  # ENCLog: Token(4) + FuncCode(4)
        0x1F: 4,  # ENCMap: Token(4)
        0x20: 4 + 2 + 2 + 2 + 2 + 4 + b + s + s,  # Assembly
        0x21: 4,  # AssemblyProcessor
        0x22: 12,  # AssemblyOS
        0x23: 2 + 2 + 2 + 2 + 4 + b + s + s + b,  # AssemblyRef (+HashValue; no HashAlgId)
        0x24: 4 + _simple_index_size(rc, 0x23),  # AssemblyRefProcessor
        0x25: 12 + _simple_index_size(rc, 0x23),  # AssemblyRefOS
        0x26: 4 + s + b,  # File: Flags(4) + Name(String) + HashValue(Blob)
        ...
        0x2B: method_def_or_ref + b,  # MethodSpec: Method(coded) + Instantiation(Blob)
