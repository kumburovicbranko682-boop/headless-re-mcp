...
def probe_de4dot_version(executable: Path, *, timeout: float = 5.0) -> tuple[bool, str]:
    ...
    # A hang is not "this build uses different flags". Measured: three timed-out
    # attempts returned True with empty text and left three sleeper children in
    # state S, so doctor marked de4dot READY while the orphans kept the box.
    saw_exit = False
