...
class UiAutomationMixin:
    ...
    def ui_screenshot(
        ...
        *,
        ...
        # Validate the session id before the platform gate: a path-traversal id
        # is malformed input on every platform, and letting the Linux
        # unsupported_on_platform answer fire first would mask the guard the
        # capture path derives its directory from.
    ...
    def ui_ocr(
        ...
        *,
        ...
        # Validate the session id before the platform gate: see ui_screenshot.
        if not session_id or Path(session_id).name != session_id:
            return _failure(
                ValueError("invalid session id for UI capture path"),
                session_id=session_id,
