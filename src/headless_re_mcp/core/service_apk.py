...
class ApkAnalysisMixin:
    ...
    def apk_classes(
        ...
        *,
        ...
        try:
            binary = self._apk_binary(session_id)
            data = ApkClient().classes(binary, offset=offset, limit=limit, contains=contains)
            return _success(data, session_id=session_id, backend="apk")
        except ApkError as exc:
            return _failure(_as_rpc(exc), session_id=session_id)
        except BaseException as exc:
        ...
        *,
        ...
        try:
            ...
            data = ApkClient().methods(
                binary, class_name, offset=offset, limit=limit, contains=contains
            )
            return _success(data, session_id=session_id, backend="apk")
        except ApkError as exc:
            return _failure(_as_rpc(exc), session_id=session_id)
    ...
    def apk_subclasses(
        self, session_id: str, type_name: str, offset: int = 0, limit: int = 100
    ) -> Result[JsonObject]:
        try:
            binary = self._apk_binary(session_id)
            data = ApkClient().subclasses(binary, type_name, offset=offset, limit=limit)
            return _success(data, session_id=session_id, backend="apk")
        except ApkError as exc:
            return _failure(_as_rpc(exc), session_id=session_id)
        except BaseException as exc:
    ...
    def apk_strings(
        ...
        *,
        ...
        try:
            binary = self._apk_binary(session_id)
            data = ApkClient().strings(binary, offset=offset, limit=limit, contains=contains)
            return _success(data, session_id=session_id, backend="apk")
        except ApkError as exc:
            return _failure(_as_rpc(exc), session_id=session_id)
        except BaseException as exc:
    ...
    def apk_xrefs(
        ...
        *,
        ...
        try:
            binary = self._apk_binary(session_id)
            data = ApkClient().xrefs(binary, method_name, limit=limit, direction=direction)
            return _success(data, session_id=session_id, backend="apk")
        except ApkError as exc:
            return _failure(_as_rpc(exc), session_id=session_id)
        except BaseException as exc:
    ...
    def apk_string_xrefs(
        ...
        *,
        ...
        try:
            binary = self._apk_binary(session_id)
            data = ApkClient().string_xrefs(binary, value, limit=limit, contains=contains)
            return _success(data, session_id=session_id, backend="apk")
        except ApkError as exc:
            return _failure(_as_rpc(exc), session_id=session_id)
        except BaseException as exc:
