...
class ApkAnalysisMixin:
    ...
    def apk_files(self, session_id: str, offset: int = 0, limit: int = 200) -> Result[JsonObject]:
        try:
            binary = self._apk_binary(session_id)
            data = ApkClient().files(binary, offset=offset, limit=limit)
            return _success(data, session_id=session_id, backend="apk")
        except ApkError as exc:
            return _failure(_as_rpc(exc), session_id=session_id)
        except BaseException as exc:
    ...
    def apk_dex_headers(self, session_id: str) -> Result[JsonObject]:
        try:
            binary = self._apk_binary(session_id)
            data = ApkClient().dex_headers(binary)
            return _success(data, session_id=session_id, backend="apk")
        except ApkError as exc:
            return _failure(_as_rpc(exc), session_id=session_id)
        except BaseException as exc:
    ...
    def apk_method_info(
        self, session_id: str, class_name: str, method_name: str
    ) -> Result[JsonObject]:
        try:
            binary = self._apk_binary(session_id)
            data = ApkClient().method_info(binary, class_name, method_name)
            return _success(data, session_id=session_id, backend="apk")
        except ApkError as exc:
            return _failure(_as_rpc(exc), session_id=session_id)
        except BaseException as exc:
    ...
    def apk_class_info(self, session_id: str, class_name: str) -> Result[JsonObject]:
        try:
            binary = self._apk_binary(session_id)
            data = ApkClient().class_info(binary, class_name)
            return _success(data, session_id=session_id, backend="apk")
        except ApkError as exc:
            return _failure(_as_rpc(exc), session_id=session_id)
        except BaseException as exc:
    ...
    def apk_exported_components(self, session_id: str) -> Result[JsonObject]:
        try:
            binary = self._apk_binary(session_id)
            data = ApkClient().exported_components(binary)
            return _success(data, session_id=session_id, backend="apk")
        except ApkError as exc:
            return _failure(_as_rpc(exc), session_id=session_id)
        except BaseException as exc:
    ...
    def apk_urls(
        ...
        try:
            binary = self._apk_binary(session_id)
            data = ApkClient().urls(binary, offset=offset, limit=limit)
            return _success(data, session_id=session_id, backend="apk")
        except ApkError as exc:
            return _failure(_as_rpc(exc), session_id=session_id)
        except BaseException as exc:
    ...
    def apk_uses_features(self, session_id: str) -> Result[JsonObject]:
        try:
            binary = self._apk_binary(session_id)
            data = ApkClient().uses_features(binary)
            return _success(data, session_id=session_id, backend="apk")
        except ApkError as exc:
            return _failure(_as_rpc(exc), session_id=session_id)
        except BaseException as exc:
    ...
    def apk_declared_permissions(self, session_id: str) -> Result[JsonObject]:
        try:
            binary = self._apk_binary(session_id)
            data = ApkClient().declared_permissions(binary)
            return _success(data, session_id=session_id, backend="apk")
        except ApkError as exc:
            return _failure(_as_rpc(exc), session_id=session_id)
        except BaseException as exc:
    ...
    def apk_api_usage(self, session_id: str) -> Result[JsonObject]:
        try:
            binary = self._apk_binary(session_id)
            data = ApkClient().api_usage(binary)
            return _success(data, session_id=session_id, backend="apk")
        except ApkError as exc:
            return _failure(_as_rpc(exc), session_id=session_id)
        except BaseException as exc:
    ...
    def apk_providers(self, session_id: str) -> Result[JsonObject]:
        try:
            binary = self._apk_binary(session_id)
            data = ApkClient().providers(binary)
            return _success(data, session_id=session_id, backend="apk")
        except ApkError as exc:
            return _failure(_as_rpc(exc), session_id=session_id)
        except BaseException as exc:
    ...
    def apk_native_methods(
        ...
        try:
            binary = self._apk_binary(session_id)
            data = ApkClient().native_methods(binary, offset=offset, limit=limit)
            return _success(data, session_id=session_id, backend="apk")
        except ApkError as exc:
            return _failure(_as_rpc(exc), session_id=session_id)
        except BaseException as exc:
    ...
    def apk_secrets(
        ...
        try:
            binary = self._apk_binary(session_id)
            data = ApkClient().secrets(binary, offset=offset, limit=limit)
            return _success(data, session_id=session_id, backend="apk")
        except ApkError as exc:
            return _failure(_as_rpc(exc), session_id=session_id)
        except BaseException as exc:
