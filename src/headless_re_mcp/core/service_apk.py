...
class ApkAnalysisMixin:
    ...
    def apk_files(self, session_id: str, offset: int = 0, limit: int = 100) -> Result[JsonObject]:
        try:
            binary = self._apk_binary(session_id)
    ...
    def apk_capabilities(self, session_id: str) -> Result[JsonObject]:
        return self._apk_call(session_id, "capabilities")
    ...
    def apk_native_methods(
        ...
        try:
            ...
            return _failure(_as_rpc(exc), session_id=session_id)
        except BaseException as exc:  # noqa: BLE001 - hostile input yields an envelope
            return _failure(exc, session_id=session_id)

    def apk_urls(self, session_id: str, offset: int = 0, limit: int = 100) -> Result[JsonObject]:
        try:
            binary = self._apk_binary(session_id)
    ...
    def apk_secrets(self, session_id: str, offset: int = 0, limit: int = 100) -> Result[JsonObject]:
        try:
            binary = self._apk_binary(session_id)
    ...
    def apk_exported_components(
        ...
        try:
            binary = self._apk_binary(session_id)
            data = ApkClient().exported_components(binary, offset=offset, limit=limit)
            return _success(data, session_id=session_id, backend="apk")
        except ApkError as exc:
            return _failure(_as_rpc(exc), session_id=session_id)
        except BaseException as exc:
    ...
    def apk_intent_filters(
        ...
        try:
            binary = self._apk_binary(session_id)
            data = ApkClient().intent_filters(binary, offset=offset, limit=limit)
            return _success(data, session_id=session_id, backend="apk")
        except ApkError as exc:
            return _failure(_as_rpc(exc), session_id=session_id)
        except BaseException as exc:
    ...
    def apk_deep_links(
        ...
        try:
            binary = self._apk_binary(session_id)
            data = ApkClient().deep_links(binary, offset=offset, limit=limit)
            return _success(data, session_id=session_id, backend="apk")
        except ApkError as exc:
            return _failure(_as_rpc(exc), session_id=session_id)
        except BaseException as exc:
    ...
    def apk_callees(
        self, session_id: str, method_name: str, limit: int = 100
    ) -> Result[JsonObject]:
        try:
            binary = self._apk_binary(session_id)
            data = ApkClient().callees(binary, method_name, limit=limit)
            return _success(data, session_id=session_id, backend="apk")
        except ApkError as exc:
            return _failure(_as_rpc(exc), session_id=session_id)
        except BaseException as exc:
    ...
    def apk_field_xrefs(
        self, session_id: str, field_name: str, limit: int = 100
    ) -> Result[JsonObject]:
        try:
            binary = self._apk_binary(session_id)
            data = ApkClient().field_xrefs(binary, field_name, limit=limit)
            return _success(data, session_id=session_id, backend="apk")
        except ApkError as exc:
            return _failure(_as_rpc(exc), session_id=session_id)
        except BaseException as exc:
