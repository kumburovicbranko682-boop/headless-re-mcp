...
class ApkAnalysisMixin:
    ...
    def apk_class_summary(self, session_id: str, class_name: str) -> Result[JsonObject]:
        try:
            binary = self._apk_binary(session_id)
            data = ApkClient().class_summary(binary, class_name)
            return _success(data, session_id=session_id, backend="apk")
        except ApkError as exc:
            return _failure(_as_rpc(exc), session_id=session_id)
        except BaseException as exc:
    ...
    def apk_subclasses(
        ...
        try:
            binary = self._apk_binary(session_id)
            data = ApkClient().subclasses(binary, class_name, offset=offset, limit=limit)
            return _success(data, session_id=session_id, backend="apk")
        except ApkError as exc:
            return _failure(_as_rpc(exc), session_id=session_id)
        except BaseException as exc:
    ...
    def apk_class_xrefs(
        ...
        try:
            binary = self._apk_binary(session_id)
            data = ApkClient().class_xrefs(
                binary, class_name, direction=direction, offset=offset, limit=limit
            )
            return _success(data, session_id=session_id, backend="apk")
        except ApkError as exc:
            return _failure(_as_rpc(exc), session_id=session_id)
    ...
    def apk_method_bytecode(
        ...
        try:
            binary = self._apk_binary(session_id)
            data = ApkClient().method_bytecode(
                binary,
                class_name,
                method_name,
    ...
    def apk_xrefs(
        ...
        try:
            binary = self._apk_binary(session_id)
            data = ApkClient().xrefs(binary, method_name, direction=direction, limit=limit)
            return _success(data, session_id=session_id, backend="apk")
        except ApkError as exc:
            return _failure(_as_rpc(exc), session_id=session_id)
        except BaseException as exc:
    ...
    def apk_string_xrefs(
        ...
        try:
            binary = self._apk_binary(session_id)
            data = ApkClient().string_xrefs(binary, value, offset=offset, limit=limit)
            return _success(data, session_id=session_id, backend="apk")
        except ApkError as exc:
            return _failure(_as_rpc(exc), session_id=session_id)
        except BaseException as exc:
    ...
    def apk_field_xrefs(
        ...
        try:
            binary = self._apk_binary(session_id)
            data = ApkClient().field_xrefs(
                binary, field_name, direction=direction, offset=offset, limit=limit
            )
            return _success(data, session_id=session_id, backend="apk")
        except ApkError as exc:
            return _failure(_as_rpc(exc), session_id=session_id)
