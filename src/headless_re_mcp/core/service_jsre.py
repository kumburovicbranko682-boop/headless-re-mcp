...
class JsReAnalysisMixin:
    ...
    def js_exports(
        ...
        try:
            data = JsClient(getattr(self.settings, "webcrack", None)).exports(
                Path(path),
                kind=kind,
                contains=contains,
                offset=offset,
    ...
    def wasm_disasm_function(
        ...
        try:
            data = WasmClient(getattr(self.settings, "wabt", None)).disasm_function(
                Path(path),
                index=index,
                offset=offset,
                limit=limit,
    ...
    def wasm_memory(
        ...
        try:
            data = WasmClient(getattr(self.settings, "wabt", None)).memory(
                Path(path),
                timeout=timeout,
            )
            return _success(data, backend="wabt")
