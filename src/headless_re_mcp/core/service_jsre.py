...
class JsReAnalysisMixin:
    ...
    def wasm_names(self, path: str, *, contains: str | None = None) -> Result[JsonObject]:
        try:
            data = WasmClient(getattr(self.settings, "wabt", None)).names(
                Path(path), contains=contains
            )
            return _success(data, backend="wasm_names")
        except JsReError as exc:
            return _failure(_as_rpc(exc))
        except BaseException as exc:
            return _failure(exc)

    def wasm_sections(self, path: str) -> Result[JsonObject]:
        try:
            data = WasmClient(getattr(self.settings, "wabt", None)).sections(Path(path))
            return _success(data, backend="wasm_sections")
        except JsReError as exc:
            return _failure(_as_rpc(exc))
        except BaseException as exc:
            return _failure(exc)

    def wasm_functions(self, path: str, *, contains: str | None = None) -> Result[JsonObject]:
        try:
            data = WasmClient(getattr(self.settings, "wabt", None)).functions(
                Path(path), contains=contains
            )
            return _success(data, backend="wasm_functions")
        except JsReError as exc:
            return _failure(_as_rpc(exc))
        except BaseException as exc:
            return _failure(exc)

    def wasm_exports(self, path: str, *, contains: str | None = None) -> Result[JsonObject]:
        try:
            data = WasmClient(getattr(self.settings, "wabt", None)).exports(
                Path(path), contains=contains
            )
            return _success(data, backend="wasm_exports")
        except JsReError as exc:
            return _failure(_as_rpc(exc))
        except BaseException as exc:
            return _failure(exc)

    def wasm_imports(self, path: str, *, contains: str | None = None) -> Result[JsonObject]:
        try:
            data = WasmClient(getattr(self.settings, "wabt", None)).imports(
                Path(path), contains=contains
            )
            return _success(data, backend="wasm_imports")
        except JsReError as exc:
            return _failure(_as_rpc(exc))
        except BaseException as exc:
            return _failure(exc)

    def wasm_globals(self, path: str) -> Result[JsonObject]:
        try:
            data = WasmClient(getattr(self.settings, "wabt", None)).globals(Path(path))
            return _success(data, backend="wasm_globals")
        except JsReError as exc:
            return _failure(_as_rpc(exc))
        except BaseException as exc:
            return _failure(exc)
