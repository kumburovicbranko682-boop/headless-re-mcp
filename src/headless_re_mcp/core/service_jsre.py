...
class JsReAnalysisMixin:
    ...
    def wasm_start(self, path: str) -> Result[JsonObject]:
        try:
            data = parse_wasm_start(Path(path))
            return _success(data, backend="jsre")
        except JsReError as exc:
            return _failure(_as_rpc(exc))
        except BaseException as exc:
    ...
    def wasm_summary(self, path: str) -> Result[JsonObject]:
        try:
            data = parse_wasm_summary(Path(path))
            return _success(data, backend="jsre")
        except JsReError as exc:
            return _failure(_as_rpc(exc))
        except BaseException as exc:
    ...
    def wasm_opcodes(self, path: str) -> Result[JsonObject]:
        try:
            data = parse_wasm_opcodes(Path(path))
            return _success(data, backend="jsre")
        except JsReError as exc:
            return _failure(_as_rpc(exc))
        except BaseException as exc:
    ...
    def wasm_locals(self, path: str, offset: int = 0, limit: int = 100) -> Result[JsonObject]:
        try:
            data = parse_wasm_locals(Path(path), offset=offset, limit=limit)
            return _success(data, backend="jsre")
        except JsReError as exc:
            return _failure(_as_rpc(exc))
        except BaseException as exc:
    ...
    def wasm_custom_sections(
        ...
        try:
            data = parse_wasm_custom_sections(Path(path), offset=offset, limit=limit)
            return _success(data, backend="jsre")
        except JsReError as exc:
            return _failure(_as_rpc(exc))
        except BaseException as exc:
