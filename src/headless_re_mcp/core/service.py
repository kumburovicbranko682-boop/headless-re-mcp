...
class AnalysisService(
    ...
    def _dynamic_request(
        ...
        *,
        ...
        # The 300-second ceiling mirrors the widest RunControlTimeout the tool
        # boundary accepts, so no caller loses range while nan/inf/negative
        # values stop reaching the deadline arithmetic below.
        if (
            isinstance(timeout, bool)
            or not isinstance(timeout, (int, float))
            or not isfinite(timeout)
            or not 0 < timeout <= 300.0
        ):
            return _failure(
                ValueError("timeout must be greater than 0 and at most 300 seconds"),
                session_id=session_id,
                backend=BackendKind.X64DBG.value,
            )
        requested_timeout = float(timeout)
        # One wall-clock budget covers every stage.  Waiting for the runtime
        # lock used to be unbounded, and the event marker, the RPC, and the
        # state wait each re-spent the full timeout, so a debug.pause with a
        # 100 ms budget could sit for seconds behind a busy lock and still
        # take the whole budget again per stage once it got in.
        ...
        def remaining() -> float:
            ...
            if value <= 0:
                raise XdbgRpcError(
                    "timeout",
                    f"{method} exceeded its {requested_timeout:g}s budget",
        ...
        try:
            runtime = self._runtime(session_id, BackendKind.X64DBG)
            if not runtime.lock.acquire(timeout=remaining()):
                raise XdbgRpcError(
                    "timeout",
