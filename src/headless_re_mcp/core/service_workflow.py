...
class WorkflowAnalysisMixin:
    ...
    def workflow_status(self, session_id: str) -> Result[JsonObject]:
        try:
            ...
            # WorkflowStateOwner synchronizes its own immutable snapshots.
            # Status does not touch the worker, so waiting behind a long RPC
            # only turns an in-memory read into an avoidable hang.
