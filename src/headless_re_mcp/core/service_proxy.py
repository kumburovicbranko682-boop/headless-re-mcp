...
class ProxyAnalysisMixin:
    ...
    def proxy_clear(self, session_id: str) -> Result[JsonObject]:
        try:
            ...
            _timeline_append(self, session_id, "proxy.clear", "capture ring cleared")
            return _success(data, session_id=session_id, backend="proxy")
        except ProxyError as exc:
            return _failure(_as_rpc(exc), session_id=session_id)
        except BaseException as exc:
    ...
    def proxy_endpoints(self, session_id: str, limit: int = 100) -> Result[JsonObject]:
        return self._proxy_wrap(session_id, "endpoints", session_id, limit=limit)

    def proxy_hosts(self, session_id: str, limit: int = 100) -> Result[JsonObject]:
        return self._proxy_wrap(session_id, "hosts", session_id, limit=limit)
    ...
    def proxy_tls(self, session_id: str, limit: int = 100) -> Result[JsonObject]:
        return self._proxy_wrap(session_id, "tls", session_id, limit=limit)
