...
def _frida_rpc(exc: FridaError) -> XdbgRpcError:
    return XdbgRpcError(
        exc.code,
        exc.message,
...
def _r2_rpc(exc: R2Error) -> XdbgRpcError:
    return XdbgRpcError(
        exc.code,
        exc.message,
        details=dict(exc.details),
...
class ExtAnalysisMixin(UiDriveMixin):
    ...
    def r2_open(self, session_id: str, timeout: float = 30.0) -> Result[JsonObject]:
        try:
            ...
            return _failure(_r2_rpc(exc), session_id=session_id)
        except BaseException as exc:
    ...
    def frida_attach(self, session_id: str) -> Result[JsonObject]:
        try:
            ...
            return _failure(_frida_rpc(exc), session_id=session_id)
        except BaseException as exc:
            return _failure(exc, session_id=session_id)
