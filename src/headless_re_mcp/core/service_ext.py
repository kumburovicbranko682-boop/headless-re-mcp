...
class ExtAnalysisMixin(UiDriveMixin):
    ...
    def r2_classes(self, session_id: str, timeout: float = 30.0) -> Result[JsonObject]:
        return _r2_request(self, session_id, ["icj"], timeout=timeout)
    ...
    def ghidra_imports(
        ...
        return _ghidra_export(self, session_id, "imports", limit=limit, timeout=timeout)
    ...
    def ghidra_strings(
        ...
        return _ghidra_export(self, session_id, "strings", limit=limit, timeout=timeout)

    def ghidra_data(
        self, session_id: str, limit: int = 256, timeout: float = 180.0
    ) -> Result[JsonObject]:
        return _ghidra_export(self, session_id, "data", limit=limit, timeout=timeout)
    ...
    *,
    ...
    try:
        ...
        if mode == "functions":
            ...
            data = client.data(session.require_binary(), project, limit=limit, timeout=timeout)
        elif mode == "callgraph":
            if address is None:
