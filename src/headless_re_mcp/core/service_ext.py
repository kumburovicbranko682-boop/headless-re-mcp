...
def _register_capture(
    ...
    *,
    ...
    failure travels in the payload rather than as an exception. ``id_key`` names
    the field the registered id lands under, so a caller that spills more than one
    file in a single payload (a flow's request *and* response body) can register
    each without the second clobbering the first.
    """
    if not path.is_file():
        return payload
    try:
    ...
    return {**payload, id_key: artifact["id"]}
...
class ExtAnalysisMixin(UiDriveMixin):
    ...
    def r2_read(
        self, session_id: str, address: int, size: int = 64, timeout: float = 30.0
    ) -> Result[JsonObject]:
        try:
            session = self.registry.get(session_id)
            if session.state in {
                ...
                raise InvalidStateTransition(
                    f"r2.read cannot run in {session.state.value} state"
                )
            exe = getattr(self.settings, "r2", None)
            client = R2Client(Path(exe) if exe else None)
            data = client.read_bytes(session.require_binary(), address, size=size, timeout=timeout)
            session = self.registry.get(session_id)
            if session.state in {
                SessionState.CLOSING,
                SessionState.CLOSED,
            ...
            _timeline_append(self, session_id, "r2.read", "r2 read bytes", address=address, size=size)
            return _success(data, session_id=session_id, backend="radare2")
        except R2Error as exc:
            return _failure(XdbgRpcError(exc.code, exc.message, details=dict(exc.details)), session_id=session_id)
        except BaseException as exc:
    ...
    def r2_disasm_function(
        ...
        try:
            ...
            if session.state in {
                ...
                raise InvalidStateTransition(
                    f"r2.disasm_function cannot run in {session.state.value} state"
                )
            exe = getattr(self.settings, "r2", None)
            client = R2Client(Path(exe) if exe else None)
            data = client.disasm_function(session.require_binary(), address, timeout=timeout)
            session = self.registry.get(session_id)
            if session.state in {
                SessionState.CLOSING,
                SessionState.CLOSED,
            ...
            _timeline_append(
                self, session_id, "r2.disasm_function", "r2 function disasm", address=address
            )
            return _success(data, session_id=session_id, backend="radare2")
        except R2Error as exc:
            return _failure(XdbgRpcError(exc.code, exc.message, details=dict(exc.details)), session_id=session_id)
    ...
    def r2_resolve(
        ...
        try:
            ...
            if session.state in {
                ...
                raise InvalidStateTransition(
                    f"r2.resolve cannot run in {session.state.value} state"
                )
            exe = getattr(self.settings, "r2", None)
            client = R2Client(Path(exe) if exe else None)
            data = client.resolve(session.require_binary(), address, timeout=timeout)
            session = self.registry.get(session_id)
            if session.state in {
                SessionState.CLOSING,
                SessionState.CLOSED,
            ...
            _timeline_append(
                self, session_id, "r2.resolve", "r2 address resolve", address=address
            )
            return _success(data, session_id=session_id, backend="radare2")
        except R2Error as exc:
            return _failure(XdbgRpcError(exc.code, exc.message, details=dict(exc.details)), session_id=session_id)
    ...
    def r2_cfg(
        ...
        try:
            ...
            if session.state in {
                ...
                raise InvalidStateTransition(
                    f"r2.cfg cannot run in {session.state.value} state"
                )
            exe = getattr(self.settings, "r2", None)
            client = R2Client(Path(exe) if exe else None)
            data = client.cfg(session.require_binary(), address, timeout=timeout)
            session = self.registry.get(session_id)
            if session.state in {
                SessionState.CLOSING,
                SessionState.CLOSED,
            ...
            _timeline_append(
                self, session_id, "r2.cfg", "r2 control-flow graph", address=address
            )
            return _success(data, session_id=session_id, backend="radare2")
        except R2Error as exc:
            return _failure(XdbgRpcError(exc.code, exc.message, details=dict(exc.details)), session_id=session_id)
    ...
    def r2_libs(self, session_id: str, timeout: float = 30.0) -> Result[JsonObject]:
        try:
            ...
            if session.state in {
                ...
                raise InvalidStateTransition(
                    f"r2.libs cannot run in {session.state.value} state"
                )
            exe = getattr(self.settings, "r2", None)
            client = R2Client(Path(exe) if exe else None)
            data = client.libs(session.require_binary(), timeout=timeout)
            session = self.registry.get(session_id)
            if session.state in {
                SessionState.CLOSING,
                SessionState.CLOSED,
            ...
            _timeline_append(self, session_id, "r2.libs", "r2 linked libraries")
            return _success(data, session_id=session_id, backend="radare2")
        except R2Error as exc:
            return _failure(XdbgRpcError(exc.code, exc.message, details=dict(exc.details)), session_id=session_id)
        except BaseException as exc:
