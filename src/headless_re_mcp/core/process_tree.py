...
def terminate_leftover_process_tree(process: Any, *, wait_s: float = 5.0) -> list[int]:
    ...
    with suppress(Exception):
        ...
        # cannot make it hit an unrelated group. The sweep also waits for the
        # kill to land: SIGKILL is asynchronous, and the caller is about to
        # report that nothing of the tool survives.
        killed.extend(terminate_process_group(pid, wait_s=wait_s))
