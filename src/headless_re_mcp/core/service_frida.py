...
class FridaDeviceMixin:
    ...
    def frida_applications(
        ...
        try:
            ...
            data = FridaClient().applications(auth.get("device_id"), offset=offset, limit=limit)
            return _success(data, session_id=session_id, backend="frida")
        except (FridaError, AdbError) as exc:
            return _failure(_as_rpc(exc), session_id=session_id)
        except BaseException as exc:
    ...
    def frida_java_methods(
        self, session_id: str, class_name: str, offset: int = 0, limit: int = 200, pid: int = 0
    ) -> Result[JsonObject]:
        return self._java(
            session_id, "methods", class_name=class_name, offset=offset, limit=limit, pid=pid
        )
