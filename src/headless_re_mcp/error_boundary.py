...
# value masked when it sits under a dict key must also be masked when it appears
# inline in an exception message, because that message is what reaches the
# on-disk incident log, the HTTP 500 body and the CLI stderr envelope. The two
# lists were maintained by hand before and drifted apart -- ``authorization``
# outside the Bearer form and ``providerApiKeys`` were masked in payloads but
# reached the log in the clear -- which is exactly the keyword-list bug this
# closes. The strict ``[:=]`` boundary (rather than a trailing ``\w*``) is
# deliberate -- it keeps "tokenized=false" and similar diagnostics readable. An
# HTTP auth scheme after the separator ("Bearer x", "Basic y") would satisfy the
# value pattern with the scheme word alone and leave the credential standing
# after it, so the scheme is kept and what follows it is what gets masked.
_SECRET_PATTERNS = (
    re.compile(
...
def _redact_text(value: object, *, limit: int = 1000) -> str:
    ...
    # value, a repr() of a header tuple -- is still one recognisable secret. The
    # structured redactor masks this form inside payload strings; the inline
    # scrubber let it through until this line.
