...
def create_server(service: AnalysisService | None = None) -> FastMCP[None]:
    ...
    server: FastMCP[None] = FastMCP(
        "Headless RE-MCP",
        instructions=(
            "Create a session for an authorized local PE, a native ELF/Mach-O, a "
            "local APK, or a web target. PE sessions open static IDA, dynamic "
            "x64dbg, or both. Native ELF/Mach-O sessions use the portable "
            "backends (radare2, ghidra.*); IDA and x64dbg are PE-only. APK and "
            "web sessions have their own tools. Dynamic PE tools expose only "
            "bounded debugger operations; arbitrary x64dbg commands are "
            "unavailable. Every tool returns an ok/data/error/meta envelope. "
