...
def build_apk_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def apk_decompile(
        ...
        Answers with class_name, path, source and bytes, plus truncated when
        the Java was cut at the buffer. bytes is the file's full size, so a cut
        is not read as the whole class. There is no java, code or text field.
        If jadx exited non-zero on the whole-APK pass but still wrote this
        class, the reply carries exit_code, tool_failed and stderr so a partial
