...
def build_apk_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def apk_manifest(session_id: str) -> dict[str, Any]:
        """Return the decoded AndroidManifest.xml (check truncated; large ones are cut)."""
    ...
    def apk_permissions(
        ...
        """List declared and requested permissions (check has_more / total)."""
        return _dump(analysis.apk_permissions(session_id, limit=limit))

    @tools.tool(name="apk.certificates")
    def apk_certificates(
        session_id: str,
        limit: Annotated[int, Field(ge=1, le=200)] = 32,
    ) -> dict[str, Any]:
        """List signing certificates and v1 signature files (check has_more / total)."""
    ...
    def apk_classes(
        ...
        """List internal DEX classes (check has_more / total)."""
    ...
    def apk_methods(
        ...
        """List methods of a class (check has_more / total)."""
        return _dump(
    ...
    def apk_decompile(
        ...
        """Decompile one class to Java via jadx (check truncated; requires jadx + JRE)."""
    ...
    def apk_export_sources(
        ...
        """Decompile the whole APK to a Java source tree via jadx (check has_more)."""
        return _dump(
