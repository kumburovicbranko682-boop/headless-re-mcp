...
def build_apk_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def apk_components(
        ...
        Each list is a window; read `totals` and `has_more` rather than
        assuming any of them is complete.
        """
    ...
    def apk_native_libs(
        ...
        Read `total` and `has_more` rather than assuming the list is complete.
        """
    ...
    def apk_decode(
        ...
        A leftover tree from a previous decode is not success when apktool
        itself failed.
        """
