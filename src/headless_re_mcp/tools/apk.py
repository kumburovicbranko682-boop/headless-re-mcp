...
def build_apk_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def apk_manifest(session_id: str) -> dict[str, Any]:
        ...
        Large manifests are capped. Read `truncated` rather than treating
        `manifest_xml` as the whole file.
        """
    ...
    def apk_classes(
        ...
        Read `total` and `has_more`. `count` is this page, not every class.
        """
