...
def build_apk_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def apk_certificates(session_id: str) -> dict[str, Any]:
        """List signing certificates and signature schemes.

        Answers with certificates (subject, issuer, serial, sha256),
        signature_files, v1_signed, v2_signed, v3_signed, and has_more so a
        list that filled the cap is not read as every signer. v2_signed and
        v3_signed are true/false when androguard can inspect the APK Signing
        Block and null when it cannot, so a modern APK signed only under
        scheme v2/v3 is not read as unsigned from v1_signed alone. There is no
