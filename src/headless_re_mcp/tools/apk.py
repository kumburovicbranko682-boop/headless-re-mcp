...
def build_apk_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def apk_manifest(session_id: str) -> dict[str, Any]:
        ...
        Large manifests are capped. Read `truncated` and `bytes` rather than
        treating `manifest_xml` as the whole file.
        """
    ...
    def apk_permissions(
        ...
        unpaged reply used to return every permission in one go.
        """
    ...
    def apk_native_libs(
        ...
        The list is capped by `limit`. Read `has_more` and `total`: the
        unpaged reply used to return every `.so` path in one go.
        """
    ...
    def apk_classes(
        ...
        Read `has_more` and `total` rather than treating a page as every
        internal class.
        """
    ...
    def apk_export_sources(
        ...
        The on-disk tree is complete. The `java_files` index is capped.
        Read `has_more` and `java_file_count` rather than treating the
        list as every file jadx wrote.
        """
        return _dump(
