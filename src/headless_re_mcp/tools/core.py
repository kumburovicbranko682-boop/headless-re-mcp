...
def build_core_session_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def session_create(
        binary: str,
        target: Annotated[
            Literal["pe", "apk", "web", "binary"] | None,
            Field(description="Force a target kind instead of inferring it"),
        ] = None,
    ) -> dict[str, Any]:
        """Create a session for a local PE, APK, binary, or a web target.
        ...
        so a PE path behaves exactly as before. An ELF or Mach-O image is a
        binary target: r2.* and ghidra.* analyse it, while the PE-only debugger
        tools (IDA, x64dbg) report a target_mismatch against it.
