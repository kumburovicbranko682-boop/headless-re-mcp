...
def build_windbg_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def windbg_modules(
        ...
        The session excerpt is capped: read truncated rather than treating
        modules as the whole lm listing.
        """
    ...
    def windbg_attach(
        ...
        Reads the process without taking control, so x64dbg keeps it. The
        session excerpt is capped: read truncated rather than treating output
        as the whole probe.
        """
