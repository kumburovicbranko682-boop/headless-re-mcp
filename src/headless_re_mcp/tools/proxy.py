...
def build_proxy_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def proxy_flows(
        ...
        row whose request/response body was over the retain cap. A flow
        ...
        with a null status; such flows were previously dropped entirely.
        A completed flow always carries a numeric status and no error field.
