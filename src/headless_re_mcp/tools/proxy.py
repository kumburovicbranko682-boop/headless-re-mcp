...
def build_proxy_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def proxy_flow_get(session_id: str, flow_id: str) -> dict[str, Any]:
        ...
        larger is response.body_path and there is no body key. headers_truncated
        is set on request or response when the header map was capped, so a cut
        map is not read as every header. There are no top-level headers or body
        fields.
        """
