...
def build_proxy_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def proxy_export_har(session_id: str) -> dict[str, Any]:
        ...
        Answers with path, entry_count, size, and truncated so a HAR that was
        cut to fit the capture cap is not read as the whole log. There is no
        har, output or artifact field. path is the file; looking for har after
        a successful export reads as a missing capture.
        """
