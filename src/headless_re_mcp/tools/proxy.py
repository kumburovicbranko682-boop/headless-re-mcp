...
def build_proxy_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def proxy_flows(
        ...
        milliseconds (from mitmproxy's flow timestamps; unmeasured phases are
        absent) -- the same values the HAR export puts in each entry's timings,
        whose non-negative sum is that entry's time. body_omitted is set on
        a row whose
        ...
        carries a numeric status and no error field. A redirect flow (a 3xx
        status) carries redirect_url -- its Location header, where it sent the
        client next -- which the HAR export puts in the entry's redirectURL so a
        viewer draws the chain; a non-3xx flow has no redirect_url. The list
        ...
        page that filled the limit is not the whole log. metadata_truncated marks
        bounded oversized summary fields.
        """
    ...
    def proxy_flow_get(session_id: str, flow_id: str) -> dict[str, Any]:
        ...
        than decoded lossily. A spilled body also carries artifact_id; if
        registering that body failed (a full or locked artifact store) the side
    ...
    def proxy_export_har(session_id: str) -> dict[str, Any]:
        ...
        is the HAR file's byte length. There is no har, output or artifact field.
        path is the file; looking for har after a successful export reads as a
        missing capture.
        """
