...
def build_proxy_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def proxy_flow_get(session_id: str, flow_id: str) -> dict[str, Any]:
        ...
        larger is response.body_path and there is no body key. A binary
        response.body is base64 with response.base64_encoded true; text is
        returned as-is. There are no top-level headers or body fields.
        """
