...
def build_proxy_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def proxy_flows(
        ...
        response_size, status_text, server_ip), count, total, offset, has_more,
        and dropped. response_size is the decoded response body length in bytes
        (0 when the response had no body). status_text is the HTTP reason phrase
        (e.g. OK, Not Found; null when the flow had no response or on HTTP/2,
        which carries none); export_har uses it for the HAR statusText. server_ip
        is the resolved upstream IP mitmproxy connected to (null when the flow
        never connected, e.g. a refused handshake); export_har uses it for the
        HAR serverIPAddress. body_omitted is set on a row whose
