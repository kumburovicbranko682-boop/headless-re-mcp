...
def build_proxy_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def proxy_replay(session_id: str, flow_id: str) -> dict[str, Any]:
        ...
        Answers with replayed, flow_id, and replayed_flow_id -- the fresh id
        the replayed request is recorded under, so its response can be found
        in proxy.flows. A flow the proxy cannot replay (live, intercepted,
        WebSocket, or missing request/content) is refused with the reason
        rather than reported as replayed.
        """
