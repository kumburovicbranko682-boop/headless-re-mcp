...
def build_proxy_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def proxy_status(session_id: str) -> dict[str, Any]:
        ...
        dropped, retained_max, retained_bytes and retained_bytes_max. There is
        no count or flows field. dropped is how many the capture ring already
        evicted, matching proxy.flows: a flow_count sitting at retained_max with
        a rising dropped means flows are being discarded, not merely buffered. A
        session with no proxy answers running false and nothing else, which is
