...
def build_web_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def web_open(
        ...
        Answers with opened, url, title, headless and status -- the HTTP
        status of the main document (null when no URL was opened or the
        navigation produced no response). Playwright only raises on
        network-level failure, so status is how a 404 or 500 landing page
        is told from a clean load. There is no session, browser, ok or page
        field.
        """
    ...
    def web_navigate(
        ...
        Answers with url, title and status -- the HTTP status of the main
        document, or null when the navigation produced no response (e.g. a
        same-document jump). Playwright only raises on network-level
        failure, so status is how a 404 or 500 landing page is told from a
        clean load. There is no navigated, ok or page field.
        """
