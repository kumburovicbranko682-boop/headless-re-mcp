...
def build_web_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def web_network_get(session_id: str, request_id: str) -> dict[str, Any]:
        ...
        body over the capture cap is refused rather than written to disk. A
        request that failed to load carries body_error along with failed=true
        and error_text carried over from the capture, so an empty body is not
