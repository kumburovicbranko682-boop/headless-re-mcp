...
def build_web_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def web_network_list(
        ...
        Paged. Read `has_more` and `total` rather than treating a page as
        every request captured overnight.
        """
    ...
    def web_scripts(
        ...
        Capped by `limit`. Read `has_more` and `total`: the unpaged list used
        to return every parsed script in one reply.
        """
    ...
    def web_wasm_list(
        ...
        Capped by `limit`. Read `has_more` and `total`.
        """
    ...
    def web_dom_snapshot(session_id: str) -> dict[str, Any]:
        ...
        Large pages are capped. Read `truncated` and `bytes` rather than
        treating `html` as the whole document.
        """
