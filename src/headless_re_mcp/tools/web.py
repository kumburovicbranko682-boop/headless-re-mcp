...
def build_web_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def web_har_export(session_id: str) -> dict[str, Any]:
        ...
        field. truncated is true when the newest entries were shed to keep the
        HAR under the capture cap, so entry_count below the captured request
        total is not read as a short capture.
        """
