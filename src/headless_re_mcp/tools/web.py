...
def build_web_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def web_open(
        ...
        Answers with opened, url, title and headless, plus status when a URL was
        given and produced an HTTP response. A 4xx/5xx page still opens, so read
        status to tell an error page from a hit. There is no session, browser, ok
        or page field.
        """
