...
def build_web_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def web_network_get(session_id: str, request_id: str) -> dict[str, Any]:
        ...
        decoded size. Whenever a body spills, the reply also carries
        artifact_id, and the whole spilled body is retrievable with
        artifacts.read on that id (page offset forward) -- not just the bare
        body_path, which nothing on the surface opens. When CDP has no body for
