...
def build_frida_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def frida_modules(
        ...
        """List modules in the attached process.

        Capped; the reply carries total and has_more so a page is not read
        as every module the process loaded.
        """
    ...
    def frida_exports(
        ...
        """List exports of a module in the attached process.

        Capped; the reply carries has_more so a page is not read as every
        export the module published.
        """
