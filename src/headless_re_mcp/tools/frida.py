...
def build_frida_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def frida_modules(
        ...
        read as the whole list and modules past the first page stay reachable
        -- the probe already counts every module, so unlike the java/exports
        probes this one pages by offset. Limited to the debuggee pid.
        """
    ...
    def frida_applications(
        ...
        device and apps past the first page stay reachable -- the enumerate
        returns the whole set, so like frida.modules it pages by offset. The
        list field is applications, not apps or packages.
        """
        return _dump(
            analysis.frida_applications(session_id, offset=offset, limit=limit)
        )
