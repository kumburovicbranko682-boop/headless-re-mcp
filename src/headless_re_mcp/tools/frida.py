...
def build_frida_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def frida_exports(
        ...
        plus count, total and has_more so a page that filled the limit is not
        read as the whole export table -- total is the full export count (the
        probe already enumerates all of them), so a caller can size its next
        limit instead of guessing. Limited to the debuggee pid.
        """
