...
def build_js_wasm_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def js_deobfuscate(
        ...
        the buffer, and partial (with note and exit_code) when webcrack exited
        with errors after producing output. An input over 16 MiB is refused as
    ...
    def js_beautify(
        ...
        truncated when the text was cut at the buffer, and partial (with note
        and exit_code) when webcrack exited with errors after producing output.
    ...
    def js_unpack_bundle(
        ...
        assuming files is complete. Adds partial (with note and exit_code) when
        webcrack exited with errors but still wrote modules, so file_count is
        not the whole bundle. An input over 16 MiB is refused as too_large
    ...
    def wasm_wat(
        ...
        the buffer, and partial (with note and exit_code) when wasm2wat exited
        with errors after producing output. An input over 16 MiB is refused as
        too_large rather than handed to wasm2wat.
        """
    ...
    def wasm_info(
        ...
        truncated when the text was cut at the buffer, and partial (with note
        and exit_code) when wasm-objdump exited with errors after producing
        output. An input over 16 MiB is refused as too_large rather than handed
        to wasm-objdump.
        """
