...
def build_js_wasm_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def js_unpack_bundle(
        ...
        Writes under artifact_root/jsre/; not a registered artifact, and only
        the newest 8 unpack trees are kept. The file list is paged: read
        `total` and `has_more` rather than assuming `files` is complete.
        """
        return _dump(
