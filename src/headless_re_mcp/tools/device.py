...
def build_device_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def device_properties(
        ...
        Capped by `limit`. Read `has_more` rather than treating `count` as the
        whole table: a full page used to look identical to a complete answer.
        """
    ...
    def device_packages(
        ...
        Capped by `limit`. Read `has_more`: the unpaged list used to return
        every package on the device in one reply.
        """
        return _dump(
    ...
    def device_logcat(
        ...
        Read `has_more` rather than treating a full page as the whole
        buffer: an oversized dump used to be sliced with no signal.
        """
