...
def build_device_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def device_list(
        ...
        Serials are sorted, and offset pages through that one stable order:
        advance offset by the page size to reach devices past has_more.
        Offline and unauthorized serials are included; a missing device is
        not the same as an offline one.
        """
