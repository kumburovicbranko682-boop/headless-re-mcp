...
def build_r2_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def r2_xrefs(
        ...
        Answers with items, each carrying direction ("to" means the item
        references address, "from" means address references the item's
        target), from, to, type, from_address and to_address, plus address
        ...
        the cap (4096). There is no integer address, xrefs, truncated or
        has_more field.
        """
