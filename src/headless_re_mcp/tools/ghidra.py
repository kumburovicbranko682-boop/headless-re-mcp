...
def build_ghidra_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def ghidra_analyze(
        ...
        on a large binary. Requires HEADLESS_RE_GHIDRA_HOME. A fat/universal
        Mach-O target requires slice_arch (Ghidra imports one slice at a time);
        the named slice is carved out and imported.
        """
        return _dump(analysis.ghidra_analyze(session_id, timeout=timeout, slice_arch=slice_arch))
    ...
    def ghidra_functions(
        ...
        and architecture once at the top level. A fat/universal Mach-O target
        requires slice_arch naming the slice to analyze (x86, x64, arm or
        arm64); coordinates then use that slice's own base, matching what the
        r2 tools report for the same slice_arch.
        """
        return _dump(
            analysis.ghidra_functions(
                session_id, limit=limit, timeout=timeout, slice_arch=slice_arch
            )
        )
    ...
    def ghidra_symbols(
        ...
        A fat/universal Mach-O target requires slice_arch, as on
        ghidra.functions.
        """
        return _dump(
            analysis.ghidra_symbols(session_id, limit=limit, timeout=timeout, slice_arch=slice_arch)
        )
    ...
    def ghidra_xrefs(
        ...
        count and has_more. Outgoing refs are not listed. A fat/universal
        Mach-O target requires slice_arch, as on ghidra.functions.
        """
        return _dump(
            analysis.ghidra_xrefs(
                session_id, address, limit=limit, timeout=timeout, slice_arch=slice_arch
            )
        )
    ...
    def ghidra_decompile(
        ...
        IDA decompiled differently, or of code it could not. A fat/universal
        Mach-O target requires slice_arch, as on ghidra.functions.
        """
        return _dump(
            analysis.ghidra_decompile(session_id, address, timeout=timeout, slice_arch=slice_arch)
        )
