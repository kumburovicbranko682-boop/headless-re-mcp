...
def build_ghidra_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def ghidra_functions(
        ...
        Adds partial (with note and exit_code) when analyzeHeadless exited with
        errors after exporting, so the listing may be incomplete.
        """
    ...
    def ghidra_symbols(
        ...
        and has_more. The listing does not include a containing scope. Adds
    ...
    def ghidra_decompile(
        ...
        code it could not. Adds partial (with note and exit_code) when
        analyzeHeadless exited with errors after exporting.
        """
