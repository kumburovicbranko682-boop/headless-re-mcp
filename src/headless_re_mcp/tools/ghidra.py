...
def build_ghidra_tools(analysis: AnalysisService) -> tuple[BoundTool, ...]:
    ...
    def ghidra_analyze(
        ...
        """Run Ghidra headless import/analyze over the session binary.

        One-shot: the project is deleted when this returns. Later ghidra.*
        tools import the file again and run their own analyzeHeadless. A long
        timeout here does not pay for those calls. Requires
    ...
    def ghidra_functions(
        ...
        Capped; read `has_more`. `count` is this page, not every function.
        """
    ...
    def ghidra_symbols(
        ...
        Capped; read `has_more`. `count` is this page, not every symbol.
        """
    ...
    def ghidra_xrefs(
        ...
        Capped; read `has_more`. `count` is this page, not every reference.
        """
    ...
    def ghidra_decompile(
        ...
        could not. Long functions are capped; read `truncated`.
        """
