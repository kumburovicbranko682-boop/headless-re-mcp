...
class ApkClient:
    ...
    def classes(self, path: Path, *, offset: int = 0, limit: int = 100) -> JsonObject:
        ...
        names.sort()
        # Clamp at the backend boundary like every sibling paginated surface
        # (web/proxy/jsre/adb): the MCP schema declares offset>=0, but the agent
        # transport invokes handlers straight from model arguments with no schema
        # enforcement, and a negative offset would slice a wrong window and report
        # has_more against it.
