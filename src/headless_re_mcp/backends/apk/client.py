...
class ApkClient:
    ...
    def xrefs(self, path: Path, method_name: str, *, limit: int = 100) -> JsonObject:
        parsed = self._parsed(path)
        # method_name is a handler kwarg the apk.xrefs tool schema types as a
        ...
        # list/int/null reaches ``method_name.strip()``. That raised AttributeError
        ...
        if not isinstance(method_name, str):
            raise ApkError("invalid_params", "method_name must be a string")
