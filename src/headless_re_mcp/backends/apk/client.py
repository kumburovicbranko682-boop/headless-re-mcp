...
class ApkClient:
    ...
    def open(self, path: Path) -> JsonObject:
        ...
        # {opened: True, package: None}. An unattended agent then treats
        # a zip that is not an APK as an opened package.
        if not package:
            raise ApkError(
                "backend_error",
    ...
    def components(self, path: Path, *, limit: int = 500) -> JsonObject:
        ...
        return {
            ...
            "has_more": any(
                len(items) > cap for items in (activities, services, receivers, providers)
            ),
        }
