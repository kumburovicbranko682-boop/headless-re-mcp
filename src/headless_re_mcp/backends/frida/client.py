...
class FridaClient:
    ...
    def applications(
        ...
        # slice over it. Sort by the package identifier first: frida-core does not
        # promise a stable enumeration order, and offset paging is only coherent
        # over a stable order (otherwise page two can repeat or skip apps from page
        # one). identifier is the unique, non-localized key, so sorting on it is
        # deterministic across calls -- the same defensive sort adb.packages and
        # apk.classes apply before they window.
        apps = sorted(apps, key=lambda app: str(app.identifier))
        # The frida.applications schema bounds offset >= 0 and limit within range,
        # but the OpenAI/agent transports call the service directly and bypass that
        # validation; clamp defensively so a negative offset cannot wrap the slice
        # into the tail (the same reason apk.* has _clamp_page). has_more compares
        # the window's far edge against the full count so paging past a filled
        # limit is not read as the whole device.
