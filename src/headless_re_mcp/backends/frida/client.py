...
rpc.exports = {
  ...
  read: function (address, size) {
    // Read through the NativePointer method, not the legacy Memory.readByteArray
    // free function: frida 17 removed the Memory.read* globals, so the old form
    // raised "TypeError: not a function" and frida.memory.read failed on every
    // frida >=17 runtime -- the exact break the live gate reproduced. The pointer
    // method has existed since frida 12, so this works across the whole >=16.5
    // range the android extra pins.
