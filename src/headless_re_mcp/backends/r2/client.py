...
class R2Client:
    ...
    def run(self, binary: Path, commands: list[str], *, timeout: float = 30.0) -> JsonObject:
        ...
        try:
            # Measured: subprocess.run(timeout=) killed the r2 launcher and
            # left the process it started alive, so a timed-out analysis
            # returned while the orphan kept a core and a lock on the sample.
