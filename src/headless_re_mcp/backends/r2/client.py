...
class R2Client:
    ...
    def run(self, binary: Path, commands: list[str], *, timeout: float = 30.0) -> JsonObject:
        ...
        try:
            ...
            # r2 is often a script that starts radare2/rizin. Killing only the
            # script leaves the analysis holding the sample. Measured: a 1s
            # deadline returned in 1.0s and the child was still running.
            raise R2Error(
                "timeout", "r2 timed out", timeout=timeout, killed_pids=exc.killed
            ) from exc
