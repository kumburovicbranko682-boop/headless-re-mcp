...
class AdbBackend:
    ...
    def properties(self, serial: str, *, limit: int = 500) -> JsonObject:
        ...
        window = parsed[: max(1, int(limit))]
        props = dict(window)
        return {
            "properties": props,
            "count": len(props),
            "total": len(parsed),
            "has_more": len(parsed) > len(props),
        }
    ...
    def logcat(self, serial: str, *, lines: int = 200) -> JsonObject:
        ...
        return {
            ...
            "requested": capped,
            "count": len(lines),
            # logcat -t N returns at most N. A full page is indistinguishable
            # from "that is all there is" unless we say so.
            "has_more": len(lines) >= capped,
        }
