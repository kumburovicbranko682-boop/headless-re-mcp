...
class ProxyBackend:
    ...
    def export_har(self, session_id: str, out_path: Path) -> JsonObject:
        ...
        # The flow ring is count-bounded, but every summary still carries a URL
        # up to _MAX_URL_BYTES, so a full ring serialises to tens of megabytes.
        # Cap the written HAR at the same ceiling web.har_export uses and drop
        # the newest entries until it fits, reporting the cut instead of letting
        # an unattended capture fill the artifact root with one oversized file.
        truncated = False
