...
class ProxyBackend:
    ...
    def export_har(self, session_id: str, out_path: Path) -> JsonObject:
        ...
        truncated = False
        # Each summary URL is bounded to 16 KiB, but a capture of URL-heavy
        # flows still serialises past the capture cap once JSON escaping doubles
        # runs of backslashes or quotes. Match web.har_export: drop the newest
        # entries until it fits and say so, rather than write an artifact the
        # cap every other writer honours was meant to forbid.
