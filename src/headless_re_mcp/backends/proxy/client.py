...
def _read_response_body(resp: Any, flow_id: str) -> bytes:
    """Response bytes, raising when a read that should have worked fails.

    Unlike ``_raw_body``, a ``raw_content`` exception here is surfaced rather
    than degraded to empty: ``proxy.flow.get`` otherwise answered status=200,
    size=0, body empty, so an agent read a failed decode as a response that
    sent nothing. A genuinely bodyless response still reads as empty.
    """
    if resp is None:
        return b""
    try:
...
class ProxyBackend:
    ...
    def flow_get(self, session_id: str, flow_id: str, artifact_dir: Path) -> JsonObject:
        ...
        # Unlike a bodyless message, a response body that raises while decoding
        # is surfaced as a failure here rather than degraded to empty: flow.get
        # otherwise answered status=200, size=0, body empty, so an agent read a
        # failed decode as a server that sent nothing.
        response.update(_emit_body(_read_response_body(resp, flow_id), artifact_dir))
