...
class ProxyBackend:
    ...
    def export_har(self, session_id: str, out_path: Path) -> JsonObject:
        ...
        # Bound the artifact before it lands, exactly as web.har_export does.
        # The flow ring is count-capped, but each URL summary can be up to
        # _MAX_URL_BYTES, so a full ring still serialises to tens of megabytes;
        # drop the oldest entries until the file fits rather than writing an
        # unbounded HAR into the session's capture directory.
