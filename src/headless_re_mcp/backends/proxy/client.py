...
class ProxyBackend:
    ...
    def replay(self, session_id: str, flow_id: str) -> JsonObject:
        ...
        try:
            ...
            def _run() -> None:
                try:
                    # replay.client silently drops a flow it cannot replay -- live,
                    # intercepted, WebSocket, or missing request/content -- logging a
                    # warning and returning normally. Reported as-is that skip became
                    # replayed=True for a request that was never sent, so ask the
                    # clientplayback addon the same question it would ask internally
        ...
        # The replay is a fresh flow with its own id; hand it back so the caller can
        # locate the replayed response in proxy.flows rather than guessing which of
        # many entries the replay produced.
        return {
            "replayed": True,
            "flow_id": flow_id,
