...
class ProxyBackend:
    ...
    def export_har(self, session_id: str, out_path: Path) -> JsonObject:
        ...
        truncated = False
        # The summaries are bounded per field, but a full ring of near-cap URLs
        # still serialises into a multi-megabyte file that lands in the artifact
        # root before retention runs. web.har_export drops its newest entries
        # until the HAR fits the capture cap and says so; the proxy sibling wrote
        # whatever the ring held and under-reported it. Match the web behaviour.
