...
class ProxyBackend:
    ...
    def export_har(self, session_id: str, out_path: Path) -> JsonObject:
        ...
        # proxy.flows already discloses dropped (the count-capped ring evicted
        # the oldest flows before the caller ever read them); the HAR export
        # left it out, so a file of entry_count entries read as the whole
        # session. seq is a monotonic per-response counter, so the newest slot's
        # seq minus what the ring still holds is how many earlier flows are gone.
