...
def _bound_nav_timeout(timeout: float) -> float:
    ...
    NaN is refused with the non-positive case (``value != value``): ``min(nan,
    cap)`` returns nan, and ``Future.result(nan)`` neither returns nor bounds the
    wait -- parking the session thread and a pool worker, worse than the
    non-positive case. The schema's ``gt=0`` already excludes NaN, but the agent
    transport bypasses it, the same reason ``clamp_cli_timeout`` rejects it.
    """
    value = float(timeout)
    ...
    def network_get(self, session_id: str, request_id: str, artifact_dir: Path) -> JsonObject:
        ...
        if base64_encoded:
            ...
            try:
                ...
                # Same documented shape as the CDP-no-body path: body,
                # base64_encoded and body_truncated must stay present so a caller
                # reading result["body"] after an error does not hit a missing key.
                # No body decoded, so base64_encoded is False (there is nothing at
                # body_path to read) rather than echoing CDP's base64Encoded flag.
                return {
                    **entry,
                    "body": "",
                    "base64_encoded": False,
