...
def _response_status(response: Any) -> int | None:
    ...
    page.goto only raises for transport failures (DNS, connection refused,
    timeout); a 4xx/5xx main document resolves normally, so without surfacing
    this a navigation onto an error page reports the same success as a real hit.
    goto also returns None for about:blank and same-document navigations, which
    is an absent status rather than a failure.
    """
    if response is None:
        return None
    try:
