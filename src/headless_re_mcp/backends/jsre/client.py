...
class JsClient:
    ...
    def endpoints(self, path: Path, *, offset: int = 0, limit: int = 200) -> JsonObject:
...
class WasmClient:
    ...
    def globals(self, path: Path, *, offset: int = 0, limit: int = 100) -> JsonObject:
        """List defined globals with type/mutability/init (no wabt). Same guards as summary."""
        resolved = _require_existing_file(path, missing="wasm file not found")
        if not _looks_like_wasm(resolved):
            raise JsReError(
                "invalid_params",
        ...
        return list_wasm_globals(data, offset=offset, limit=limit)

    def exports(self, path: Path, *, offset: int = 0, limit: int = 100) -> JsonObject:
        """List exports with resolved function signatures (no wabt). Same guards as summary."""
        resolved = _require_existing_file(path, missing="wasm file not found")
        if not _looks_like_wasm(resolved):
            raise JsReError(
                "invalid_params",
    ...
    def names(self, path: Path, *, offset: int = 0, limit: int = 100) -> JsonObject:
        """Recover module/function/local names from the name section (no wabt)."""
        resolved = _require_existing_file(path, missing="wasm file not found")
        if not _looks_like_wasm(resolved):
            raise JsReError(
                "invalid_params",
        ...
        return list_wasm_names(raw, offset=offset, limit=limit)

    def tables(self, path: Path, *, offset: int = 0, limit: int = 100) -> JsonObject:
        """List table definitions (reftype + limits), imported first (no wabt)."""
        resolved = _require_existing_file(path, missing="wasm file not found")
        if not _looks_like_wasm(resolved):
            raise JsReError(
                "invalid_params",
        ...
        return list_wasm_tables(raw, offset=offset, limit=limit)

    def code(self, path: Path, *, offset: int = 0, limit: int = 100) -> JsonObject:
        """List per-function code bodies (body size, locals), imports skipped (no wabt)."""
        resolved = _require_existing_file(path, missing="wasm file not found")
        if not _looks_like_wasm(resolved):
            raise JsReError(
                "invalid_params",
    ...
    def callgraph(self, path: Path, *, offset: int = 0, limit: int = 100) -> JsonObject:
        """Build the module's static call graph from the code section (no wabt)."""
        resolved = _require_existing_file(path, missing="wasm file not found")
        if not _looks_like_wasm(resolved):
            raise JsReError(
                "invalid_params",
        ...
        return list_wasm_callgraph(raw, offset=offset, limit=limit)
