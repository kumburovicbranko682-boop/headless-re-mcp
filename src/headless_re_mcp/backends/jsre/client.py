...
class JsClient:
    ...
    def unpack_bundle(
        ...
        *,
        ...
        resolved = self._require_input(path)
        # webcrack 2.x refuses an -o directory that already exists ("output
        # directory already exists") and exits non-zero without unpacking, so
        # create only the parent and let webcrack create the leaf itself.
