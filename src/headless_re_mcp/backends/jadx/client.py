...
class JadxClient:
    ...
    def _run(
        ...
        *,
        ...
        # Measured: exit 1 with a leftover Leftover.java from an earlier export
        # still answered java_file_count=1, so an unattended agent treated a
        # failed decompile as recovered sources. Compare fingerprints so a
        # stale tree does not stand in for this run's output.
